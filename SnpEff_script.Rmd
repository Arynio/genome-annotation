...
#2a: Toy annotation using the immunocapture VCF.
...
#2b: Annotation using the contemporary lp and ll VCFs.
...
for pop in "${POPLIST[@]}"
  ...
  #done
  cd $C_PATH
  java -Xmx16g -jar $S_PATH/snpEff.jar LYPA.23 -v -s $O_PATH/c_ll_lp_perpop_standard_filter_annotation/c_ll_lp_perpop_standard_filter_annotation.ann.html $V_PATH/${pop} > $O_PATH/c_ll_lp_perpop_standard_filter_annotation/${pop/.vcf/.ann.vcf} #run this code from the directory where the config is located.
done
