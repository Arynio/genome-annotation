...
INDIVIDUALS=$(bcftools query -l ${CALLING}"_polarized_filtered5_"${TYPE}".lr_ann.vcf" | sort | uniq)
for k in ${INDIVIDUALS[@]}
  do
  echo "${k}"
  ID=$(echo "${k}")
...
docker run -t -i ensemblorg/ensembl-vep ./vep -i c_lp_5x_c_lp_sm_nm2_origcov_perpop_SNP.lr_ann.vcf -gff LYPA23C.all.fix.nr.gff3.gz -fasta c_lr_zz_0001_recal1.fa.gz -vcf -species "lynx_pardinus"
