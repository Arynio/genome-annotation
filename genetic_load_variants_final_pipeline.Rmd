...
echo 'synonymous_variant' >> features_derived_allele_freq_ratio.txt
echo 'missense_variant' >> features_derived_allele_freq_ratio.txt
...
COVERAGE=$(echo "${CALLING}" | rev | cut -d'_' -f1 | rev)
NM_COV=$(echo "${CALLING}" | rev | cut -d'_' -f1,2 | rev)
FEATURES=$(cat /GRUPOS/grupolince/lynx_genomes_5x/genetic_load_analysis/derived_allele_freq_ratio/features_derived_allele_freq_ratio.txt)
cd $V_PATH/$CALLING/annotation
POP_VCFS=($(ls `find . -name *'_perpop.lr_ann.vcf' -print`))
mkdir -p derived_allele_freq_ratio
rm derived_allele_freq_ratio/"${CALLING}_derived_freq_ratio.txt"
echo -e "population_1\tpopulation_2\tfeature\tratio" > derived_allele_freq_ratio/"${CALLING}_derived_freq_ratio.txt"
...
for f in ${FEATURES[@]}
  ...
  for p in ${POP_VCFS[@]}
    ...
    grep "${f}" "${p}" | awk -F"\t|;|=" '{printf ("%s_%s_%s\t%s\n", $1,$2-1,$2,$13)}' > ${p/perpop.lr_ann.vcf/perpop_"${f}".lr_ann.bed}
...
for f in ${FEATURES[@]}
  ...
  for p1 in ${POPS[@]}
    ...
    for p2 in ${POPS[@]}
      ...
      echo -e "$p1\t$p2\t$f\t$RATIO" >> derived_allele_freq_ratio/"${CALLING}_derived_freq_ratio.txt"
      done
    done
  done
