...
#8: Filter the annotated VCF. Subset the VCF files in order to keep only good quality biallelic variants of the desired type (SNPs or INDELs).
##Separate callings:
...
if [ $TYPE != "SNP" ] && [ $TYPE != "INDEL" ] #Don't apply filters if $TYPE is invalid
  ...
  #Filter 3: Remove polarized ANC fixed variants (AF=0) or those that weren't polarizable (AA different from either REF or ALT):
  echo "Filtering out non-polarizable and fixed ancestral variants"
  $BCF view -e '(INFO/AA!=REF & INFO/AA!=ALT) | INFO/AF=0.00' ${CALLING}"_polarized_filtered2_"${TYPE}".lr_ann.vcf" > ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf"
  ...
  #Filter 5: Finally, for each species exclude those positions that have more than 20% missing genotypes.
  echo "Filtering out high missingness variants"
  $BCF filter -e "F_MISSING > 0.2" -Ov -o ${CALLING}"_polarized_filtered5_"${TYPE}".lr_ann.vcf" ${CALLING}"_polarized_filtered4_"${TYPE}".lr_ann.vcf"
...
if [ $TYPE != "SNP" ] && [ $TYPE != "INDEL" ] #Don't apply filters if $TYPE is invalid
  ...
  if [ -f ${CALLING}"_polarized_filtered1.lr_ann.vcf" ] #Since filter 1 is common for both types of variants, skip it if it's been performed before
    ...
    #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov (XL nr filtered) 5784144
  ...
  -select "QUAL >= 30 && QD >= 2.0 && FS <= 60.0 && MQ >= 40.0 && MQRankSum >= -12.5 && ReadPosRankSum >= -8.0" \
  -R $REF \
  -V ${CALLING}"_polarized_filtered3_"${TYPE}"L.lr_ann.vcf" \
  -o ${CALLING}"_polarized_filtered4_"${TYPE}".lr_ann.vcf"
...
#9: Obtain per dataset VCFs. Split the VCF into per datasets VCFs. Generate a VCF for each dataset in order to apply proper depth filters.
##Separate callings:
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "perdataset_varssubs_${CALLING}_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
for j in ${DATASETS[@]}
  ...
  -R $REF \
  -V ${CALLING}"_polarized_filtered5_"${TYPE}".lr_ann.vcf" \
  -o ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset_varssubs_"${TYPE}".lr_ann.vcf" \
  -env \
  --sample_file /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered/${j}_samples
  grep -v '#' ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset_varssubs_"${TYPE}".lr_ann.vcf" | wc -l #
  done
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_depth_filter_varssubs_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
for j in ${DATASETS[@]}
  ...
  echo $MIN_DP
  echo $MAX_DP
  $BCF filter -i "DP < ${MIN_DP} || DP > ${MAX_DP}" -Ov -o ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_minmaxdepth_to_remove_varssubs_"${TYPE}".lr_ann.vcf" ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset_varssubs_"${TYPE}".lr_ann.vcf"
  sed -e 's/chr//' ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_minmaxdepth_to_remove_varssubs_"${TYPE}".lr_ann.vcf" | awk '{OFS="\t"; if (!/^#/){print $1,$2-1,$2}}' > ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_minmaxdepth_to_remove_varssubs_"${TYPE}".bed" #convert from VCF to BED
  done
cat *"_"${NM_COV}"_perdataset/"*"_"${NM_COV}"_minmaxdepth_to_remove_varssubs_"${TYPE}".bed" | bedtools sort | uniq > ${CALLING}"_joined_minmaxdepth_to_remove_varssubs_"${TYPE}".bed" #join all BEDs from each species (i.e. from each calling)
for j in ${DATASETS[@]}
  do
  echo "${j}"
  bedtools subtract -a ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset_varssubs_"${TYPE}".lr_ann.vcf" -b ${CALLING}"_joined_minmaxdepth_to_remove_varssubs_"${TYPE}".bed" -header > ${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset_filtered_varssubs_"${TYPE}".lr_ann.vcf"
  done

```

#12: Obtain per population VCFs. Split the VCF into per population VCFs. Generate a VCF for each population.
##Separate callings:
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "perpop_varssubs_${CALLING}_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
for i in ${DATASETS[@]}
  do
  echo "${i}"
  declare POP=$(bcftools query -l ${i}"_"${NM_COV}"_perdataset/"${i}"_"${NM_COV}"_perdataset_filtered_varssubs_"${TYPE}".lr_ann.vcf" | cut -c1-7 | sort | uniq)
  for j in ${POP[@]}
    do
    echo "${j}"
    rm ${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt"
    $BCF query -l ${i}"_"${NM_COV}"_perdataset/"${i}"_"${NM_COV}"_perdataset_filtered_varssubs_"${TYPE}".lr_ann.vcf" | grep "${j}" > ${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt"
    cat ${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt"
    mkdir -p "${j}"_"${NM_COV}"_perpop
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V ${i}"_"${NM_COV}"_perdataset/"${i}"_"${NM_COV}"_perdataset_filtered_varssubs_"${TYPE}".lr_ann.vcf" \
    -o ${j}"_"${NM_COV}"_perpop/"${i}"_"${j}"_"${NM_COV}"_perpop_varssubs_"${TYPE}".lr_ann.vcf" \
    -env \
    --sample_file ${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt"
    rm ${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt"
    grep -v '#' ${j}"_"${NM_COV}"_perpop/"${i}"_"${j}"_"${NM_COV}"_perpop_varssubs_"${TYPE}".lr_ann.vcf" | wc -l #
    done
  done

```

##Separate callings (transversions only).
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "perpop_TVonly_${CALLING}_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
for i in "${POPLIST[@]}"
  ...
  echo "${ind}"
  awk -F '\t' '(!($4 == "A" && $5 == "G") && !($4 == "G" && $5 == "A") && !($4 == "C" && $5 == "T") && !($4 == "T" && $5 == "C") || $0 ~ /^#/ )' ${i} > ${i/_perpop_${TYPE}.lr_ann.vcf/_perpop_TVonly_${TYPE}.lr_ann.vcf}
  chgrp grupolince ${i/_perpop_${TYPE}.lr_ann.vcf/_perpop_TVonly_${TYPE}.lr_ann.vcf}
  done
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "individual_IBD_notIBD_${CALLING}_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
for i in "${INDLIST[@]}"
  ...
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}' | cut -c1-12)
  echo "${ind}"
  bedtools intersect -a ${i} -b /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/ngsF-HMM/*/${ind}.ibdtract.bed -header > ${i/_individual_${TYPE}.lr_ann.vcf/_individual_IBD_${TYPE}.lr_ann.vcf}
  bedtools intersect -a ${i} -b <(awk '$4 >= 1000000' /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/ngsF-HMM/*/${ind}.ibdtract.bed) -header > ${i/_individual_${TYPE}.lr_ann.vcf/_individual_IBD1000k_${TYPE}.lr_ann.vcf}
  bedtools intersect -a ${i} -b <(awk '$4 >= 100000 && $4 < 1000000' /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/ngsF-HMM/*/${ind}.ibdtract.bed) -header > ${i/_individual_${TYPE}.lr_ann.vcf/_individual_IBD100k_${TYPE}.lr_ann.vcf}
  bedtools intersect -a ${i} -b <(awk '$4 >= 10000 && $4 < 100000' /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/ngsF-HMM/*/${ind}.ibdtract.bed) -header > ${i/_individual_${TYPE}.lr_ann.vcf/_individual_IBD10k_${TYPE}.lr_ann.vcf}
  bedtools subtract -a ${i} -b /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/ngsF-HMM/*/${ind}.ibdtract.bed -header > ${i/_individual_${TYPE}.lr_ann.vcf/_individual_notIBD_${TYPE}.lr_ann.vcf}
  done
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "individual_TVonly_${CALLING}_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
for i in "${INDLIST[@]}"
  ...
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}' | cut -c1-12)
  echo "${ind}"
  awk -F '\t' '(!($4 == "A" && $5 == "G") && !($4 == "G" && $5 == "A") && !($4 == "C" && $5 == "T") && !($4 == "T" && $5 == "C") || $0 ~ /^#/ )' ${i} > ${i/_individual_${TYPE}.lr_ann.vcf/_individual_TVonly_${TYPE}.lr_ann.vcf}
  chgrp grupolince ${i/_individual_${TYPE}.lr_ann.vcf/_individual_TVonly_${TYPE}.lr_ann.vcf}
  done
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "individual_varssubs_${CALLING}_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
for i in ${POP[@]}
  ...
  echo "${i}"
  mkdir -p "${i}"_"${NM_COV}"_individuals
  VCF_LIST=$(ls ${i}"_"${NM_COV}"_perpop/"*"_"${i}"_"${NM_COV}"_perpop_varssubs_"${TYPE}".lr_ann.vcf")
  for j in ${VCF_LIST[@]}
    do 
    INDIVIDUALS=$(bcftools query -l "${j}" | sort | uniq)
    for k in ${INDIVIDUALS[@]}
      ...
      -R $REF \
      -V "${j}" \
      -o ${i}"_"${NM_COV}"_individuals/"${k}"_"${NM_COV}"_individual_varssubs_"${TYPE}".lr_ann.vcf" \
      -env \
      -sn $ID
      done
    done
...
#16: Obtain per population diversity estimates. Join María's diversity estimates with the rest of the information on variants.
##Join diversity and VCF data.
###Option A: the good one.
####Whole-genome.
```{r Obtain diversity estimates, eval=FALSE, engine='bash'}

#This code should be run in server B using María's account. Otherwise some files won't be accessible.
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_diversity_estimates_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
NM_COV=$(echo "${CALLING}" | rev | cut -d'_' -f1,2 | rev)
cd $V_PATH/$CALLING/annotation
POP_VCFS=($(ls `find . -name *'_5x_'*'_perpop_'$TYPE'.lr_ann.vcf' -print`))
for p in ${POP_VCFS[@]}
  do
  POP_SHORT=$(echo ${p} | rev | cut -d'/' -f1 | rev | cut -d'_' -f4-6)
  echo "retrieving VCF information from variants from population" $POP_SHORT
  grep -v '#' ${p} | awk -F"\t|;|=" '{printf ("%s\t%s\t%s\t%s\t%s\n", $1,$2-1,$2,$13,$17)}' > ${p/perpop_${TYPE}.lr_ann.vcf/perpop_${TYPE}.lr_ann.bed}
  NUMBER_IND=$(printf "n%03d" `$BCF query -l ${CALLING}_polarized.lr_ann.vcf | grep $POP_SHORT | wc -l`) #retrieve the total number of individuals in the population, irrespective of the dataset
  DIV_FILES=$(ls /home/mlucena/ANGSD_analysis/whole_genome_analysis/sfs/${POP_SHORT}_${NUMBER_IND}_separated_by_scaffold/${POP_SHORT}_${NUMBER_IND}.transformedThetas_lp23.*)
  rm -f ${POP_SHORT}_${NM_COV}_perpop/${POP_SHORT}_${NUMBER_IND}_variants_diversity_${TYPE}.lr_ann.bed
  echo "retrieving diversity information from variants from population" $POP_SHORT
  for scaffold in ${DIV_FILES[@]}
    do
    echo "processing scaffold" $(echo ${scaffold} | rev | cut -d'_' -f1 | rev)
    bedtools intersect -a ${scaffold} -b ${p/perpop_${TYPE}.lr_ann.vcf/perpop_${TYPE}.lr_ann.bed} | awk -F"\t" '{printf ("%s_%s_%s\t%s\t%s\n", $1,$2,$3,$4,$5)}' >> ${POP_SHORT}_${NM_COV}_perpop/${POP_SHORT}_${NUMBER_IND}_variants_diversity_${TYPE}.lr_ann.bed
    done
  echo "joining VCF and diversity information from variants from population" $POP_SHORT
  LANG=en_EN join -j 1 -t $'\t' <(awk -F"\t" '{printf ("%s_%s_%s\t%s\t%s\n", $1,$2,$3,$4,$5)}' ${p/perpop_${TYPE}.lr_ann.vcf/perpop_${TYPE}.lr_ann.bed} | LANG=en_EN sort -k1) <(LANG=en_EN sort -k1 ${POP_SHORT}_${NM_COV}_perpop/${POP_SHORT}_${NUMBER_IND}_variants_diversity_${TYPE}.lr_ann.bed) | awk -F"\t" '{gsub(/[_]/, "\t", $1); printf ("%s\t%s\t%s\t%s\t%s\n", $1,$2,$4,$5,$3)}' | sort -k1,1 -k2n > ${p/perpop_${TYPE}.lr_ann.vcf/perpop_diversity_${TYPE}.lr_ann.bed}
  chgrp grupolince ${p/perpop_${TYPE}.lr_ann.vcf/perpop_diversity_${TYPE}.lr_ann.bed}
  done
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_TVonly_diversity_estimates_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
NM_COV=$(echo "${CALLING}" | rev | cut -d'_' -f1,2 | rev)
cd $V_PATH/$CALLING/annotation
POP_VCFS=($(ls `find . -name *'_5x_'*'_perpop_TVonly_'$TYPE'.lr_ann.vcf' -print`))
for p in ${POP_VCFS[@]}
  do
  POP_SHORT=$(echo ${p} | rev | cut -d'/' -f1 | rev | cut -d'_' -f4-6)
  echo "retrieving VCF information from variants from population" $POP_SHORT
  grep -v '#' ${p} | awk -F"\t|;|=" '{printf ("%s\t%s\t%s\t%s\t%s\n", $1,$2-1,$2,$13,$17)}' > ${p/perpop_TVonly_${TYPE}.lr_ann.vcf/perpop_TVonly_${TYPE}.lr_ann.bed}
  NUMBER_IND=$(printf "n%03d" `$BCF query -l ${CALLING}_polarized.lr_ann.vcf | grep $POP_SHORT | wc -l`) #retrieve the total number of individuals in the population, irrespective of the dataset
  DIV_FILES=$(ls /home/mlucena/ANGSD_analysis/whole_genome_analysis/sfs/${POP_SHORT}_${NUMBER_IND}_separated_by_scaffold/${POP_SHORT}_${NUMBER_IND}.transformedThetas_lp23.*)
  rm -f ${POP_SHORT}_${NM_COV}_perpop/${POP_SHORT}_${NUMBER_IND}_TVonly_variants_diversity_${TYPE}.lr_ann.bed
  echo "retrieving diversity information from variants from population" $POP_SHORT
  for scaffold in ${DIV_FILES[@]}
    do
    echo "processing scaffold" $(echo ${scaffold} | rev | cut -d'_' -f1 | rev)
    bedtools intersect -a ${scaffold} -b ${p/perpop_TVonly_${TYPE}.lr_ann.vcf/perpop_TVonly_${TYPE}.lr_ann.bed} | awk -F"\t" '{printf ("%s_%s_%s\t%s\t%s\n", $1,$2,$3,$4,$5)}' >> ${POP_SHORT}_${NM_COV}_perpop/${POP_SHORT}_${NUMBER_IND}_TVonly_variants_diversity_${TYPE}.lr_ann.bed
    done
  echo "joining VCF and diversity information from variants from population" $POP_SHORT
  LANG=en_EN join -j 1 -t $'\t' <(awk -F"\t" '{printf ("%s_%s_%s\t%s\t%s\n", $1,$2,$3,$4,$5)}' ${p/perpop_TVonly_${TYPE}.lr_ann.vcf/perpop_TVonly_${TYPE}.lr_ann.bed} | LANG=en_EN sort -k1) <(LANG=en_EN sort -k1 ${POP_SHORT}_${NM_COV}_perpop/${POP_SHORT}_${NUMBER_IND}_TVonly_variants_diversity_${TYPE}.lr_ann.bed) | awk -F"\t" '{gsub(/[_]/, "\t", $1); printf ("%s\t%s\t%s\t%s\t%s\n", $1,$2,$4,$5,$3)}' | sort -k1,1 -k2n > ${p/perpop_TVonly_${TYPE}.lr_ann.vcf/perpop_TVonly_diversity_${TYPE}.lr_ann.bed}
  chgrp grupolince ${p/perpop_TVonly_${TYPE}.lr_ann.vcf/perpop_TVonly_diversity_${TYPE}.lr_ann.bed}
  done
...
for p in ${POP_VCFS[@]}
  do
  POP_SHORT=$(echo ${p} | rev | cut -d'/' -f1 | rev | cut -d'_' -f4-6)
  echo "retrieving target coordinates from population" $POP_SHORT
  grep -v '#' ${p} | awk -F"\t|;|=" '{printf ("%s\t%s\t%s\t%s\t%s\n", $1,$2-1,$2,$13,$17)}' > ${p/perpop_${TYPE}.lr_ann.vcf/perpop_${TYPE}.lr_ann.bed}
  NUMBER_IND=$(printf "n%03d" `$BCF query -l ${CALLING}_polarized.lr_ann.vcf | grep $POP_SHORT | wc -l`) #retrieve the total number of individuals in the population, irrespective of the dataset
  NUMBER_POS=$(cat ${p} | wc -l)
  echo -e "scaffold\tstart_pos\end_pos\ttotal_V\tAF\tWatterson\tPi\tANN" > ${p/perpop_${TYPE}.lr_ann.vcf/perpop_diversity_${TYPE}.lr_ann.bed}
  i=0
  echo "processing variants from population" $POP_SHORT
  while ((i++)); IFS=$'\t' read -r SCAFFOLD START_POS END_POS AF ANN
    do
    if (($i % 1000 == 0))
      then echo $i "processed variants out of" $NUMBER_POS
    fi
    DIV_FILE=$(find /home/mlucena/ANGSD_analysis/whole_genome_analysis/sfs/${POP_SHORT}_${NUMBER_IND}_separated_by_scaffold/ -name ${POP_SHORT}_${NUMBER_IND}.transformedThetas_$SCAFFOLD)
    DIV_ROW=$(grep $SCAFFOLD$'\t'$START_POS$'\t'$END_POS $DIV_FILE | awk -F"\t" '{print $4,$5}' OFS="\t")
    echo -e "$SCAFFOLD\t$START_POS\t$END_POS\t$AF\t$DIV_ROW\t$ANN" >> ${p/perpop_${TYPE}.lr_ann.vcf/perpop_diversity_${TYPE}.lr_ann.bed} #the diversity information will refer to all individuals sampled from one population, irrespective of the dataset, while the rest of the information will be relative to the 5x individuals only
    done < ${p/perpop_${TYPE}.lr_ann.vcf/perpop_${TYPE}.lr_ann.bed}    
  done

```

##Calculate per-feature diversity values.
```{r Get annotation statistics, eval=FALSE, engine='bash'}

CALLING=(c_ll_ki_c_ll_no_c_ll_po_nm3_origcov) #c_lp_sm_c_lp_do_nm2_origcov #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov
TYPE=(SNP) #write down SNP or INDEL
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation
screen -S "${CALLING}-${TYPE}"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_ann_population_summary_diversity_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
cd $V_PATH/$CALLING/annotation
rm ${CALLING}"_ann_population_summary_diversity_"${TYPE}".lr_ann.txt"
echo -e "species\tpopulation\tdataset\ttotal_Z_DIV\ttotal_P_DIV\tintergenic_Z_DIV\tintergenic_P_DIV\tintronic_Z_DIV\tintronic_P_DIV\tcoding_Z_DIV\tsynonymous_Z_DIV\tsynonymous_P_DIV\tmissense_Z_DIV\tmissense_P_DIV\tnonsense_Z_DIV\tnonsense_P_DIV\tUCNE_Z_DIV\tUCNE_P_DIV" > ${CALLING}"_ann_population_summary_diversity_"${TYPE}".lr_ann.txt"
POPLIST=($(ls `find . -name *"_perpop_diversity_"${TYPE}".lr_ann.bed" -print`))
for i in "${POPLIST[@]}"
  do
  echo "${i}"
  ...
  TOTAL_Z=$(grep -v '#' ${i} | cut -f5 | paste -sd+ | bc)
  TOTAL_Z_DIV=$(echo "scale=4; $TOTAL_Z/$TOTAL_V" | bc | awk '{printf "%f", $0}')
  TOTAL_P=$(grep -v '#' ${i} | cut -f6 | paste -sd+ | bc)
  TOTAL_P_DIV=$(echo "scale=4; $TOTAL_P/$TOTAL_V" | bc | awk '{printf "%f", $0}')
  INTERGENIC_V=$(grep 'intergenic' ${i} | wc -l)
  INTERGENIC_Z=$(grep 'intergenic' ${i} | cut -f5 | paste -sd+ | bc)
  INTERGENIC_Z_DIV=$(echo "scale=4; $INTERGENIC_Z/$INTERGENIC_V" | bc | awk '{printf "%f", $0}')
  INTERGENIC_P=$(grep 'intergenic' ${i} | cut -f6 | paste -sd+ | bc)
  INTERGENIC_P_DIV=$(echo "scale=4; $INTERGENIC_P/$INTERGENIC_V" | bc | awk '{printf "%f", $0}')
  INTRONIC_V=$(grep 'intron_variant' ${i} | wc -l)
  INTRONIC_Z=$(grep 'intron_variant' ${i} | cut -f5 | paste -sd+ | bc)
  INTRONIC_Z_DIV=$(echo "scale=4; $INTRONIC_Z/$INTRONIC_V" | bc | awk '{printf "%f", $0}')
  INTRONIC_P=$(grep 'intron_variant' ${i} | cut -f6 | paste -sd+ | bc)
  INTRONIC_P_DIV=$(echo "scale=4; $INTRONIC_P/$INTRONIC_V" | bc | awk '{printf "%f", $0}')
  CODING_V=$(grep 'CDS' ${i} | wc -l)
  SYNONYMOUS_V=$(grep 'synonymous_variant' ${i} | wc -l)
  SYNONYMOUS_Z=$(grep 'synonymous_variant' ${i} | cut -f5 | paste -sd+ | bc)
  SYNONYMOUS_Z_DIV=$(echo "scale=4; $SYNONYMOUS_Z/$SYNONYMOUS_V" | bc | awk '{printf "%f", $0}')
  SYNONYMOUS_P=$(grep 'synonymous_variant' ${i} | cut -f6 | paste -sd+ | bc)
  SYNONYMOUS_P_DIV=$(echo "scale=4; $SYNONYMOUS_P/$SYNONYMOUS_V" | bc | awk '{printf "%f", $0}')
  MISSENSE_V=$(grep 'missense_variant' ${i} | wc -l)
  MISSENSE_Z=$(grep 'missense_variant' ${i} | cut -f5 | paste -sd+ | bc)
  MISSENSE_Z_DIV=$(echo "scale=4; $MISSENSE_Z/$MISSENSE_V" | bc | awk '{printf "%f", $0}')
  MISSENSE_P=$(grep 'missense_variant' ${i} | cut -f6 | paste -sd+ | bc)
  MISSENSE_P_DIV=$(echo "scale=4; $MISSENSE_P/$MISSENSE_V" | bc | awk '{printf "%f", $0}')
  NONSENSE_V=$(grep '|HIGH|' ${i} | wc -l)
  NONSENSE_Z=$(grep '|HIGH|' ${i} | cut -f5 | paste -sd+ | bc)
  NONSENSE_Z_DIV=$(echo "scale=4; $NONSENSE_Z/$NONSENSE_V" | bc | awk '{printf "%f", $0}')
  NONSENSE_P=$(grep '|HIGH|' ${i} | cut -f6 | paste -sd+ | bc)
  NONSENSE_P_DIV=$(echo "scale=4; $NONSENSE_P/$NONSENSE_V" | bc | awk '{printf "%f", $0}')
  UCNE_V=$(grep 'UCNE' ${i} | wc -l)
  UCNE_Z=$(grep 'UCNE' ${i} | cut -f5 | paste -sd+ | bc)
  UCNE_Z_DIV=$(echo "scale=4; $UCNE_Z/$UCNE_V" | bc | awk '{printf "%f", $0}')
  UCNE_P=$(grep 'UCNE' ${i} | cut -f6 | paste -sd+ | bc)
  UCNE_P_DIV=$(echo "scale=4; $UCNE_P/$UCNE_V" | bc | awk '{printf "%f", $0}')
  echo -e "$SPECIES\t$POPULATION\t$DATASET\t$TOTAL_Z_DIV\t$TOTAL_P_DIV\t$INTERGENIC_Z_DIV\t$INTERGENIC_P_DIV\t$INTRONIC_Z_DIV\t$INTRONIC_P_DIV\t$CODING_Z_DIV\t$SYNONYMOUS_Z_DIV\t$SYNONYMOUS_P_DIV\t$MISSENSE_Z_DIV\t$MISSENSE_P_DIV\t$NONSENSE_Z_DIV\t$NONSENSE_P_DIV\t$UCNE_Z_DIV\t$UCNE_P_DIV" >> ${CALLING}"_ann_population_summary_diversity_"${TYPE}".lr_ann.txt"
  done
...
INDIVIDUALS=$(bcftools query -l ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" | sort | uniq)
for k in ${INDIVIDUALS[@]}
  do
  echo "${k}"
  ID=$(echo "${k}")
...
NM_COV=$(echo "${CALLING}" | rev | cut -d'_' -f1,2 | rev)
cd $V_PATH/$CALLING/annotation
declare INDIVIDUALS=$(bcftools query -l ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" | cut -c1-12 | sort | uniq)
for i in ${INDIVIDUALS[@]}
  ...
  -R $REF \
  -V ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" \
  -o "testing_variant_numbers/"${i}"_"${NM_COV}"_individual_varssubs_"${TYPE}".lr_ann.vcf" \
  -env \
  -sn ${i}
  done
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_ann_individual_varssubs_summary_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
#It works but we need to build first the SIFT scores database.
docker run -t -i -v $HOME/vep_data:/opt/vep/.vep ensemblorg/ensembl-vep ./vep -i /opt/vep/.vep/c_lp_5x_c_lp_sm_nm2_origcov_perpop_SNP.lr_ann.vcf -gff /opt/vep/.vep/LYPA23C.all.fix.nr.gff3.gz -fasta /opt/vep/.vep/c_lr_zz_0001_recal1.fa.gz -o /opt/vep/.vep/c_lp_5x_c_lp_sm_nm2_origcov_perpop_SNP.lr_ann_vep.vcf -vcf -species "lynx_pardinus"
...
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov) #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_nm2_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
TYPE=(SNP) #write down SNP or INDEL
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_testing_hard_filters_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
rm ${CALLING}_testing_hard_filters_${TYPE}.lr_ann.txt
echo "QUAL"
grep -v '#' ../${CALLING}_polarized_filtered3_${TYPE}.lr_ann.vcf | awk -F"\t" '{printf ("%s\t%s\t%s\n", "QUAL",$6,"pre")}' >> ${CALLING}_testing_hard_filters_${TYPE}.lr_ann.txt
echo "pre-filter complete"
grep -v '#' ../${CALLING}_polarized_filtered4_${TYPE}.lr_ann.vcf | awk -F"\t" '{printf ("%s\t%s\t%s\n", "QUAL",$6,"post")}' >> ${CALLING}_testing_hard_filters_${TYPE}.lr_ann.txt
echo "post-filter complete"
ANNOTATIONS=(QD FS MQ MQRankSum ReadPosRankSum)
for a in ${ANNOTATIONS[@]}
  do
  echo "${a}"
  grep -v '#' ../${CALLING}_polarized_filtered3_${TYPE}.lr_ann.vcf | awk -F"${a}=" '{print $2}' | awk -v ann="$a" -F";" '{printf ("%s\t%s\t%s\n", ann,$1,"pre")}' >> ${CALLING}_testing_hard_filters_${TYPE}.lr_ann.txt
  echo "pre-filter complete"
  grep -v '#' ../${CALLING}_polarized_filtered4_${TYPE}.lr_ann.vcf | awk -F"${a}=" '{print $2}' | awk -v ann="$a" -F";" '{printf ("%s\t%s\t%s\n", ann,$1,"post")}' >> ${CALLING}_testing_hard_filters_${TYPE}.lr_ann.txt
  echo "post-filter complete"
  done
echo "SOR"
grep -v '#' ../${CALLING}_polarized_filtered3_${TYPE}.lr_ann.vcf | awk -F"SOR=" '{print $2}' | awk -F"\t" '{printf ("%s\t%s\t%s\n", "SOR",$1,"pre")}' >> ${CALLING}_testing_hard_filters_${TYPE}.lr_ann.txt
echo "pre-filter complete"
grep -v '#' ../${CALLING}_polarized_filtered4_${TYPE}.lr_ann.vcf | awk -F"SOR=" '{print $2}' | awk -F"\t" '{printf ("%s\t%s\t%s\n", "SOR",$1,"post")}' >> ${CALLING}_testing_hard_filters_${TYPE}.lr_ann.txt
echo "post-filter complete"
echo "DONE"

#From outside the server:
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov) #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_nm2_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
scp dkleinman@genomics-b.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation/testing_hard_filters/${CALLING}_testing_hard_filters*.txt /Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters
...
lp_distributions <- read_tsv("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters/c_lp_sm_c_lp_do_nm2_origcov_testing_hard_filters_SNP.lr_ann.txt",col_names=c("ANN","value","filter"))
lp_distributions

ll_distributions <- read_tsv("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters/c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_testing_hard_filters_SNP.lr_ann.txt",col_names=c("ANN","value","filter"))
ll_distributions

joint_distributions <- read_tsv("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov_testing_hard_filters_SNP.lr_ann.txt",col_names=c("ANN","value","filter"))
joint_distributions
  
#Change the following variable to detect the calling and construct the plots:
calling = "c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov" #c_lp_sm_c_lp_do_nm2_origcov #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
if (calling=="c_lp_sm_c_lp_do_nm2_origcov") {
  distribution_file <- lp_distributions
} else if (calling=="c_ll_ki_c_ll_no_c_ll_po_nm3_origcov") {
  distribution_file <- ll_distributions
} else if (calling=="c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov") {
  distribution_file <- joint_distributions
}
...
echo "Filtering out low QUAL variants"
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
-T SelectVariants \
-select "QUAL >= 30" \
-R $REF \
-V ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" \
-o ./testing_hard_filters/${CALLING}"_polarized_filtered4_QUAL_"${TYPE}".lr_ann.vcf"
...
echo "Filtering out low QD variants"
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
-T SelectVariants \
-select "QD >= 2.0" \
-R $REF \
-V ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" \
-o ./testing_hard_filters/${CALLING}"_polarized_filtered4_QD_"${TYPE}".lr_ann.vcf"
...
echo "Filtering out high FS variants"
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
-T SelectVariants \
-select "FS <= 60.0" \
-R $REF \
-V ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" \
-o ./testing_hard_filters/${CALLING}"_polarized_filtered4_FS_"${TYPE}".lr_ann.vcf"
...
$BCF view -e 'INFO/MQ<40.0' ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" > ./testing_hard_filters/${CALLING}"_polarized_filtered4_MQ_"${TYPE}".lr_ann.vcf"
...
-R $REF \
-V ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" \
-o ./testing_hard_filters/${CALLING}"_polarized_filtered4_MQRankSum_"${TYPE}".lr_ann.vcf"
...
-R $REF \
-V ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" \
-o ./testing_hard_filters/${CALLING}"_polarized_filtered4_ReadPosRankSum_"${TYPE}".lr_ann.vcf"
...
CALLING=(c_ll_ki_c_ll_no_c_ll_po_nm3_origcov) #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_nm2_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
TYPE=(SNP) #write down SNP or INDEL
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation
mkdir -p testing_hard_filters
cd testing_hard_filters
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_testing_hard_filters_separate_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
FILES=$(ls ${CALLING}_polarized_filtered4_*_${TYPE}.lr_ann.vcf)
for f in ${FILES[@]}
  do
  FILTER=$(echo ${f} | cut -d'_' -f14)
  echo "working with filter:" ${FILTER}
  rm ${CALLING}_testing_hard_filters_${FILTER}_${TYPE}.lr_ann.txt
  echo "QUAL"
  grep -v '#' ../${CALLING}_polarized_filtered3_${TYPE}.lr_ann.vcf | awk -F"\t" '{printf ("%s\t%s\t%s\n", "QUAL",$6,"pre")}' >> ${CALLING}_testing_hard_filters_${FILTER}_${TYPE}.lr_ann.txt
  echo "pre-filter complete"
  grep -v '#' ${f} | awk -F"\t" '{printf ("%s\t%s\t%s\n", "QUAL",$6,"post")}' >> ${CALLING}_testing_hard_filters_${FILTER}_${TYPE}.lr_ann.txt
  echo "post-filter complete"
  ANNOTATIONS=(QD FS MQ MQRankSum ReadPosRankSum)
  for a in ${ANNOTATIONS[@]}
    do
    echo "${a}"
    grep -v '#' ../${CALLING}_polarized_filtered3_${TYPE}.lr_ann.vcf | awk -F"${a}=" '{print $2}' | awk -v ann="$a" -F";" '{printf ("%s\t%s\t%s\n", ann,$1,"pre")}' >> ${CALLING}_testing_hard_filters_${FILTER}_${TYPE}.lr_ann.txt
    echo "pre-filter complete"
    grep -v '#' ${f} | awk -F"${a}=" '{print $2}' | awk -v ann="$a" -F";" '{printf ("%s\t%s\t%s\n", ann,$1,"post")}' >> ${CALLING}_testing_hard_filters_${FILTER}_${TYPE}.lr_ann.txt
    echo "post-filter complete"
    done
  echo "SOR"
  grep -v '#' ../${CALLING}_polarized_filtered3_${TYPE}.lr_ann.vcf | awk -F"SOR=" '{print $2}' | awk -F"\t" '{printf ("%s\t%s\t%s\n", "SOR",$1,"pre")}' >> ${CALLING}_testing_hard_filters_${FILTER}_${TYPE}.lr_ann.txt
  echo "pre-filter complete"
  grep -v '#' ${f} | awk -F"SOR=" '{print $2}' | awk -F"\t" '{printf ("%s\t%s\t%s\n", "SOR",$1,"post")}' >> ${CALLING}_testing_hard_filters_${FILTER}_${TYPE}.lr_ann.txt
  echo "post-filter complete"
  echo ${FILTER} "DONE"
  done
...
#From outside the server:
CALLING=(c_ll_ki_c_ll_no_c_ll_po_nm3_origcov) #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_nm2_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
scp dkleinman@genomics-b.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation/testing_hard_filters/${CALLING}_testing_hard_filters_*_SNP.lr_ann.txt /Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters
...
#Load data from each calling
distributions <- grep(list.files("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters",full.names=T,pattern="*testing_hard_filters*"),pattern="*_SNP.lr_ann.txt",value=T)
distributions
for (file in distributions) {
  distribution_file <- read_tsv(file,col_names=c("ANN","value","statistic"))
  calling <- strsplit(file,"/|_testing_hard_filters_")[[1]][9]
  filter <- strsplit(file,"/|_")[[1]][27]
  distribution_file$statistic = factor(distribution_file$statistic,levels=c("pre","post"))
  for (ann in grep("FS|QUAL",unique(distribution_file$ANN),value=TRUE,invert=TRUE)) {
    distribution_ann <- distribution_file %>% filter(ANN==ann)
    distribution_ann_ggplot <- ggplot(data=distribution_ann, aes(x=value,colour=statistic)) +
    geom_density() +
    ggtitle(paste("filtered by",filter)) +
    xlab(ann) +
    theme_bw() +
    theme(text=element_text(size=12,face="bold"),
          rect=element_rect(size=1),
    ...
    distribution_ann_ggplot
    ggsave(paste0(calling,"_testing_hard_filters_by_",filter,"_showing_",ann,".pdf"), width=16, height=14, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters/")
  }
  for (ann in grep("FS|QUAL",unique(distribution_file$ANN),value=TRUE)) {
    distribution_ann <- distribution_file %>% filter(ANN==ann)
    distribution_ann_ggplot <- ggplot(data=distribution_ann, aes(x=value,colour=statistic)) +
