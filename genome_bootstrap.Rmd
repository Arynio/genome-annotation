...
type="varssubs" #varssubs #variants #substitutions #segregating #fixed #private_segregating
wd_path <- ("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/bootstrap/nm3/")

averages <- read_tsv(paste0(wd_path,"c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm3nm3_origcov_ann_5xspecies_average_",type,"_SNP.empirmeanweight_ll_rel.lr_ann.txt")) %>% select(.,species,contains("_A")) %>% rename_at(vars(ends_with("_A")),funs(gsub("_A","",.)))
averages$species = factor(averages$species,levels=c("ll","lp"))
print.data.frame(averages)

errors <- read_tsv(paste0(wd_path,"c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm3nm3_origcov_ann_5xspecies_average_",type,"_SNP.totalerrorweight_ll_rel.lr_ann.txt")) %>% select(.,species,contains("_A")) %>% rename_at(vars(ends_with("_A")),funs(gsub("_A","",.))) 
errors$species = factor(averages$species,levels=c("ll","lp"))
print.data.frame(errors)
