...
for g in ${GREP[@]}
  ...
  for p in ${POP_VCFS[@]}
    ...
    grep "${g}" "${p}" | awk -F"\t|;|=" '{printf ("%s_%s_%s\t%s\n", $1,$2-1,$2,$13)}' > ${p/perpop.lr_ann.vcf/perpop_${FEATURE}_${TYPE}.lr_ann.bed}
    done
  done
...
#Follows: https://www.math.wustl.edu/~sawyer/handouts/Jackknife.pdf
...
echo -e "population_pair\tfeature\tglobal_ratio\tmean_subsample_ratio\tvariance_subsample_ratio" > ${CALLING}"_vars_subs_derived_freq_ratio_variance_"${TYPE}".lr_ann.txt"
...
for FILE in ${FILES[@]}
  ...
  SUM_P1=$(cut -f6  ${FILE} | paste -sd+ | bc)
  SUM_P2=$(cut -f7  ${FILE} | paste -sd+ | bc)
  RATIO_ALL=$(if [ $SUM_P1 == 0 ] || [ $SUM_P2 == 0 ]; then echo 0.00; else echo "scale=6; $SUM_P1/$SUM_P2" | bc; fi)
  ...
  for i in $(seq 1 $BLOCK_N)
    ...
    SUM_P1=$(cut -f6  ${FILE/.bed/.temp} | paste -sd+ | bc)
    SUM_P2=$(cut -f7  ${FILE/.bed/.temp} | paste -sd+ | bc)
    RATIO_SUBS=$(if [ $SUM_P1 == 0 ] || [ $SUM_P2 == 0 ]; then echo 0.00; else echo "scale=6; $SUM_P1/$SUM_P2" | bc; fi) ADD ZERO TO THE LEFT
    echo $RATIO_SUBS >> ${FILE/.bed/.jack} #store all subsample ratios in a file
    done
  rm ${FILE/.bed/.temp}
  #Compute the subsamples average ratio:
  RATIO_SUM=$(cut -f1 ${FILE/.bed/.jack} | paste -sd+ | bc)
  RATIO_MEAN=$(echo "scale=6; $RATIO_SUM/$BLOCK_N" | bc)
  #Obtain the jackknife pseudovalues (for more info, see https://www.math.wustl.edu/~sawyer/handouts/Jackknife.pdf)
  awk -v mean_ratio=$RATIO_MEAN -v num_blocks=$BLOCK_N -F "\t" '{printf "%s\t%s\t%s\t%s\n", $1, $num_blocks*$mean_ratio, ($num_blocks-1)*$1, ($num_blocks*$mean_ratio)-($num_blocks-1)*$1}' ${FILE/.bed/.jack} > ${FILE/.bed/.var} LEAVE ONLY CURRENT COLS 1 AND 4
  PSEUDOVALUES_SUM=$(cut -f2 ${FILE/.bed/.var} | paste -sd+ | bc)
  #Compute variance of the pseudovalues:
  VARIANCE=$(echo "scale=6; $PSEUDOVALUES_SUM/($BLOCK_N-1)" | bc)
  echo -e "$POP_PAIR\t$FEATURE\t$RATIO_ALL\t$RATIO_MEAN\t$VARIANCE" >> 
${CALLING}"_vars_subs_derived_freq_ratio_variance_"${TYPE}".lr_ann.txt"
    done

($num_blocks*$mean_ratio)-($num_blocks-1)*$1
...
tidy_derived_freq_ratios <- filtered_derived_freq_ratios %>% mutate(populations=paste(population_1,population_2,sep="/")) %>% select(-population_1,-population_2) %>% gather(ratio_type,ratio_value,-populations)
...
derived_freq_ratios_ggplot <- ggplot(data=filter(tidy_derived_freq_ratios,ratio_type=="NSYN_norm_ratio"|ratio_type=="LOF_norm_ratio"),aes(x=populations,y=ratio_value_from1,fill=ratio_type)) +
  ...
  scale_fill_manual("legend",values=c("LOF_norm_ratio"="red","NSYN_norm_ratio"="orange"),guide=guide_legend(reverse = TRUE)) +
  coord_flip() +
  xlab("Populations") +
  ylab(expression(R["X/Y"])) +
  theme_bw() 
derived_freq_ratios_ggplot
ggsave("c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov_ALL_vars_subs_derived_freq_ratio.pdf", width=20, height=15, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_freq_ratio")
...
##2A: Get variant and allele counts.
...
####Visualise counts and count ratios.
...
####Visualise counts and count ratios for the calling that includes h_ll_pv.
...
##2B: Get homozygous variant counts.
...
homoz_prop_ggplot
ggsave("individual_homozygous_derived_proportion_boxplot_dp4.pdf", width=15, height=15, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous")
