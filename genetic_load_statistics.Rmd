...
#4: Plot variant counts comparisons:
```{r}
...
for (file in calling_counts_files) {
  calling_counts <- read_tsv(paste0("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/",file),col_names=T) %>% select(., -species, -dataset) %>% gather(feature,N,-population,factor_key=T)
