...
###Global ratios (variants + substitutions) for the joint calling:
```{r Calculate derived allele frequency ratio, eval=FALSE, engine='bash'}
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_derived_freq_ratio_varssubs_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
GREP=$(cat /GRUPOS/grupolince/lynx_genomes_5x/genetic_load_analysis/derived_allele_freq_ratio/features_derived_allele_freq_ratio.txt | cut -f1)
cd $V_PATH/$CALLING/annotation
POP_VCFS=($(ls `find . -name *'_perpop_varssubs_'$TYPE'.lr_ann.vcf' -print`))
echo "retrieving variants from each feature from each population"
for g in ${GREP[@]}
  do
  echo "feature: ${g}"
  ...
  for p in ${POP_VCFS[@]}
    do
    echo "working with vcf ${p}"
    grep "${g}" "${p}" | awk -F"\t|;|=" '{printf ("%s_%s_%s\t%s\n", $1,$2-1,$2,$13)}' > ${p/perpop_varssubs_${TYPE}.lr_ann.vcf/perpop_varssubs_${FEATURE}_${TYPE}.lr_ann.bed}
    done
  done
#Prepare file that will contain population and SYN ratio data:
mkdir -p derived_allele_freq_ratio
rm derived_allele_freq_ratio/${CALLING}_SYN_derived_freq_ratio_varssubs_${TYPE}.txt
echo -e "population_1\tpopulation_2\tSYN_ratio" > derived_allele_freq_ratio/${CALLING}_SYN_derived_freq_ratio_varssubs_${TYPE}.txt
DEL_FEATURES=$(cat /GRUPOS/grupolince/lynx_genomes_5x/genetic_load_analysis/derived_allele_freq_ratio/features_derived_allele_freq_ratio.txt | grep -Ev 'synonymous_variant|intron_variant' | cut -f2)
#Prepare files that will contain DEL categories ratio data:
for f in ${DEL_FEATURES[@]}
  do
  rm derived_allele_freq_ratio/${CALLING}_${f}_derived_freq_ratio_varssubs_${TYPE}.txt
  echo -e "${f}_ratio\t${f}_norm_ratio" > derived_allele_freq_ratio/${CALLING}_${f}_derived_freq_ratio_varssubs_${TYPE}.txt
  done
POPS=($(ls `find . -name *'_perpop_varssubs_'$TYPE'.lr_ann.vcf' -print` | cut -d'/' -f3 | cut -d'_' -f1,2,3,4,5,6))
echo "calculating ratio for each pair of populations"
for p1 in ${POPS[@]}
  do
  for p2 in ${POPS[@]}
    ...
    echo "${p1} vs ${p2}"
    echo "feature: SYN"
    LANG=en_EN join -a1 -a2 -e 0.00 -o auto -j 1 <(LANG=en_EN sort -k1 ./*/${p1}_${NM_COV}_perpop_varssubs_SYN_${TYPE}.lr_ann.bed) <(LANG=en_EN sort -k1 ./*/${p2}_${NM_COV}_perpop_varssubs_SYN_${TYPE}.lr_ann.bed) | awk -F"\t|_" '{print $1,$2,$3,$4,$5}' OFS="\t" | LANG=en_EN sort -k1,1 -k2,2n | awk '{printf "%s\t%s\t%s\t%.3f\t%.3f\t%.3f\t%.3f\n", $1,$2,$3,$4,$5,$4*(1-$5),$5*(1-$4)}' > derived_allele_freq_ratio/${p1}_vs_${p2}_${NM_COV}_perpop_varssubs_SYN_${TYPE}.lr_ann.bed #col1: scaffold, col2: pos_start, col3: pos_end, col4: p1_af, col5: p2_af, col6: p1_af*(1-p2_af), col7: p2_af*(1-p1_af)
    SUM_P1=$(cut -f6 derived_allele_freq_ratio/${p1}_vs_${p2}_${NM_COV}_perpop_varssubs_SYN_${TYPE}.lr_ann.bed | paste -sd+ | bc)
    SUM_P2=$(cut -f7 derived_allele_freq_ratio/${p1}_vs_${p2}_${NM_COV}_perpop_varssubs_SYN_${TYPE}.lr_ann.bed | paste -sd+ | bc)
    RATIO=$(echo "scale=6; $SUM_P1/$SUM_P2" | bc)
    echo -e "$p1\t$p2\t$RATIO" >> derived_allele_freq_ratio/${CALLING}_SYN_derived_freq_ratio_varssubs_${TYPE}.txt
    for f in ${DEL_FEATURES[@]}
      do
      echo "feature: ${f}"
      LANG=en_EN join -a1 -a2 -e 0.00 -o auto -j 1 <(LANG=en_EN sort -k1 ./*/${p1}_${NM_COV}_perpop_varssubs_${f}_${TYPE}.lr_ann.bed) <(LANG=en_EN sort -k1 ./*/${p2}_${NM_COV}_perpop_varssubs_${f}_${TYPE}.lr_ann.bed) | awk -F"\t|_" '{print $1,$2,$3,$4,$5}' OFS="\t" | LANG=en_EN sort -k1,1 -k2,2n | awk '{printf "%s\t%s\t%s\t%.3f\t%.3f\t%.3f\t%.3f\n", $1,$2,$3,$4,$5,$4*(1-$5),$5*(1-$4)}' > derived_allele_freq_ratio/${p1}_vs_${p2}_${NM_COV}_perpop_varssubs_${f}_${TYPE}.lr_ann.bed
      SUM_P1=$(cut -f6 derived_allele_freq_ratio/${p1}_vs_${p2}_${NM_COV}_perpop_varssubs_${f}_${TYPE}.lr_ann.bed | paste -sd+ | bc)
      SUM_P2=$(cut -f7 derived_allele_freq_ratio/${p1}_vs_${p2}_${NM_COV}_perpop_varssubs_${f}_${TYPE}.lr_ann.bed | paste -sd+ | bc)
      RATIO=$(echo "scale=6; $SUM_P1/$SUM_P2" | bc)
      echo -e "$RATIO" >> derived_allele_freq_ratio/${CALLING}_${f}_derived_freq_ratio_varssubs_${TYPE}.txt
      done
    done
  done
cd $V_PATH/$CALLING/annotation/derived_allele_freq_ratio
#Make an ALL features file storing the population and SYN data
cat ${CALLING}_SYN_derived_freq_ratio_varssubs_${TYPE}.txt > ${CALLING}_ALL_derived_freq_ratio_varssubs_${TYPE}.txt
for f in ${DEL_FEATURES[@]}
  do
  #First build a file with the feature's ratio and normalised ratio (i.e. divided by the SYN ratio):
  head ${CALLING}_${f}_derived_freq_ratio_varssubs_${TYPE}.txt -n1 > ${CALLING}_temp_derived_freq_ratio_varssubs_${TYPE}.borrar
  paste ${CALLING}_SYN_derived_freq_ratio_varssubs_${TYPE}.txt ${CALLING}_${f}_derived_freq_ratio_varssubs_${TYPE}.txt | awk 'FNR == 1 {next}{print $4,$4/$3}' OFS="\t" >> ${CALLING}_temp_derived_freq_ratio_varssubs_${TYPE}.borrar
  #Now join the ALL and the current DEL feature data
  paste <(head ${CALLING}_ALL_derived_freq_ratio_varssubs_${TYPE}.txt -n1) <(head ${CALLING}_temp_derived_freq_ratio_varssubs_${TYPE}.borrar -n1) > "${CALLING}"_joint_derived_freq_ratio_varssubs_${TYPE}.borrar
  paste ${CALLING}_ALL_derived_freq_ratio_varssubs_${TYPE}.txt ${CALLING}_temp_derived_freq_ratio_varssubs_${TYPE}.borrar | awk 'FNR == 1 {next}{print $0}' OFS="\t" >> ${CALLING}_joint_derived_freq_ratio_varssubs_${TYPE}.borrar
  #Replace the ALL table with the updated one
  mv ${CALLING}_joint_derived_freq_ratio_varssubs_${TYPE}.borrar ${CALLING}_ALL_derived_freq_ratio_varssubs_${TYPE}.txt
  rm ${CALLING}_temp_derived_freq_ratio_varssubs_${TYPE}.borrar
  done

#From outside the server:
scp dkleinman@genomics-b.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/*/annotation/derived_allele_freq_ratio/*ALL_derived_freq_ratio_varssubs*.txt /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_freq_ratio/
...
#From outside the server:
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov)
scp dkleinman@genomics-a.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation/*_ann_individual_summary_*_SNP.lr_ann.txt /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_ann_individual_summary_IBD_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
cd $V_PATH/$CALLING/annotation
rm -f ${CALLING}"_ann_individual_summary_IBD_"${TYPE}".lr_ann.txt"
echo -e "species\tpopulation\tdataset\tsample\tIBD_type\ttotal_V\ttotal_A\tintergenic_V\tintergenic_A\tintronic_V\tintronic_A\tcoding_V\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tUCNE_V\tUCNE_A\tmissense/synonymous_V\tmissense/synonymous_A\tsynonymous/intronic_V\tmissense/intronic_V" > ${CALLING}"_ann_individual_summary_IBD_"${TYPE}".lr_ann.txt"
INDLIST=($(ls `find . -name *"_individual_"*"IBD"*"_"${TYPE}".lr_ann.vcf" -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}' | cut -c1-12)
  ...
  SYNONYMOUS_INTRONIC_V=$(echo "scale=4; $SYNONYMOUS_V/$INTRONIC_V" | bc)
  MISSENSE_INTRONIC_V=$(echo "scale=4; $MISSENSE_V/$INTRONIC_V" | bc)
  echo -e "$SPECIES\t$POPULATION\t$DATASET\t$SAMPLE\t$IBD\t$TOTAL_V\t$TOTAL_A\t$INTERGENIC_V\t$INTERGENIC_A\t$INTRONIC_V\t$INTRONIC_A\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$UCNE_V\t$UCNE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A\t$SYNONYMOUS_INTRONIC_V\t$MISSENSE_INTRONIC_V" >> ${CALLING}"_ann_individual_summary_IBD_"${TYPE}".lr_ann.txt"
  done

#From outside the server:
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov) #c_lp_sm_c_lp_do_nm2_origcov #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
scp dkleinman@genomics-a.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation/*_ann_individual_summary_IBD_SNP.lr_ann.txt /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/IBD_and_notIBD/
...
variants_IBD <- rbind(read_tsv(paste0(wd_path,grep(grep(variant_files,pattern='c_lp',value=T),pattern='_IBD_',value=T))),read_tsv(paste0(wd_path,grep(grep(variant_files,pattern='c_ll',value=T),pattern='_IBD_',value=T))))
variants_IBD[is.na(variants_IBD)] <- 0

subst_IBD <- read_tsv(paste0(wd_path,grep(substitution_files,pattern='_IBD_',value=T)))
subst_IBD[is.na(subst_IBD)] <- 0

IBD_summary <- cbind(variants_IBD[c(1:5)],variants_IBD[-c(1:5,21:24)]+subst_IBD[-c(1:5,21:24)])

LOF_SYN_ratio <- IBD_summary %>% mutate(LOF_SYN_ratio=nonsense_V/synonymous_V) %>% select(c(1:5,21))
...
for (pop in unique(LOF_SYN_ratio$population)) {
  ...
  ks_test <- ks.test(unlist(filter(pop_df,IBD_type=="IBD") %>% select(LOF_SYN_ratio),use.names=F), unlist(filter(pop_df,IBD_type=="notIBD") %>% select(LOF_SYN_ratio),use.names=F), alternative="g")
...
ggplot_LOF_SYN_ratio_IBD <- ggplot(data=filter(LOF_SYN_ratio,IBD_type=="IBD" | IBD_type=="notIBD"),aes(x=population,y=LOF_SYN_ratio,colour=IBD_type)) +
  ...
  xlab("Populations") +
  ylab("LoF/synonymous variant ratio") +
...
ggsave("LOF_SYN_ratio_IBD_and_notIBD.pdf", width=15, height=10, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/IBD_and_notIBD/")
...
ggsave("LOF_SYN_ratio_IBD_and_notIBD_detailed.pdf", width=15, height=10, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/IBD_and_notIBD/")

LOF_INTR_ratio <- IBD_summary %>% mutate(LOF_INTR_ratio=nonsense_V/intronic_V) %>% select(c(1:5,21))
...
ggplot_LOF_INTR_ratio_IBD <- ggplot(data=filter(LOF_INTR_ratio,IBD_type=="IBD" | IBD_type=="notIBD"),aes(x=population,y=LOF_INTR_ratio,colour=IBD_type)) +
  ...
  #ggtitle("Proportion of derived homozygous sites") +
  xlab("Populations") +
  ylab("LoF/intronic variant ratio") +
  #scale_y_continuous(limits = c(0.005, 0.030), breaks=seq(0.005,0.03,by=0.005)) +
  theme_bw() +
  theme(text=element_text(size=12,face="bold"),
      rect=element_rect(size=1),
...
ggsave("LOF_INTR_ratio_IBD_and_notIBD.pdf", width=15, height=10, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/IBD_and_notIBD/")
...
ggsave("LOF_INTR_ratio_IBD_and_notIBD_detailed.pdf", width=15, height=10, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/IBD_and_notIBD/")
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_ann_individual_homozygous_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
NM_COV=$(echo "${CALLING}" | rev | cut -d'_' -f1,2 | rev)
...
cd $V_PATH/$CALLING/annotation
rm ${CALLING}"_ann_individual_homozygous_"${TYPE}".lr_ann.txt"
echo -e "species\tpopulation\tdataset\tsample\ttotal_V\ttotal_H\ttotal_HP\tintergenic_V\tintergenic_H\tintergenic_HP\tintronic_V\tintronic_H\tintronic_HP\tsynonymous_V\tsynonymous_H\tsynonymous_HP\tmissense_V\tmissense_H\tmissense_HP\tnonsense_V\tnonsense_H\tnonsense_HP\tUCNE_V\tUCNE_H\tUCNE_HP" > ${CALLING}"_ann_individual_homozygous_"${TYPE}".lr_ann.txt"
POPLIST=($(ls `find . -name *"_perpop_"${TYPE}".lr_ann.vcf" -print`))
for p in "${POPLIST[0]}"
  do
  echo "extracting genotypes from vcf" ${p}
...
for i in "${INDLIST[@]}"
  ...
  DATASET=$(if [ $ind = "c_lp_sm_0221" ]; then echo "REF"; elif [ $ind = "c_ll_ki_0090" ]; then echo "MG"; elif [ $ind = "h_ll_pv_0223" ]; then echo "LD"; elif grep -Fxq $ind /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/c_lp_5x_samples || [ $SPECIES = "ll" ]; then echo "5x"; else echo "GP"; fi)
  SAMPLE=$(echo "${ind}" | cut -c9-12)
  GROUP_DATASET=$(grep -F $ind /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered/*_samples | cut -d':' -f1 | rev | cut -d'/' -f1 | rev | cut -d'_' -f-3)
  FILE=$(ls `find . -name ${GROUP_DATASET}"_c_"${SPECIES}"_"${POPULATION}"_"${NM_COV}"_perpop_"${TYPE}".lr_ann.table" -print`) #get current individual's population file
  GENO_COLUMN=$(head -n1 $FILE | tr "\t" "\n" | grep -n $ind":GT" | cut -d':' -f1) #get index for the column storing the current individual's genotypes
  DEPTH_COLUMN=$(head -n1 $FILE | tr "\t" "\n" | grep -n $ind":DP" | cut -d':' -f1) #get index for the column storing the current individual's genotypes
  #TOTAL_V=$(grep -v '#' $FILE | cut -f$GENO_COLUMN | grep -v "\./\." | wc -l) #escape the dots to grep the pattern './.' (to exclude missing genotypes from the count). This is the old way that doesn't take depth into account.
  #TOTAL_H=$(grep -v '#' $FILE | cut -f$GENO_COLUMN | grep "1/1" | wc -l) #This is the old way that doesn't take depth into account.
  TOTAL_V=$(grep -v '#' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if($dp_col >= 4) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  TOTAL_H=$(grep -v '#' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if(($dp_col >= 4) && ($geno_col == "1/1")) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  TOTAL_HP=$(echo "scale=4; $TOTAL_H/($TOTAL_V+$TOTAL_H)" | bc)
  INTERGENIC_V=$(grep 'intergenic' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if($dp_col >= 4) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  INTERGENIC_H=$(grep 'intergenic' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if(($dp_col >= 4) && ($geno_col == "1/1")) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  INTERGENIC_HP=$(echo "scale=4; $INTERGENIC_H/($INTERGENIC_V+$INTERGENIC_H)" | bc)
  INTRONIC_V=$(grep 'intron_variant' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if($dp_col >= 4) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  INTRONIC_H=$(grep 'intron_variant' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if(($dp_col >= 4) && ($geno_col == "1/1")) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  INTRONIC_HP=$(echo "scale=4; $INTRONIC_H/($INTRONIC_V+$INTRONIC_H)" | bc)
  SYNONYMOUS_V=$(grep 'synonymous_variant' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if($dp_col >= 4) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  SYNONYMOUS_H=$(grep 'synonymous_variant' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if(($dp_col >= 4) && ($geno_col == "1/1")) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  SYNONYMOUS_HP=$(echo "scale=4; $SYNONYMOUS_H/($SYNONYMOUS_V+$SYNONYMOUS_H)" | bc)
  MISSENSE_V=$(grep 'missense_variant' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if($dp_col >= 4) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  MISSENSE_H=$(grep 'missense_variant' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if(($dp_col >= 4) && ($geno_col == "1/1")) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  MISSENSE_HP=$(echo "scale=4; $MISSENSE_H/($MISSENSE_V+$MISSENSE_H)" | bc)
  NONSENSE_V=$(grep '|HIGH|' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if($dp_col >= 4) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  NONSENSE_H=$(grep '|HIGH|' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if(($dp_col >= 4) && ($geno_col == "1/1")) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  NONSENSE_HP=$(echo "scale=4; $NONSENSE_H/($NONSENSE_V+$NONSENSE_H)" | bc)
  UCNE_V=$(grep 'UCNE' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if($dp_col >= 4) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
  UCNE_H=$(grep 'UCNE' $FILE | awk -v geno_col=$GENO_COLUMN -v dp_col=$DEPTH_COLUMN -F "\t" \
  '{ if(($dp_col >= 4) && ($geno_col == "1/1")) {printf "%s\t%s\n", $geno_col, $dp_col} }' | wc -l)
...
#From outside the server:
CALLING=(c_lp_sm_c_lp_do_nm2_origcov) #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_nm2_origcov
scp dkleinman@genomics-b.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation/*_ann_individual_homozygous*.lr_ann.txt /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous/
...
calling_counts_files <- grep(list.files("/Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous/", pattern="*individual_homozygous_SNP_dp4.lr_ann.txt"),pattern='nm2nm3',inv=T,value=T)
homozygous_proportion <- data_frame()
for (file in calling_counts_files) {
  calling_counts <- read_tsv(paste0("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous/",file),col_names=T) %>% select(.,population,sample,contains("_HP")) %>% rename_at(vars(ends_with("_HP")),funs(gsub("_HP","",.))) %>% gather(feature,homoz_prop,-population,-sample,factor_key=T)
...
homoz_prop_ggplot <- ggplot(data=as.data.frame(filter(homozygous_proportion,population!="ki"|sample!="0090")),aes(x=feature,y=homoz_prop,colour=population)) +
  geom_boxplot() +
  #geom_point(position=position_dodge(width=0.5)) +
  #geom_line() +
  #ggtitle("Proportion of derived homozygous variants (DP ≥ 4)") +
  xlab("Features") +
  ylab("Proportion of derived homozygous\nvariants (DP > 3)") +
  theme_bw() +
  theme(text=element_text(size=12,face="bold"),
      rect=element_rect(size=1),
      #axis.line=element_line(colour="black"),
...
homoz_prop_ggplot
ggsave("individual_homozygous_derived_proportion_boxplot_dp4.pdf", width=20, height=15, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous")
...
calling_counts_files <- grep(list.files("/Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous/", pattern="*individual_homozygous_SNP.lr_ann.txt"),pattern='nm2nm3',inv=T,value=T)
homozygous_proportion <- data_frame()
for (file in calling_counts_files) {
  calling_counts <- read_tsv(paste0("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous/",file),col_names=T) %>% select(.,population,sample,contains("_HP")) %>% rename_at(vars(ends_with("_HP")),funs(gsub("_HP","",.))) %>% gather(feature,homoz_prop,-population,-sample,factor_key=T)
  homozygous_proportion <- rbind(homozygous_proportion,calling_counts)
...
homoz_prop_ggplot
ggsave("individual_homozygous_derived_proportion_woutAA.pdf", width=15, height=15, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/derived_allele_homozygous")
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_ann_individual_summary_heterozygous_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
cd $V_PATH/$CALLING/annotation
rm ${CALLING}"_ann_individual_summary_heterozygous_"${TYPE}".lr_ann.txt"
echo -e "species\tpopulation\tdataset\tsample\ttotal_H\tintergenic_H\tintronic_H\tcoding_H\tsynonymous_H\tmissense_H\tnonsense_H\tUCNE_H" > ${CALLING}"_ann_individual_summary_heterozygous_"${TYPE}".lr_ann.txt"
INDLIST=($(ls `find . -name *"_individual_"${TYPE}".lr_ann.vcf" -print` | grep -v 'testing_variant_numbers'))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}' | cut -c1-12)
  ...
  DATASET=$(if [ $ind = "c_lp_sm_0221" ]; then echo "REF"; elif [ $ind = "c_ll_ki_0090" ]; then echo "MG"; elif [ $ind = "h_ll_pv_0223" ]; then echo "LD"; elif grep -Fxq $ind /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/c_lp_5x_samples || [ $SPECIES = "ll" ]; then echo "5x"; else echo "GP"; fi)
  SAMPLE=$(echo "${ind}" | cut -c9-12)
  TOTAL_H=$(grep -v '#' ${i} | grep ';AF=0.5' | wc -l)
  INTERGENIC_H=$(grep 'intergenic' ${i} | grep ';AF=0.5' | wc -l)
  INTRONIC_H=$(grep 'intron_variant' ${i} | grep ';AF=0.5' | wc -l)
  CODING_H=$(grep 'CDS' ${i} | grep ';AF=0.5' | wc -l)
  SYNONYMOUS_H=$(grep 'synonymous_variant' ${i} | grep ';AF=0.5' | wc -l)
  MISSENSE_H=$(grep 'missense_variant' ${i} | grep ';AF=0.5' | wc -l)
  NONSENSE_H=$(grep '|HIGH|' ${i} | grep ';AF=0.5' | wc -l)
  UCNE_H=$(grep 'UCNE' ${i} | grep ';AF=0.5' | wc -l)
  echo -e "$SPECIES\t$POPULATION\t$DATASET\t$SAMPLE\t$TOTAL_H\t$INTERGENIC_H\t$INTRONIC_H\t$CODING_H\t$SYNONYMOUS_H\t$MISSENSE_H\t$NONSENSE_H\t$UCNE_H" >> ${CALLING}"_ann_individual_summary_heterozygous_"${TYPE}".lr_ann.txt"
  done

#From outside the server:
CALLING=(c_lp_sm_c_lp_do_nm2_origcov) #c_lp_sm_c_lp_do_nm2_origcov #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
scp dkleinman@genomics-a.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation/*_ann_individual_summary_heterozygous_SNP.lr_ann.txt /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/

```

###Calculate PI per category.
...
###Plot genetic load vs. diversity graphs.
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_ann_population_summary_heterozygosis_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
...
bedtools subtract -a ${CALLING}"_polarized_filtered5_"${TYPE}".lr_ann.vcf" -b ${CALLING}"_joined_minmaxdepth_to_remove_"${TYPE}".bed" > ${CALLING}"_polarized_filtered6_"${TYPE}".lr_ann.vcf"
N_SITES_FILE=$(echo ${CALLING}"_polarized_filtered6_"${TYPE}".lr_ann.vcf")

rm ${CALLING}"_ann_population_summary_heterozygosis_"${TYPE}".lr_ann.txt"
echo -e "species\tpopulation\tdataset\ttotal_HET_AVG\tintergenic_HET_AVG\tintronic_HET_AVG\tcoding_HET_AVG\tsynonymous_HET_AVG\tmissense_HET_AVG\tnonsense_HET_AVG\tUCNE_HET_AVG" > ${CALLING}"_ann_population_summary_heterozygosis_"${TYPE}".lr_ann.txt"
POPLIST=($(ls `find . -name *"_perpop_"${TYPE}".lr_ann.vcf" -print`))
for i in "${POPLIST[@]}"
  do
  echo "${i}"
...
CALLING=${CALLING%-*}
TYPE=${STY#*-}
script "${CALLING}_ann_population_summary_PI_${TYPE}.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
TYPE=${STY#*-}
