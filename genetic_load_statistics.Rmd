...
for p1 in ${POPS[@]}
  ...
  for p2 in ${POPS[@]}
    ...
    for f in ${DEL_FEATURES[@]}
      ...
      RATIO=$(if [ $SUM_P1 == 0 ] || [ $SUM_P2 == 0 ]; then echo 0.00; else echo "scale=3; $SUM_P1/$SUM_P2" | bc; fi)
      echo -e "$RATIO" >> "${CALLING}_${f}_vars_subs_derived_freq_ratio"_${TYPE}".txt"
      done
    done
  done
...
for f in ${DEL_FEATURES[@]}
  ...
  paste ${CALLING}_ALL_vars_subs_derived_freq_ratio.txt ${CALLING}_temp_vars_subs_derived_freq_ratio_${TYPE}.borrar | awk 'FNR == 1 {next}{print $0}' OFS="\t" >> ${CALLING}_joint_vars_subs_derived_freq_ratio_${TYPE}.borrar
  #Replace the ALL table with the updated one
