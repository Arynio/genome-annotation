...
variants_and_subst_wg_varN <- variants_and_subst_wg %>% select(c(1:5,9,12,14,16,18)) %>% !!!MODIFY COLUMNS gather(feature,N,-species,-population,-dataset,-sample,factor_key=T)
variants_and_subst_wg_varN
...
#From outside the server:
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov) #c_lp_sm_c_lp_do_nm2_origcov #c_ll_ki_c_ll_no_c_ll_po_nm3_origcov #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov
scp dkleinman@genomics-a.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation/$CALLING"_ann_individual_summary_Xchr_"*"_SNP.lr_ann.txt" /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/IBD_and_notIBD
...
INDLIST=($(ls -r `find . -name *"_individual_"${VAR}"_"${TYPE}".lr_ann.vcf" ! -path '*testing_variant_numbers*' -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}' | cut -c1-12)
...
echo -e "species\tpopulation\tdataset\tsample\ttotal_H\tintergenic_H\tintronic_H\tcoding_H\tsynonymous_H\tmissense_H\tnonsense_H\tUCNE_H" > ${CALLING}"_ann_individual_summary_heterozygous_"${VAR}"_"${TYPE}".lr_ann.txt"
INDLIST=($(ls `find . -name *"_individual_"${VAR}"_"${TYPE}".lr_ann.vcf" ! -path '*testing_variant_numbers*' -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
...
###Calculate PI per category (joint calling).
...
wd_path <- ("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/")
variant_files <- grep(list.files("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/", pattern="*origcov_ann_individual_summary_heterozygous_varssubs_SNP.lr_ann.txt"),pattern='wrong',inv=T,value=T)
...
ggplot_coding_pi
ggsave("coding_PI_joint_calling.pdf", width=10, height=20, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/genetic_load_vs_diversity")
...
wd_path <- ("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/")
variant_files <- grep(list.files("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/", pattern="*origcov_ann_individual_summary_variants_SNP.lr_ann.txt"),pattern='wrong',inv=T,value=T)
...
ggsave("NS_S_ratio_vs_diversity_joint_calling.pdf", width=15, height=10, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/genetic_load_vs_diversity")
...
ggsave("LOF_S_ratio_vs_diversity_joint_calling.pdf", width=15, height=10, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/genetic_load_vs_diversity")
...
ALL_SITES_FILE=$(echo /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov/annotation/${CALLING}"_category_sites_total_counts_"${TYPE}".lr_ann.txt")
...
for i in "${POPLIST[@]}"
  ...
  POPULATION=$(echo "${vcf}" | cut -c14-15)
  DATASET=$(echo "${vcf}" | cut -c6-7)
  TOTAL_N_SITES=$(grep -E "$SPECIES.*all" $ALL_SITES_FILE | cut -f4)
  TOTAL_HET_SUM=$(grep -v '#' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
  TOTAL_PI_AVG=$(echo "scale=8; $TOTAL_HET_SUM/$TOTAL_N_SITES" | bc)
  INTERGENIC_N_SITES=$(grep -E "$SPECIES.*intergenic" $ALL_SITES_FILE | cut -f4)
  INTERGENIC_HET_SUM=$(grep 'intergenic' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
  INTERGENIC_PI_AVG=$(echo "scale=8; $INTERGENIC_HET_SUM/$INTERGENIC_N_SITES" | bc)
  INTRONIC_N_SITES=$(grep -E "$SPECIES.*intron" $ALL_SITES_FILE | cut -f4)
  INTRONIC_HET_SUM=$(grep 'intron_variant' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
  INTRONIC_PI_AVG=$(echo "scale=8; $INTRONIC_HET_SUM/$INTRONIC_N_SITES" | bc)
  CODING_N_SITES=$(grep -E "$SPECIES.*CDS" $ALL_SITES_FILE | cut -f4)
  CODING_HET_SUM=$(grep 'CDS' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
  CODING_PI_AVG=$(echo "scale=8; $CODING_HET_SUM/$CODING_N_SITES" | bc)
  SYNONYMOUS_N_SITES=$(grep -E "$SPECIES.*CDS" $ALL_SITES_FILE | cut -f4)
  SYNONYMOUS_HET_SUM=$(grep 'synonymous_variant' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
  SYNONYMOUS_PI_AVG=$(echo "scale=8; $SYNONYMOUS_HET_SUM/($SYNONYMOUS_N_SITES*1/4)" | bc)
  MISSENSE_N_SITES=$(grep -E "$SPECIES.*CDS" $ALL_SITES_FILE | cut -f4)
  MISSENSE_HET_SUM=$(grep 'missense_variant' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
  MISSENSE_PI_AVG=$(echo "scale=8; $MISSENSE_HET_SUM/($MISSENSE_N_SITES*3/4)" | bc)
  NONSENSE_N_SITES=$(grep -E "$SPECIES.*CDS" $ALL_SITES_FILE | cut -f4)
  NONSENSE_HET_SUM=$(grep '|HIGH|' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
  NONSENSE_PI_AVG=$(echo "NA")
  UCNE_N_SITES=$(grep -E "$SPECIES.*UCNE" $ALL_SITES_FILE | cut -f4)
  UCNE_HET_SUM=$(grep 'UCNE' ${i} | awk -F"\t|;|=" '{printf ("%f\n", 2*$13*(1-$13))}' | paste -sd+ | bc)
