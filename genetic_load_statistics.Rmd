...
type_range <- data.frame("var_type"=c("varssubs","varssubs","fixed","fixed"),"plot"=c("up","down","up","down"),"min"=c(0.9,0.7,0.9,0.5),"max"=c(1.05,1.2,1.4,3.5),"breaks"=c(0.05,0.1,0.2,0.5))

average_relativised_derived_allele_allele_ratio_up <- ggplot(data=filter(average_relativised_variants_and_subst_wg_alleleR,grepl('total|intergenic|introns|synonymous|synonymous_pref|synonymous_unpref',ratio)), aes(population,avg_ki_relative_value,colour=population)) +
  #facet_wrap(feature ~ species,nrow=6,ncol=2,scales="free") +
  facet_grid(. ~ ratio) +
  geom_point() +
  ...
  scale_y_continuous(labels=twodecimalsFUN, breaks=seq(filter(type_range,var_type==type & plot=="up") %>% select(min) %>% unlist(.,use.names=F), filter(type_range,var_type==type & plot=="up") %>% select(max) %>% unlist(.,use.names=F), by = filter(type_range,var_type==type & plot=="up") %>% select(breaks) %>% unlist(.,use.names=F)), limits=c(filter(type_range,var_type==type & plot=="up") %>% select(min) %>% unlist(.,use.names=F),filter(type_range,var_type==type & plot=="up") %>% select(max) %>% unlist(.,use.names=F))) +
...
average_relativised_derived_allele_allele_ratio_down <- ggplot(data=filter(average_relativised_variants_and_subst_wg_alleleR,grepl('missense|missense_tol|missense_del|nonsense|UCNE|UCNElow|UCNEmid|UCNEhigh',ratio)), aes(population,avg_ki_relative_value,colour=population)) +
  #facet_wrap(feature ~ species,nrow=6,ncol=2,scales="free") +
  facet_grid(. ~ ratio) +
  geom_point() +
  geom_errorbar(aes(ymin=avg_ki_relative_value-se_ki_relative_value, ymax=avg_ki_relative_value+se_ki_relative_value), position=position_dodge(), width=0.5) +
  ...
  xlab("Population") +
  ylab("Average genetic load relative to ki") +
  scale_y_continuous(labels=twodecimalsFUN, breaks=seq(filter(type_range,var_type==type & plot=="down") %>% select(min) %>% unlist(.,use.names=F), filter(type_range,var_type==type & plot=="down") %>% select(max) %>% unlist(.,use.names=F), by = filter(type_range,var_type==type & plot=="down") %>% select(breaks) %>% unlist(.,use.names=F)), limits=c(filter(type_range,var_type==type & plot=="down") %>% select(min) %>% unlist(.,use.names=F),filter(type_range,var_type==type & plot=="down") %>% select(max) %>% unlist(.,use.names=F))) +
  #ggtitle(paste0("ratio of ",type," relative to synonymous and Kirov")) +
  theme_bw() +
  theme(text=element_text(size=12,face="bold"),
      rect=element_rect(size=1),
...
average_relativised_derived_allele_allele_ratio_combined <- grid.arrange(set_panel_size(average_relativised_derived_allele_allele_ratio_up,width=unit(4.5,"cm"),height=unit(10,"cm")), set_panel_size(average_relativised_derived_allele_allele_ratio_down,width=unit(4.5,"cm"),height=unit(10,"cm")),nrow=2)
...
average_relativised_derived_allele_allele_ratio_galtier <- ggplot(data=filter(average_relativised_variants_and_subst_wg_alleleR,grepl('introns|synonymous|synonymous_pref|synonymous_unpref|missense|missense_tol|missense_del|nonsense',ratio)), aes(population,avg_ki_relative_value,colour=population)) +
  #facet_wrap(feature ~ species,nrow=6,ncol=2,scales="free") +
  facet_grid(. ~ ratio) +
  geom_point() +
  geom_errorbar(aes(ymin=avg_ki_relative_value-se_ki_relative_value, ymax=avg_ki_relative_value+se_ki_relative_value), position=position_dodge(), width=0.5) +
  ...
  xlab("Population") +
  ylab("Average genetic load relative to ki") +
  scale_y_continuous(labels=twodecimalsFUN, breaks=seq(0.75,1.05,by=c(0.1)), limits=c(0.75,1.05)) +
  #ggtitle(paste0("ratio of ",type," relative to synonymous and Kirov")) +
  theme_bw() +
  theme(text=element_text(size=12,face="bold"),
      rect=element_rect(size=1),
...
average_relativised_derived_allele_allele_ratio_galtier

ggsave(paste0(type,"_genetic_load_relative2Kirov_coding.pdf"), width=40, height=10, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios",average_relativised_derived_allele_allele_ratio_galtier)
...
average_relativised_derived_allele_allele_ratio_ggplotcombined <- grid.arrange(average_relativised_derived_allele_allele_ratio_ggplot1, average_relativised_derived_allele_allele_ratio_ggplot2, nrow = 2, layout_matrix = rbind(c(rep(1,20)), c(rep(2,18),rep(NA,3))))
ggsave(paste0(type,"_derived_allele_allele_ratio_centr_relative2introns_all.pdf"), width=40, height=20, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/centromere/", average_relativised_derived_allele_allele_ratio_ggplotcombined)
...
average_relativised_derived_allele_allele_ratio_ggplotcombined <- grid.arrange(average_relativised_derived_allele_allele_ratio_ggplot1, average_relativised_derived_allele_allele_ratio_ggplot2, nrow = 2, layout_matrix = rbind(c(rep(1,20)), c(rep(2,18),rep(NA,3))))
ggsave(paste0(type,"_derived_allele_allele_ratio_tel_relative2introns_all.pdf"), width=40, height=20, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/telomere/", average_relativised_derived_allele_allele_ratio_ggplotcombined)
...
average_ggplot_all_pi_upright <- ggplot(data=filter(average_variants_het_joined_all,grepl('intron|synonymous|UCNE_low',feature)), aes(population,avg_het_by_total,colour=population)) +
  #facet_wrap(feature ~ species,nrow=6,ncol=2,scales="free") +
  facet_grid(. ~ feature) +
  geom_point() +
  geom_errorbar(aes(ymin=avg_het_by_total-se_het_by_total, ymax=avg_het_by_total+se_het_by_total), position=position_dodge(), width=0.5) +
...
average_ggplot_all_pi_downleft <- ggplot(data=filter(average_variants_het_joined_all,grepl('missense|UCNE_mid',feature)), aes(population,avg_het_by_total,colour=population)) +
  #facet_wrap(feature ~ species,nrow=6,ncol=2,scales="free") +
  facet_grid(. ~ feature) +
  geom_point() +
  geom_errorbar(aes(ymin=avg_het_by_total-se_het_by_total, ymax=avg_het_by_total+se_het_by_total), position=position_dodge(), width=0.5) +
...
average_ggplot_all_pi_downright <- ggplot(data=filter(average_variants_het_joined_all,grepl('nonsense|\\<UCNE\\>|UCNE_high',feature)), aes(population,avg_het_by_total,colour=population)) +
  #facet_wrap(feature ~ species,nrow=6,ncol=2,scales="free") +
  facet_grid(. ~ feature) +
  geom_point() +
  geom_errorbar(aes(ymin=avg_het_by_total-se_het_by_total, ymax=avg_het_by_total+se_het_by_total), position=position_dodge(), width=0.5) +
...
#average_ggplot_all_pi_combined <- ggarrange(average_ggplot_all_pi_upleft,average_ggplot_all_pi_upright,average_ggplot_all_pi_downleft,average_ggplot_all_pi_downright,widths=c(1.35,2),nrow=2,ncol=2)

average_ggplot_all_pi_combined <- grid.arrange(grobs=lapply(list(average_ggplot_all_pi_upleft,average_ggplot_all_pi_upright,average_ggplot_all_pi_downleft,average_ggplot_all_pi_downright), set_panel_size,width=unit(4.5,"cm"),height=unit(10,"cm")),nrow=2)

ggsave(paste0(type,"_",plot_variable,"_5xonly_mean_definitive.pdf"), width=29, height=26, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/genetic_load_vs_diversity",average_ggplot_all_pi_combined)
