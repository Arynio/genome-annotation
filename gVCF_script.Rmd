...
for j in ${POP[@]}
  ...
  -T SelectVariants \
  -R $REF \
  -V $V_PATH/c_ll_species_SNPs_strict_filter.vcf \
...
#And with this subsequent step, fixed positions within each population will be dropped and only the variables will remain.
cd $V_PATH
declare POP=$(ls *perpop_strict_filter.vcf | cut -c1-7 | uniq)
for i in ${POP[@]}
  do
  echo "${i}"
  $BCF view --min-ac 1:minor -Ov -o $V_PATH/trimmed_VCFs/"${i}"_perpop_strict_filter.trimmed.vcf $V_PATH/"${i}"_perpop_strict_filter.vcf
  done
...
#And with this subsequent step, fixed positions within each population will be dropped and only the variables will remain.
cd $V_PATH
declare POP=$(ls *perpop_no_filter.vcf | cut -c1-7 | uniq)
for i in ${POP[@]}
  do
  echo "${i}"
  $BCF view --min-ac 1:minor -Ov -o $V_PATH/trimmed_VCFs/"${i}"_perpop_no_filter.trimmed.vcf $V_PATH/"${i}"_perpop_no_filter.vcf
  done
...
cd $V_PATH
shopt -s extglob #the extglob shell option gives you more powerful pattern matching in the command line.
POPLIST=($(ls c_*_perpop_no_filter.vcf | cut -d "_" -f1,2,3 | sort | uniq))
rm c_VCF_no_filter_individual.bcf.stats
for pop in "${POPLIST[@]}"
  do
  echo "${pop}"
  ...
  $BCF stats -v -S individuals_list_to_remove.txt ${pop}_perpop_no_filter.vcf > ${pop}_perpop_no_filter.bcf.stats
  grep 'PSC' ${pop}_perpop_no_filter.bcf.stats | grep -v '#' >> c_VCF_no_filter_individual.bcf.stats
  done
rm individuals_list_to_remove.txt
HEADER=$(grep '# PSC' ${pop}_perpop_no_filter.bcf.stats | tail -1)
sed -i -e '1i'"$HEADER"'\' c_VCF_no_filter_individual.bcf.stats

#Save locally the .stats file
scp dkleinman@genomics-b.ebd.csic.es:$V_PATH/c_VCF_no_filter_individual.bcf.stats /Users/Dani/ownCloud/backup/annotation/gVCFs_analysis/
...
common_db <- rbind(std_filter_db,no_filter_db,strict_filter_db)
common_db <- common_db %>% 
  mutate(population=substr(common_db$sample,6,7))
...
joined_db <- full_join(full_join(no_filter_db[,-c(1)],std_filter_db[,-c(1)],"sample"),strict_filter_db[,-c(1)],"sample")
joined_db
joined_db <- joined_db %>% 
  mutate(n01_no_minus_std=n01_no-n01_std,n01_std_minus_strict=n01_std-n01_strict,population=substr(joined_db$sample,6,7))
joined_db

tidy_joined_db <- joined_db[,c(1,4,7,10:13)] %>%
  mutate(perc_loss_1=round(100*n01_no_minus_std/n01_no,1),perc_loss_2=round(100*n01_std_minus_strict/n01_std,1)) %>%
  gather(filter,N,-sample,-n01_no,-n01_std,-n01_strict,-population,-perc_loss_1,-perc_loss_2)
tidy_joined_db

joined_db_ggplot <- ggplot(data=tidy_joined_db, aes(x=sample,y=N)) + 
  geom_col(aes(fill=filter)) +
  geom_text(aes(label=ifelse(filter=="n01_no_minus_std",perc_loss_1,NA)),size=2,position="identity") +
  geom_text(aes(label=ifelse(filter=="n01_std_minus_strict",perc_loss_2,NA)),size=2,position="identity",vjust=1.5) +
  scale_fill_hue(labels=c("no_filter - standard_filter","standard_filter - strict_filter")) +
...
joined_db_ggplot
ggsave("VCFs_filtered_out_SNPs_per_sample.pdf", width=40, height=10, units="cm", device="pdf", path=local_repo)

tidy_joined_db2 <- joined_db[,c(1,13,11,12,10)] %>%
...
joined_db_ggplot2 <- ggplot(data=tidy_joined_db2, aes(x=sample,y=N)) + 
  ...
  scale_alpha_manual(values=c(0.5,0.75,1), labels=c("no_filter","standard_filter","strict_filter")) +
...
joined_db_ggplot2
ggsave("VCFs_SNPs_per_sample.pdf", width=30, height=10, units="cm", device="pdf", path=local_repo)
