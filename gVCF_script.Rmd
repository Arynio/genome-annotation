...
for i in ${POP[@]}
  ...
  /opt/bcftools/bcftools view -c1 -Oz -S list_to_remove.txt --min-ac=1 --no-update -O v -o "${i}"_perpop.vcf all_c_ll_lp_renamed_SNPs.vcf
...
rm c_VCF_raw.stats
echo "sample_name,total_SNPs,homoz_ref,heteroz,homoz_alt,unaccounted,unaccounted_cat" > c_VCF_raw.stats 
for pop in "${POPLIST[@]}"
do
echo "${pop}"
#done
...
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/c_ll_ya_perpop.vcf | cut -d$'\t' -f 10- | cut -d ":" -f1 | sort | uniq | paste -s -d ","

# ^ keep all things before ":" (all columns) and then count the number of 1/1 (substitutions when all individuals are 1/1?).
# ^ keep only that + the total number of SNPs. The rest doesn't make sense in a population VCF.
