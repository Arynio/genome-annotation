...
#Step 2c: Analyse the sanity checks file. Perform per population analyses on the sanity checks file.
...
#Step 3a: Combine gVCF into VCF. Run joint genotyping of all desired gVCF files to produce a multisample VCF file.
...
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -jar /opt/GATK-3.7/GenomeAnalysisTK.jar -T GenotypeGVCFs -R /home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/lp23.fa -V c_lp_do_0443_BIS_recal_round-1.g.vcf -V c_lp_sm_0325_recal_round-1.g.vcf -o toy_lp_VCF_3.vcf
...
#Step 3b: Split VCF into per population VCFs.

```{r Combine gVCF into VCF, eval=FALSE, engine='bash'}

#Rename those samples in the global VCF that don't have the proper name (the 4 samples that were sequenced at Macrogen and the DON sample from before 1990)
#!/bin/bash
