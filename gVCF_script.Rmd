...
cd /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final #this works for the 5x samples (25x samples are stored in another folder and are named differently)
...
REF=/home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/lp23.fa
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx64g -jar /opt/GATK-3.7/GenomeAnalysisTK.jar \
-T HaplotypeCaller \
-R $REF \
-I c_lp_do_0443_recal_round-1.bam \
--emitRefConfidence GVCF \
-o /home/GRUPOS/grupolince/lynx_genomes_5x/gVCFs/c_lp_do_0443_BIS_recal_round-1.g.vcf
...
-o lp_all_pop_recal_round-1.vcf
...
#2c: Split the VCF into per species VCFs. Generate a VCF for the Iberian lynx and another one for the Eurasian lynx.
...
for i in ${SPECIES[@]}
  ...
  /opt/bcftools-1.6/bcftools view -S list_to_remove.txt --min-ac 1:minor -Ov -o "${i}"_species.vcf all_c_ll_lp_renamed.vcf
  done
rm list_to_remove.txt
  
!!!!!!!! IMPORTANT TO CHECK WHETHER THE REFERENCE ALLELE IS STILL THE SAME BETWEEN BOTH VCFs !!!!!!!!!
...
for i in ${SPECIES[@]}
  ...
  -R /home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/lp23.fa \
  -V /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/"${i}"_species.vcf \
...
#SNPs that don't meet certain quality criteria should be tagged (and eventually removed). QD: QualbyDepth; FS: FisherStrand; MQ: root mean square of Mapping Quality; MQRankSum: Mann-Whitney Rank Sum Test for Mappin Qualities; ReadPosRankSum: Mann-Whitney Rank Sum Test for distance from end of the read.
cd /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/
...
!!!!!!!!!! FILTERING CRITERIA MUST BE REVISED !!!!!!!!!!
...
#Split both species VCFs in order to get a sub-VCF for each population.

!!!!!!!!!! REVISAR CODIGO. COMPROBAR QUE FUNCIONA ANTES DE USAR !!!!!!!!!!
...
for i in ${SPECIES[@]}
  ...
  for j in ${POP[@]}
    ...
    /opt/bcftools/bcftools view -c1 -S list_to_remove.txt -Ov -o "${j}"_perpop.vcf "${i}"_species_SNPs_tagged_filtered.vcf
    done
  done
rm list_to_remove.txt
