...
Step 3. Run joint genotyping of all desired gVCF files to produce a multisample VCF file. First, I generate a VCF file using all lp gVCF files. It doesn't work on compressed files, so I have to unzip all .g.vcf.gz files using the gunzip command, and then repeat.

```{r Combine gVCF into VCF, eval=FALSE, engine='bash'}

cd /home/dkleinman/datos/c_lp_bamfiles

java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms64g -Xmx128g -jar /opt/GATK-3.7/GenomeAnalysisTK.jar -T GenotypeGVCFs -R /home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/lp23.fa $(for var in *.g.vcf; do echo -V ${var}" ";done) -o lp_all_pop_recal_round-1.vcf
...
ls /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/gVCF_lp/${pop}recal_round-1.g.vcf  > ${pop/*_l*_*_/lp_and_ll}_recal_round-1.g.vcf.list
ls /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/gVCF_ll/${pop}recal_round-1.g.vcf.gz  >> ${pop/*_l*_*_/lp_and_ll}_recal_round-1.g.vcf.list;
done

#añadir algo como en el bucle anterior con INPUT_BAMS_FOR_CALLING

```

#Extract SNPs
Step 4a. Subset the VCF file in order to keep only SNP variants.
...
#Extract INDELs
Step 4b. Subset the VCF file in order to keep only INDELs.
...
#Tag SNPs
Step 5a. Tag SNPs which don't reach certain quality thresholds (QD: QualbyDepth; FS: FisherStrand; MQ: root mean square of Mapping Quality; MQRankSum: Mann-Whitney Rank Sum Test for Mappin Qualities; ReadPosRankSum: Mann-Whitney Rank Sum Test for distance from end of the read).
...
#Filter SNPs
Step 5b. Remove all SNPs tagged in the previous step.
...
#Polarize SNPs
Step 6. Later on, use VCFtools in order to polarize according to the ancestral genome of preference.
