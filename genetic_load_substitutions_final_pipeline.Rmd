...
$BCF view -e '(INFO/AA!=REF & INFO/AA!=ALT) | INFO/AF=0.00 | INFO/AF=1.00' "${CALLING}_polarized_filtered2.lr_ann.vcf" > "${CALLING}_polarized_filtered3.lr_ann.vcf"
...
screen -S "${CALLING}"
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov)
script "${CALLING}_polarized_substitutions.lr_ann.log"
...
if [[ $CALLING == "c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3"* ]] #the double brackets turns the test into a regex
  then
    ...
    grep -v '#' /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/c_lp_sm_c_lp_do_nm2_"$COVERAGE"/annotation/c_lp_sm_c_lp_do_nm2_"$COVERAGE"_polarized_filtered5.lr_ann.vcf | cut -f1,2 | awk '{printf ("%s\t%s\t%s\n", $1, $2-1, $2)}' > /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/c_lp_sm_c_lp_do_nm2_"$COVERAGE"/annotation/c_lp_sm_c_lp_do_nm2_"$COVERAGE"_polarized_filtered5.bed #grab coordinates of the true lp variants (obtained in the separate calling)
    echo "c_lp true variants retrieved"
    grep -v '#' /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/c_ll_ki_c_ll_no_c_ll_po_nm3_"$COVERAGE"/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_"$COVERAGE"_polarized_filtered5.lr_ann.vcf | cut -f1,2 | awk '{printf ("%s\t%s\t%s\n", $1, $2-1, $2)}' > /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/c_ll_ki_c_ll_no_c_ll_po_nm3_"$COVERAGE"/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_"$COVERAGE"_polarized_filtered5.bed #grab coordinates of the true ll variants (obtained in the separate calling)
    echo "c_ll true variants retrieved"
    cat /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/*"$COVERAGE"/annotation/*_"$COVERAGE"_polarized_filtered5.bed | bedtools sort | uniq > "${CALLING}"_variants_to_remove.bed #join the lp and ll true variants bed
    echo "true variants joined"
    bedtools subtract -a "${CALLING}"_polarized_filtered5.lr_ann.vcf -b "${CALLING}"_variants_to_remove.bed -header > "${CALLING}"_polarized_filtered6.lr_ann.vcf #remove true lp and ll variants from the vcf
    echo "true variants filtered out"
    grep -v '#' "${CALLING}"_polarized_filtered6.lr_ann.vcf | wc -l 
    #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov 1527213
    #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov 1554497
    for s in ${SPECIES[@]}
      do
      echo "retrieving 1 and 0 positions from ${s}"
      bcftools query -l "${CALLING}"_polarized_filtered6.lr_ann.vcf | cut -c1-12 | sort | uniq | grep "${s}" > "${s}"_samples.txt
      N_SAMPLES=$(cat "${s}"_samples.txt | wc -l)
      AF_THRES=$(echo "scale=4; ($N_SAMPLES*2-2)/($N_SAMPLES*2)" | bc)
      bcftools view -S "${s}"_samples.txt "${CALLING}"_polarized_filtered6.lr_ann.vcf | bcftools view -i "AC/AN = 0 | AC/AN > ${AF_THRES}" -Ov -o "${CALLING}"_"${s}"_1_and_0_positions.vcf #grab only those positions that have AF=0 or AF=(2*num_alleles, i.e. at most one derived allele). Use AC/AN instead of AF since the latter isn't recalculated for the subset of samples
      grep -v '#' "${CALLING}"_"${s}"_1_and_0_positions.vcf | cut -f1,2 | awk '{printf ("%s\t%s\t%s\n", $1, $2-1, $2)}' >  "${CALLING}"_"${s}"_1_and_0_positions.bed
      echo "${s} 1 and 0 positions retrieved"
      done
    bedtools intersect -a *c_lp_1_and_0_positions.bed -b *c_ll_1_and_0_positions.bed > "${CALLING}"_joined_1_and_0_positions.bed
    bedtools intersect -a "${CALLING}"_polarized_filtered6.lr_ann.vcf -b "${CALLING}"_joined_1_and_0_positions.bed -header > "${CALLING}"_polarized_substitutions.lr_ann.vcf
    echo "non 1 and 0 positions filtered out"
    grep -v '#' "${CALLING}"_polarized_substitutions.lr_ann.vcf | wc -l
    #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov 1440530
    #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov 1448249
    bedtools subtract -a "${CALLING}"_polarized_filtered6.lr_ann.vcf -b "${CALLING}"_joined_1_and_0_positions.bed -header | grep -v '#' | wc -l #sanity check that counts the number of discarded variants. This number should equal the difference between _filtered6 and _substitutions. It is for both treatments:
    #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov 86683
    #c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov 106248
  else
...
screen -S "${CALLING}"
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov)
script "perdataset_${CALLING}.lr_ann.log"
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov)
...
for j in ${DATASETS[@]}
  do
  echo "${j}"
  cat /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered/"${j}"_samples
  mkdir -p "${j}"_"${NM_COV}"_perdataset
  java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
  -T SelectVariants \
  -R $REF \
  -V "${CALLING}_polarized_substitutions.lr_ann.vcf" \
  -o "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset.lr_ann.vcf \
  -env \
  --sample_file /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered/"${j}"_samples
  grep -v '#' "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset.lr_ann.vcf | wc -l #
  done
...
for i in ${DATASETS[@]}
  do
    N_SIZE0=$(cat /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered/"${i}_samples" | wc -l)
...
screen -S "${CALLING}"
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov)
script "${CALLING}_depth_filter.lr_ann.log"
...
for j in ${DATASETS[@]}
  ...
  echo $MIN_DP
  echo $MAX_DP
  $BCF filter -i "DP < ${MIN_DP} || DP > ${MAX_DP}" -Ov -o "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_minmaxdepth_to_remove.lr_ann.vcf "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset.lr_ann.vcf
  sed -e 's/chr//' "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_minmaxdepth_to_remove.lr_ann.vcf | awk '{OFS="\t"; if (!/^#/){print $1,$2-1,$2}}' > "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_minmaxdepth_to_remove.bed #convert from VCF to BED
  done
cat *_"${NM_COV}"_perdataset/*_"${NM_COV}"_minmaxdepth_to_remove.bed | bedtools sort | uniq > "${CALLING}"_joined_minmaxdepth_to_remove.bed #join all BEDs from each species (i.e. from each calling)
for j in ${DATASETS[@]}
  do
  echo "${j}"
  bedtools subtract -a "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset.lr_ann.vcf -b "${CALLING}"_joined_minmaxdepth_to_remove.bed -header > "${j}"_"${NM_COV}"_perdataset/"${j}"_"${NM_COV}"_perdataset_filtered.lr_ann.vcf
  done
...
for i in ${DATASETS[@]}
  do
  echo "${i}"
  declare POP=$(bcftools query -l "${i}"_"${NM_COV}"_perdataset/"${i}"_"${NM_COV}"_perdataset_filtered.lr_ann.vcf | cut -c1-7 | sort | uniq)
  for j in ${POP[@]}
    do
    echo "${j}"
    rm "${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt
    $BCF query -l "${i}"_"${NM_COV}"_perdataset/"${i}"_"${NM_COV}"_perdataset_filtered.lr_ann.vcf | grep "${j}" > "${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt
    cat "${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt
    mkdir -p "${j}"_"${NM_COV}"_perpop
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V "${i}"_"${NM_COV}"_perdataset/"${i}"_"${NM_COV}"_perdataset_filtered.lr_ann.vcf \
    -o "${j}"_"${NM_COV}"_perpop/"${i}"_"${j}"_"${NM_COV}"_perpop.lr_ann.vcf \
    -env \
    --sample_file "${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt
    rm "${i}"_"${NM_COV}"_perdataset/"${i}"_"${j}"_pop_list_to_remove.txt
    grep -v '#' "${j}"_"${NM_COV}"_perpop/"${i}"_"${j}"_"${NM_COV}"_perpop.lr_ann.vcf | wc -l #
    done
  done

```

#13: Split the population VCFs into per individual VCFs. Generate a VCF for each individual.
...
NM_COV=$(echo "${CALLING}" | rev | cut -d'_' -f1,2 | rev)
cd $V_PATH/$CALLING/annotation
declare POP=$(bcftools query -l "${CALLING}_polarized_substitutions.lr_ann.vcf" | cut -c1-7 | sort | uniq)
for i in ${POP[@]}
  do
  echo "${i}"
  mkdir -p "${i}"_"${NM_COV}"_individuals
  VCF_LIST=$(ls "${i}"_"${NM_COV}"_perpop/*_"${i}"_"${NM_COV}"_perpop.lr_ann.vcf)
  for j in ${VCF_LIST[@]}
    do 
    ...
    for k in ${INDIVIDUALS[@]}
      ...
      -R $REF \
      -V "${j}" \
      -o "${i}"_"${NM_COV}"_individuals/"${k}"_"${NM_COV}"_individual.lr_ann.vcf \
      -env \
      -sn $ID
      done
    done
...
#First draw the NM distribution for each individual:
sample_files <- grep(list.files("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/", pattern="*_ann_individual_summary.lr_ann.txt"),pattern='wrong',inv=T,value=T)
snpeff_filters_summary <- data_frame()
for (file in sample_files) {
...
snpeff_filters_summary$dataset = factor(snpeff_filters_summary$dataset,levels(snpeff_filters_summary$dataset)[c(4,2,1,3)]) #Reorder factor levels to: REF, GP, 5x, MG
