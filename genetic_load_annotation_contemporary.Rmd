...
grep ';AF=1' c_ll_lp_plus_h_ll_renamed.vcf | wc -l #
...
grep -v '#' c_ll_lp_plus_h_ll_polarized.vcf | wc - #23999314
grep ";AF=1" c_ll_lp_plus_h_ll_polarized.vcf | wc -l #31548
...
screen -S c_ll_lp_plus_h_ll_polarized.lr_ann.log #open a dettachable screen in case the test takes too long
script c_ll_lp_plus_h_ll_polarized.lr_ann.log #initiate the log file
...
java -Xmx16g -jar $S_PATH/snpEff.jar LYRU.23 -v -s $V_PATH/annotation/c_ll_lp_plus_h_ll_polarized.lr_ann.html -csvStats $V_PATH/annotation/c_ll_lp_plus_h_ll_polarized.lr_ann.csv -interval $C_PATH/data/LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCNE.intergenic.nr.bed $V_PATH/c_ll_lp_plus_h_ll_polarized.vcf > $V_PATH/annotation/c_ll_lp_plus_h_ll_polarized.lr_ann.vcf #run this code from the directory where the config is located.

grep -v '#' c_ll_lp_plus_h_ll_polarized.lr_ann.vcf | wc -l #23999314
...
cd $V_PATH/annotation
screen -S c_ll_lp_plus_h_ll_polarized_filtered1.lr_ann.log
script c_ll_lp_plus_h_ll_polarized_filtered1.lr_ann.log

bedtools subtract -a c_ll_lp_plus_h_ll_polarized.lr_ann.vcf -b /home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/repeats_and_lowcomp_no_redundant_mappability.k75_lessthan90.bed -header > c_ll_lp_plus_h_ll_polarized_filtered1.lr_ann.vcf
grep -v '#' c_ll_lp_plus_h_ll_polarized_filtered1.lr_ann.vcf | wc -l #9308565
...
cd $V_PATH/annotation
screen -S c_ll_lp_plus_h_ll_polarized_filtered2.lr_ann.log
script c_ll_lp_plus_h_ll_polarized_filtered2.lr_ann.log
...
-R $REF \
-V c_ll_lp_plus_h_ll_polarized_filtered1.lr_ann.vcf \
-o c_ll_lp_plus_h_ll_polarized_filtered2.lr_ann.vcf
grep -v '#' c_ll_lp_plus_h_ll_polarized_filtered2.lr_ann.vcf | wc -l #7598146

```

##Remove unpolarizable and fixed positions. Variants where AA is different from REF/ALT alleles (including AA=N) will be dropped at this step, as will all where AF=1 (i.e. low depth, reference genome sequencing errors...) or AF=0 (same but for polarized positions).

```{r Filter the annotated VCF, eval=FALSE, engine='bash'}

#Remove fixed variants (AF=1) or those that weren't polarizable (AA different from either REF or ALT):
cd $V_PATH/annotation
screen -S c_ll_lp_plus_h_ll_polarized_filtered3.lr_ann.log
script c_ll_lp_plus_h_ll_polarized_filtered3.lr_ann.log

$BCF view -e '(INFO/AA!=REF & INFO/AA!=ALT) | INFO/AF=1.00 | INFO/AF=0.00' c_ll_lp_plus_h_ll_polarized_filtered2.lr_ann.vcf > c_ll_lp_plus_h_ll_polarized_filtered3.lr_ann.vcf
grep -v '#' c_ll_lp_plus_h_ll_polarized_filtered3.lr_ann.vcf | wc -l #7422027
...
cd $V_PATH/annotation
screen -S c_ll_lp_plus_h_ll_polarized_filtered4.lr_ann.log
script c_ll_lp_plus_h_ll_polarized_filtered4.lr_ann.log

java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
-T SelectVariants \
-select "QUAL >= 30 && QD >= 2.0 && FS <= 60.0 && MQ >= 40.0 && MQRankSum >= -12.5 && ReadPosRankSum >= -8.0" \
-R $REF \
-V c_ll_lp_plus_h_ll_polarized_filtered3.lr_ann.vcf \
-o c_ll_lp_plus_h_ll_polarized_filtered4.lr_ann.vcf
grep -v '#' c_ll_lp_plus_h_ll_polarized_filtered4.lr_ann.vcf | wc -l #5063416

```

#5: Split the VCF into per species VCFs and then test per population and per individual splits without filtering. 
##Generate a VCF for each species.

```{r Get annotation statistics for each population, eval=FALSE, engine='bash'}

#Split the annotated VCF into per species VCFs.
cd $V_PATH/annotation/
screen -S perspecies.lr_ann.log
script perspecies.lr_ann.log
...
for i in ${SPECIES[@]}
  do
  echo "${i}"
  $BCF query -l $V_PATH/annotation/c_ll_lp_plus_h_ll_polarized_filtered4.lr_ann.vcf | grep "${i}" > list_to_remove.txt
  cat list_to_remove.txt
  java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
  -T SelectVariants \
  -R $REF \
  -V $V_PATH/annotation/c_ll_lp_plus_h_ll_polarized_filtered4.lr_ann.vcf \
  -o $V_PATH/annotation/"${i}"_perspecies.lr_ann.vcf \
  --sample_file list_to_remove.txt
  $BCF view --min-ac 1 -Ov -o $V_PATH/annotation/"${i}"_perspecies.trimmed.lr_ann.vcf $V_PATH/annotation/"${i}"_perspecies.lr_ann.vcf
  done
rm list_to_remove.txt

grep -v '#' lp_perspecies.trimmed.lr_ann.vcf | wc -l #1849314
grep -v '#' ll_perspecies.trimmed.lr_ann.vcf | wc -l #4104697

```

##Test: split the unfiltered VCF into per population VCFs, subsample populations, and split the VCF into per individual VCFs. Then get annotation statistics for all populational and individual VCFs.

```{r Obtain per population VCFs, eval=FALSE, engine='bash'}

#This code is adapted from that from subsequent sections (8-10). It ends up yielding allele counts for all populations (subsampled to the minimum size -8-, and for all individuals) starting from the original per species VCFs (without any filtering), to test if the strange patterns found with the filtered ones -high number of synonymous mutations in pardinus- was already present prior to the filtering.

cd $V_PATH/annotation/
screen -S testing_allele_counts.lr_ann.log
script testing_allele_counts.lr_ann.log
...
#Split the annotated VCF into per population VCFs, and then keep only those positions that are variable or fixed for the derived allele within each population (with -env flag).
    ...
    $BCF query -l $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered1.lr_ann.vcf | grep "${j}" > pop_list_to_remove.txt
    cat pop_list_to_remove.txt
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/c_ll_lp_plus_h_ll_polarized.lr_ann.vcf \
    -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts_with1.lr_ann.vcf \
    -env \
    --sample_file pop_list_to_remove.txt
    rm pop_list_to_remove.txt

#Subsample all populations that need subsampling (original code where the files with the ids are created is in section 8):

    cd $V_PATH/annotation/"${j}"_perpop
    pwd
    POPULATION=$(echo "${j}" | cut -c6-7)
    if [ -e "${j}"_perpop_minimum_subsample_with1.lr_ann.vcf ]; then
      echo "Minimum "$POPULATION" subsample available"
      FILE="${j}"_perpop_minimum_ids.txt
      java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
      -T SelectVariants \
      -R $REF \
      -V $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts_with1.lr_ann.vcf \
      -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_minimum_subsample_test_allele_counts_with1.lr_ann.vcf \
      -env \
      --sample_file $FILE
     elif [ -e "${j}"_perpop_random_subsample_with1.lr_ann.vcf ]; then
      echo "Random "$POPULATION" subsample available"
      FILE="${j}"_perpop_random_ids.txt
      java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
      -T SelectVariants \
      -R $REF \
      -V $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts_with1.lr_ann.vcf \
      -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_random_subsample_test_allele_counts_with1.lr_ann.vcf \
      -env \
      --sample_file $FILE
      else
...
<!--     cd $G_PATH -->
<!--     declare INDIVIDUALS=$(ls "${j}"*.g.vcf.gz | cut -c1-12 | uniq) -->
...
<!--       -T SelectVariants \ -->
<!--       -R $REF \ -->
<!--       -V $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts.ann.vcf \ -->
<!--       -o $V_PATH/annotation/"${j}"_individuals/"${k}"_individual_test_allele_counts.ann.vcf \ -->
<!--       -env \ -->
<!--       -sn $ID -->
<!--       done -->
    done
...
#Get annotation statistics at the population level:  

cd $V_PATH/annotation/
rm snpeff_population_between_test_allele_counts.lr_ann.txt
echo -e "species\tpopulation\tsize\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_population_between_test_allele_counts.lr_ann.txt
...
for pop in "${POPLIST[@]}"
  ...
  if [ -e ${pop}_minimum_subsample_test_allele_counts_with1.lr_ann.vcf ]; then
    echo "Minimum "$POPULATION" subsample available"
    FILE=${pop}_minimum_subsample_test_allele_counts_with1.lr_ann.vcf
  elif [ -e ${pop}_random_subsample_test_allele_counts_with1.lr_ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample_test_allele_counts_with1.lr_ann.vcf
  else
    echo "Original "$POPULATION" available"
    FILE=${pop}_test_allele_counts_with1.lr_ann.vcf
  fi
  ...
  SYNONYMOUS_A=$(grep 'synonymous_variant' $FILE | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | cut -d',' -f1 | paste -sd+ | bc)
  MISSENSE_V=$(grep 'missense_variant' $FILE | wc -l)
  MISSENSE_A=$(grep 'missense_variant' $FILE | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | cut -d',' -f1 | paste -sd+ | bc)
  NONSENSE_V=$(grep -e 'lost' -e 'gained' $FILE | wc -l)
  NONSENSE_A=$(grep -e 'lost' -e 'gained' $FILE | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | cut -d',' -f1 | paste -sd+ | bc)
  ...
  echo -e "$SPECIES\t$POPULATION\t$SIZE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_between_test_allele_counts.lr_ann.txt
  done
  
#Get annotation statistics at the individual level:  

<!-- cd $V_PATH/annotation/ -->
<!-- rm snpeff_individual_test_allele_counts.txt -->
<!-- echo -e "species\tpopulation\tsample\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_individual_test_allele_counts.txt -->
<!-- INDLIST=($(ls `find . -name '*individual_test_allele_counts.ann.vcf' -print`)) -->
<!-- for i in "${INDLIST[@]}" -->
<!--   do -->
<!--   echo "${i}" -->
...
<!--   MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc) -->
<!--   MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc) -->
<!--   echo -e "$SPECIES\t$POPULATION\t$SAMPLE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_individual_test_allele_counts.txt -->
<!--   done -->

#All NS/S rations are a bit higher than in the final, filtered VCFs, but they are proportionally higher for all populations, i.e. those disparate ratios between ll and lp are truly in our data.
...
scp dkleinman@genomics-b.ebd.csic.es:/home/dkleinman/datos/c_lp_depth_calculus/*.depth* /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/lp_depth
...
write.table(x = depth_per_sample,file = paste("/Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/lp_depth/mean_sd_depthGlobal_lynx_per_pop_mean_folds_",mean_folds,".csv", sep= ""),quote=FALSE, col.names = FALSE, row.names = FALSE, sep= " ")
...
scp /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/lp_depth/mean_sd_depthGlobal_lynx_per_pop_mean_folds_0.95.csv dkleinman@genomics-b.ebd.csic.es:/home/dkleinman/datos/c_lp_depth_calculus/
...
##Apply low and high DP and 0.5 missingness filters. Use filtered2 instead.
...
cd $V_PATH/annotation/
screen -S perspecies_filtered1.lr_ann.log
script perspecies_filtered1.lr_ann.log

declare SPECIES=$(ls *_perspecies.trimmed.lr_ann.vcf | cut -c1-2 | uniq)
for i in ${SPECIES[@]}
  do
  echo "${i}"
  ...
  echo $MIN_DP
  echo $MAX_DP
  $BCF filter -e "DP < ${MIN_DP} || DP > ${MAX_DP} || F_MISSING > 0.5" -Ov -o $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered1.lr_ann.vcf $V_PATH/annotation/"${i}"_perspecies.trimmed.lr_ann.vcf
  done
grep -v '#' lp_perspecies.trimmed_filtered1.lr_ann.vcf | wc -l #1808148
grep -v '#' ll_perspecies.trimmed_filtered1.lr_ann.vcf | wc -l #4065488

```

##Apply low and high DP and 0.325 missingness filters. This is the chosen filter.
...
cd $V_PATH/annotation/
screen -S perspecies_filtered2.lr_ann.log
script perspecies_filtered2.lr_ann.log
...
for i in ${SPECIES[@]}
  ...
  echo $MIN_DP
  echo $MAX_DP
  $BCF filter -e "DP < ${MIN_DP} || DP > ${MAX_DP} || F_MISSING > 0.325" -Ov -o $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered2.lr_ann.vcf $V_PATH/annotation/"${i}"_perspecies.trimmed.lr_ann.vcf
  done
grep -v '#' lp_perspecies.trimmed_filtered2.lr_ann.vcf | wc -l #1760359
grep -v '#' ll_perspecies.trimmed_filtered2.lr_ann.vcf | wc -l #4058311

```

##Apply low and high DP and 0.2 missingness filters.

```{r Apply per species filters, eval=FALSE, engine='bash'}

#First, for each species exclude those positions that have more than 32.5% missing genotypes (i.e. missing at least in all individuals within the 5x dataset, or within the smaller 25x dataset), as well as those that have lower (higher) depth than the minimum (maximum) within 0.95 of the distribution, as calculated above:
cd $V_PATH/annotation/
screen -S perspecies_filtered3.lr_ann.log
script perspecies_filtered3.lr_ann.log
...
for i in ${SPECIES[@]}
  ...
  echo $N
  MIN_DP=$(cat /home/dkleinman/datos/c_lp_depth_calculus/c_"${i}"*$N*.csv | awk '{print $9}') #Obtained in section 6
  MAX_DP=$(cat /home/dkleinman/datos/c_lp_depth_calculus/c_"${i}"*$N*.csv | awk '{print $8}') #Obtained in section 6
  echo $MIN_DP
  echo $MAX_DP
  $BCF filter -e "DP < ${MIN_DP} || DP > ${MAX_DP} || F_MISSING > 0.2" -Ov -o $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered3.lr_ann.vcf $V_PATH/annotation/"${i}"_perspecies.trimmed.lr_ann.vcf
  done
grep -v '#' lp_perspecies.trimmed_filtered3.lr_ann.vcf | wc -l #1686227
grep -v '#' ll_perspecies.trimmed_filtered3.lr_ann.vcf | wc -l #4036258

```


##Compute heterozygosity ratio using a custom script. Unused in final version.
...
for i in ${SPECIES[@]}
  do
  echo "${i}"
  grep -v '^#' $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered1.lr_ann.vcf | perl -lne '@x=split /\s+/,$_;$var11=grep{/1\/1/}@x[9..$#x];$var01=(grep{/0\/1/}@x[9..$#x])+(grep{/1\/0/}@x[9..$#x]);$var00=grep{/0\/0/}@x[9..$#x];$varRAT=($var01/($var01+$var00+$var11));$varBED=$x[1]-1;print"$x[0]\t$varBED\t$x[1]\t$varRAT\n"' > "${i}"_perspecies.trimmed_filtered1.het.bed
  #| awk -F"\t" '$4>0.8'
  done
  
#Download from the server the bed files with the Het ratios, and jump to the next chunk to check the distributions in R:
scp dkleinman@genomics-b.ebd.csic.es:/home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/l*.het.bed /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/het_ratio
...
#   do
#   echo "${i}"
#   bedtools subtract -a "${i}"_perspecies.trimmed_filtered1.lr_ann.vcf -b "${i}"_perspecies.trimmed_filtered1.het.bed -header > "${i}"_perspecies.trimmed_filtered2.lr_ann.vcf
#   done
# grep -v '#' lp_perspecies.trimmed_filtered2.lr_ann.vcf | wc -l #
# grep -v '#' ll_perspecies.trimmed_filtered2.lr_ann.vcf | wc -l #

```

##Plot the distribution of Het ratio to determine the filtering threshold. Unused in final version.
...
ll_het_ratio <- read_tsv("/Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/het_ratio/ll_perspecies.trimmed_filtered1.het.bed",col_names=F)
ll_het_ratio
...
lp_het_ratio <- read_tsv("/Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/het_ratio/lp_perspecies.trimmed_filtered1.het.bed",col_names=F)
lp_het_ratio
...
cd $V_PATH/annotation/
screen -S perpop.lr_ann.log
script perpop.lr_ann.log
...
for i in ${SPECIES[@]}
  ...
  for j in ${POP[@]}
    ...
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered1.lr_ann.vcf \
    -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_with1.lr_ann.vcf \
    -env \
    --sample_file pop_list_to_remove.txt
    rm pop_list_to_remove.txt
    done
...
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_no_perpop/c_ll_no_perpop_with1.lr_ann.vcf | wc -l #1828234
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_po_perpop/c_ll_po_perpop_with1.lr_ann.vcf | wc -l #1881324
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_ki_perpop/c_ll_ki_perpop_with1.lr_ann.vcf | wc -l #2314621
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_do_perpop/c_lp_do_perpop_with1.lr_ann.vcf | wc -l #1328679
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_sm_perpop/c_lp_sm_perpop_with1.lr_ann.vcf | wc -l #1689201
...
cd $V_PATH/annotation/
screen -S perpop_filtered2.lr_ann.log
script perpop_filtered2.lr_ann.log
...
for i in ${SPECIES[@]}
  ...
  for j in ${POP[@]}
    ...
    echo "${j}"
    rm pop_list_to_remove.txt
    $BCF query -l $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered2.lr_ann.vcf | grep "${j}" > pop_list_to_remove.txt
    cat pop_list_to_remove.txt
    mkdir "${j}"_perpop
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered2.lr_ann.vcf \
    -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_filtered2_with1.lr_ann.vcf \
    -env \
    --sample_file pop_list_to_remove.txt
    rm pop_list_to_remove.txt
    done
...
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_no_perpop/c_ll_no_perpop_filtered2_with1.lr_ann.vcf | wc -l #1828234
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_po_perpop/c_ll_po_perpop_filtered2_with1.lr_ann.vcf | wc -l #1881324
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_ki_perpop/c_ll_ki_perpop_filtered2_with1.lr_ann.vcf | wc -l #2314621
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_do_perpop/c_lp_do_perpop_filtered2_with1.lr_ann.vcf | wc -l #1328679
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_sm_perpop/c_lp_sm_perpop_filtered2_with1.lr_ann.vcf | wc -l #1689201


#Filtered3: Split the annotated filtered3 VCF into per population VCFs, and then keep only those positions that are variable or fixed for the derived allele within each population (with -env flag).

cd $V_PATH/annotation/
screen -S perpop_filtered3.lr_ann.log
script perpop_filtered3.lr_ann.log
...
for i in ${SPECIES[@]}
  ...
  for j in ${POP[@]}
    ...
    echo "${j}"
    rm pop_list_to_remove.txt
    $BCF query -l $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered3.lr_ann.vcf | grep "${j}" > pop_list_to_remove.txt
    cat pop_list_to_remove.txt
    mkdir "${j}"_perpop
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perspecies.trimmed_filtered3.lr_ann.vcf \
    -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_filtered3_with1.lr_ann.vcf \
    -env \
    --sample_file pop_list_to_remove.txt
    rm pop_list_to_remove.txt
    done
...
grep -v '#' /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_no_perpop/c_ll_no_perpop_filtered3_with1.lr_ann.vcf | wc -l #1809091
grep -v '#' /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_po_perpop/c_ll_po_perpop_filtered3_with1.lr_ann.vcf | wc -l #1861524
grep -v '#' /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_ll_ki_perpop/c_ll_ki_perpop_filtered3_with1.lr_ann.vcf | wc -l #2291797
grep -v '#' /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_do_perpop/c_lp_do_perpop_filtered3_with1.lr_ann.vcf | wc -l #1245151
grep -v '#' /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_sm_perpop/c_lp_sm_perpop_filtered3_with1.lr_ann.vcf | wc -l #1576151


#For lp, repeat the process with the unfiltered lp VCF to check if the missingness threshold is affecting differently each dataset (project genome vs 5x).

cd $V_PATH/annotation/
screen -S perpop_unfiltered.lr_ann.log
script perpop_unfiltered.lr_ann.log
...
for i in ${SPECIES[@]}
  ...
  for j in ${POP[@]}
    ...
    echo "${j}"
    rm pop_list_to_remove.txt
    $BCF query -l $V_PATH/annotation/"${i}"_perspecies.trimmed.lr_ann.vcf | grep "${j}" > pop_list_to_remove.txt
    cat pop_list_to_remove.txt
    mkdir "${j}"_perpop
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perspecies.trimmed.lr_ann.vcf \
    -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_unfiltered_with1.lr_ann.vcf \
    -env \
    --sample_file pop_list_to_remove.txt
    rm pop_list_to_remove.txt
    done
...
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_do_perpop/c_lp_do_perpop_unfiltered_with1.lr_ann.vcf | wc -l #1328679
grep -v '#' /home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/c_lp_sm_perpop/c_lp_sm_perpop_unfiltered_with1.lr_ann.vcf | wc -l #1689201
...
#Generate individual VCFs from the per species filtered1 VCFs:
cd $V_PATH/annotation
screen -S individuals.lr_ann.log
script individuals.lr_ann.log
...
for i in ${POP[@]}
  ...
  for j in ${INDIVIDUALS[@]}
    ...
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perpop/"${i}"_perpop_with1.lr_ann.vcf \
    -o $V_PATH/annotation/"${i}"_individuals/"${j}"_individual_with1.lr_ann.vcf \
    -env \
    -sn $ID
    done
  done
...
#Generate individual VCFs from the per species filtered2 VCFs:
cd $V_PATH/annotation
screen -S individuals_filtered2.lr_ann.log
script individuals_filtered2.lr_ann.log
...
for i in ${POP[@]}
  ...
  for j in ${INDIVIDUALS[@]}
    ...
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perpop/"${i}"_perpop_filtered2_with1.lr_ann.vcf \
    -o $V_PATH/annotation/"${i}"_individuals/"${j}"_individual_filtered2_with1.lr_ann.vcf \
    -env \
    -sn $ID
    done
  done
...
#Generate individual VCFs from the per species filtered3 VCFs:
cd $V_PATH/annotation
screen -S individuals_filtered3.lr_ann.log
script individuals_filtered3.lr_ann.log
...
for i in ${POP[@]}
  ...
  for j in ${INDIVIDUALS[@]}
    ...
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perpop/"${i}"_perpop_filtered3_with1.lr_ann.vcf \
    -o $V_PATH/annotation/"${i}"_individuals/"${j}"_individual_filtered3_with1.lr_ann.vcf \
    -env \
    -sn $ID
    done
  done
...
#Generate individual VCFs from the lp unfiltered VCFs:
cd $V_PATH/annotation
screen -S individuals_unfiltered.lr_ann.log
script individuals_unfiltered.lr_ann.log

#For each individual in the sm & do populations, subset its variants from the respective population VCF:
cd $G_PATH
...
for i in ${POP[@]}
  ...
  for j in ${INDIVIDUALS[@]}
    ...
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perpop/"${i}"_perpop_unfiltered_with1.lr_ann.vcf \
    -o $V_PATH/annotation/"${i}"_individuals/"${j}"_individual_unfiltered_with1.lr_ann.vcf \
    -env \
    -sn $ID
    done
  done
...
cd $V_PATH/annotation/
screen -S snpeff_individual_summary_filtered3_with1.lr_ann.log
script snpeff_individual_summary_filtered3_with1.lr_ann.log

rm snpeff_individual_summary_filtered3_with1.lr_ann.txt
echo -e "species\tpopulation\tsample\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_individual_summary_filtered3_with1.lr_ann.txt
INDLIST=($(ls `find . -name '*individual_filtered3_with1.lr_ann.vcf' -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}')
  ...
  MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
  MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
  echo -e "$SPECIES\t$POPULATION\t$SAMPLE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_individual_summary_filtered3_with1.lr_ann.txt
  done
...
#EXTRA: Test hard-filters.
##Retrieve values for the statistics from the VCFs:
```{r Split the population VCFs into per individual VCFs, eval=FALSE, engine='bash'}

CALLING=(c_ll_lp_plus_h_ll)
/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation
mkdir -p testing_hard_filters
cd testing_hard_filters
screen -S "${CALLING}"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
script "${CALLING}_testing_hard_filters.lr_ann.log"
CALLING=${STY#*.}
CALLING=${CALLING%-*}
...
rm ${CALLING}_testing_hard_filters.lr_ann.txt
echo "QUAL"
grep -v '#' ../${CALLING}_polarized_filtered3.lr_ann.vcf | awk -F"\t" '{printf ("%s\t%s\t%s\n", "QUAL",$6,"pre")}' >> ${CALLING}_testing_hard_filters.lr_ann.txt
echo "pre-filter complete"
grep -v '#' ../${CALLING}_polarized_filtered4.lr_ann.vcf | awk -F"\t" '{printf ("%s\t%s\t%s\n", "QUAL",$6,"post")}' >> ${CALLING}_testing_hard_filters.lr_ann.txt
echo "post-filter complete"
ANNOTATIONS=(QD FS MQ MQRankSum ReadPosRankSum)
for a in ${ANNOTATIONS[@]}
  do
  echo "${a}"
  grep -v '#' ../${CALLING}_polarized_filtered3.lr_ann.vcf | awk -F"${a}=" '{print $2}' | awk -v ann="$a" -F";" '{printf ("%s\t%s\t%s\n", ann,$1,"pre")}' >> ${CALLING}_testing_hard_filters.lr_ann.txt
  echo "pre-filter complete"
  grep -v '#' ../${CALLING}_polarized_filtered4.lr_ann.vcf | awk -F"${a}=" '{print $2}' | awk -v ann="$a" -F";" '{printf ("%s\t%s\t%s\n", ann,$1,"post")}' >> ${CALLING}_testing_hard_filters.lr_ann.txt
  echo "post-filter complete"
  done
echo "SOR"
grep -v '#' ../${CALLING}_polarized_filtered3.lr_ann.vcf | awk -F"SOR=" '{print $2}' | awk -F"\t" '{printf ("%s\t%s\t%s\n", "SOR",$1,"pre")}' >> ${CALLING}_testing_hard_filters.lr_ann.txt
echo "pre-filter complete"
grep -v '#' ../${CALLING}_polarized_filtered4.lr_ann.vcf | awk -F"SOR=" '{print $2}' | awk -F"\t" '{printf ("%s\t%s\t%s\n", "SOR",$1,"post")}' >> ${CALLING}_testing_hard_filters.lr_ann.txt
echo "post-filter complete"
echo "DONE"

#From outside the server:
CALLING=(c_ll_lp_plus_h_ll)
scp dkleinman@genomics-b.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/testing_hard_filters/${CALLING}_testing_hard_filters.lr_ann.txt /Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters

```

##Draw distributions of the statistics.
```{r Other stuff}
...
#Load data from each calling
distribution_file <- read_tsv("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters/c_ll_lp_plus_h_ll_testing_hard_filters.lr_ann.txt",col_names=c("ANN","value","filter"))
calling <- "c_ll_lp_plus_h_ll"

#Construct the plots:
distribution_file$filter = factor(distribution_file$filter,levels=c("pre","post"))
for (ann in grep("FS|QUAL",unique(distribution_file$ANN),value=TRUE,invert=TRUE)) {
  distribution_ann <- distribution_file %>% filter(ANN==ann)
  distribution_ann_ggplot <- ggplot(data=distribution_ann, aes(x=value,colour=filter)) +
  geom_density() +
  #scale_x_log10() +
  xlab(ann) +
  theme_bw() +
  ...
  distribution_ann_ggplot
  ggsave(paste0(calling,"_testing_hard_filters_",ann,".pdf"), width=16, height=14, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/testing_hard_filters/")
}
for (ann in grep("FS|QUAL",unique(distribution_file$ANN),value=TRUE)) {
  distribution_ann <- distribution_file %>% filter(ANN==ann)
  distribution_ann_ggplot <- ggplot(data=distribution_ann, aes(x=value,colour=filter)) +
  geom_density() +
  scale_x_log10() +
...
echo "Filtering out low MQRankSum variants"
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
-T SelectVariants \
-select "MQRankSum >= -12.5" \
-R $REF \
-V ../${CALLING}"_polarized_filtered3.lr_ann.vcf" \
-o ${CALLING}"_polarized_filtered4_MQRankSum.lr_ann.vcf"
...
echo "Filtering out low ReadPosRankSum variants"
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
-T SelectVariants \
-select "ReadPosRankSum >= -8.0" \
-R $REF \
-V ../${CALLING}"_polarized_filtered3.lr_ann.vcf" \
-o ${CALLING}"_polarized_filtered4_ReadPosRankSum.lr_ann.vcf"
...
$BCF view -e 'QUAL < 30.0 | INFO/QD<2.0 | INFO/FS>60.0 | INFO/MQ<40.0 | INFO/MQRankSum<-12.5 | INFO/ReadPosRankSum<-8.0' ${CALLING}"_polarized_filtered3_"${TYPE}".lr_ann.vcf" > ${CALLING}"_polarized_filtered4bis_"${TYPE}".lr_ann.vcf"
