...
cd $V_PATH/annotation/
screen -S snpeff_population_summary.log
script snpeff_population_summary.log

rm snpeff_population_summary.txt
echo -e "species\tpopulation\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_population_summary.txt
...
for pop in "${POPLIST[@]}"
  ...
  if [ -e ${pop}_random_subsample.ann.vcf ]; then
    echo "Subsample available"
    TOTAL_V=$(grep -v '#' ${pop}_random_subsample.ann.vcf | wc -l)
    INTERGENIC_V=$(grep 'intergenic' ${pop}_random_subsample.ann.vcf | wc -l)
    INTRONIC_V=$(grep 'intron_variant' ${pop}_random_subsample.ann.vcf | wc -l)
    CODING_V=$(grep 'CDS' ${pop}_random_subsample.ann.vcf | wc -l)
    SYNONYMOUS_V=$(grep 'synonymous_variant' ${pop}_random_subsample.ann.vcf | wc -l)
    SYNONYMOUS_A=$(grep 'synonymous_variant' ${pop}_random_subsample.ann.vcf | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | paste -sd+ | bc)
    MISSENSE_V=$(grep 'missense_variant' ${pop}_random_subsample.ann.vcf | wc -l)
    MISSENSE_A=$(grep 'missense_variant' ${pop}_random_subsample.ann.vcf | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | paste -sd+ | bc)
    NONSENSE_V=$(grep -e 'lost' -e 'gained' ${pop}_random_subsample.ann.vcf | wc -l)
    NONSENSE_A=$(grep -e 'lost' -e 'gained' ${pop}_random_subsample.ann.vcf | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | paste -sd+ | bc)
    ...
    echo -e "$SPECIES\t$POPULATION\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_summary.txt
  else
    echo "Original available"
    TOTAL_V=$(grep -v '#' ${pop}.ann.vcf | wc -l)
    INTERGENIC_V=$(grep 'intergenic' ${pop}.ann.vcf | wc -l)
    INTRONIC_V=$(grep 'intron_variant' ${pop}.ann.vcf | wc -l)
    CODING_V=$(grep 'CDS' ${pop}.ann.vcf | wc -l)
    SYNONYMOUS_V=$(grep 'synonymous_variant' ${pop}.ann.vcf | wc -l)
    SYNONYMOUS_A=$(grep 'synonymous_variant' ${pop}.ann.vcf | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | paste -sd+ | bc)
    MISSENSE_V=$(grep 'missense_variant' ${pop}.ann.vcf | wc -l)
    MISSENSE_A=$(grep 'missense_variant' ${pop}.ann.vcf | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | paste -sd+ | bc)
    NONSENSE_V=$(grep -e 'lost' -e 'gained' ${pop}.ann.vcf | wc -l)
    NONSENSE_A=$(grep -e 'lost' -e 'gained' ${pop}.ann.vcf | cut -f8 | cut -d';' -f2 | cut -d'=' -f2 | paste -sd+ | bc)
    MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
    MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
    echo -e "$SPECIES\t$POPULATION\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_summary.txt
  fi
