...
#Split the annotated VCF into per population VCFs, and then keep only those positions that are variable or fixed for the derived allele within each population (with -env flag).
    ...
    -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts.lr_ann.vcf \
    -env \
...
#Subsample all populations that need subsampling (original code where the files with the ids are created is in section 8):
    ...
    if [ -e "${j}"_perpop_minimum_subsample.lr_ann.vcf ]; then
      echo "Minimum "$POPULATION" subsample available"
      FILE="${j}"_perpop_minimum_ids.txt
      java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
      -T SelectVariants \
      -R $REF \
      -V $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts.lr_ann.vcf \
      -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_minimum_subsample_test_allele_counts.lr_ann.vcf \
      -env \
      --sample_file $FILE
     elif [ -e "${j}"_perpop_random_subsample.lr_ann.vcf ]; then
      ...
      -R $REF \
      -V $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts.lr_ann.vcf \
      -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_random_subsample_test_allele_counts.lr_ann.vcf \
      -env \
      --sample_file $FILE
      else
...
for pop in "${POPLIST[@]}"
  ...
  if [ -e ${pop}_minimum_subsample_test_allele_counts.lr_ann.vcf ]; then
    echo "Minimum "$POPULATION" subsample available"
    FILE=${pop}_minimum_subsample_test_allele_counts.lr_ann.vcf
  elif [ -e ${pop}_random_subsample_test_allele_counts.lr_ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample_test_allele_counts.lr_ann.vcf
  else
    echo "Original "$POPULATION" available"
    FILE=${pop}_test_allele_counts.lr_ann.vcf
  fi
