...
grep -v '#' lp_perspecies.trimmed.ann.vcf | wc -l #
...
for i in ${SPECIES[@]}
  do
  echo "${i}"
  MIN_DP=$(cat /home/dkleinman/datos/c_lp_depth_calculus/c_"${i}"*.csv | awk '{print $8}')
  MAX_DP=$(cat /home/dkleinman/datos/c_lp_depth_calculus/c_"${i}"*.csv | awk '{print $9}')
  java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
  -T SelectVariants \
  -select "DP > ${MIN_DP} && DP < ${MAX_DP}" \
  -R $REF \
  -V "${i}"_perspecies.trimmed.ann.vcf \
  -o "${i}"_perspecies.trimmed_filtered.ann.vcf
  done
