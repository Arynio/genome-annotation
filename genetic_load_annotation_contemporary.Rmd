...
for pop in "${POPLIST[@]}"
  ...
  SPECIES=$(echo "${pop}" | cut -c3-4)
  POPULATION=$(echo "${pop}" | cut -c6-7)
  if [ -e ${pop}_mixed_subsample.ann.vcf ]; then
    echo "Mixed "$POPULATION" subsample available"
    FILE=${pop}_mixed_subsample.ann.vcf
  elif [ -e ${pop}_random_subsample.ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample.ann.vcf
  else
    echo "Original "$POPULATION" available"
    FILE=${pop}.ann.vcf
  fi
...
for pop in "${POPLIST[@]}"
  ...
  SPECIES=$(echo "${pop}" | cut -c3-4)
  POPULATION=$(echo "${pop}" | cut -c6-7)
  if [ -e ${pop}_minimum_subsample.ann.vcf ]; then
    echo "Minimum "$POPULATION" subsample available"
    FILE=${pop}_minimum_subsample.ann.vcf
  elif [ -e ${pop}_random_subsample.ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample.ann.vcf
  else
...
INDLIST=($(ls `find . -name '*individual.ann.vcf' -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}')
