...
#This is the command to use in order to recode a VCF with ancestral/derived alleles: /opt/vcftools_0.1.13/perl/fill-aa. There's documentation inside that command that I'll follow here.

#First, the ancestral alleles file should be bgzipped (according the documentation, they should be gzipped, but later on when trying to run faidx I got an error stating that files should be gzipped in order to build a fai index):
...
screen -S c_ll_lp_plus_h_ll_polarized_vcf.log
script c_ll_lp_plus_h_ll_polarized_vcf.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani #VCFs path

cat $V_PATH/c_ll_lp_plus_h_ll_renamed.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz > c_ll_lp_plus_h_ll_polarized.vcf

#Next, polarize the VCF according to the new AA annotation.

This needs to be resolved...



#Another option is to generate a new VCF using the ancestral as the reference...
...
#3b: Toy annotation using the immunocapture VCF.
...
#4a: Trim contemporary population lp and ll VCFs. Trim previously generated VCFs with species-wide variants in order to keep populational variants only.
...
#4b: Population annotation (detailed) using the contemporary lp and ll VCFs.
...
POPLIST=($(ls ls {lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}.csv))
for pop in "${POPLIST[@]}"
  do
...
#4c: Individual annotation (detailed) using the contemporary lp and ll VCFs.
