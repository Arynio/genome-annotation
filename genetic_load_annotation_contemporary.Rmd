...
BAMLIST=$(ls /home/dkleinman/datos/c_lp_depth_calculus/"$POP".intergenic.bamlist)
...
####Obtain kinship coefficient estimates between individuals.
...
bcftools query -l toy.vcf | tr _ - > samples_renamed.txt
bcftools reheader -s samples_renamed.txt toy.vcf > toy_renamed.vcf
plink_1.9 --vcf toy_renamed.vcf --make-bed --out toy --allow-extra-chr #turn into plink binary format
#Then download the files to use KING (it's in my local PC)
./king -b toy.bed --kinship #it doesn't work because it doesn't recognize scaffolds as chromosomes...
