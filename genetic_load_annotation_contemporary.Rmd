...
##Test: split the VCF into per population VCFs, subsample populations, and split the VCF into per individual VCFs. Then get annotation statistics for all populational and individual VCFs.

```{r Obtain per population VCFs, eval=FALSE, engine='bash'}

#This code is adapted from that from subsequent sections (8-10). It ends up yielding allele counts for all populations (subsampled to the minimum size -8-, and for all individuals) starting from the original per species VCFs (without any filtering), to test if the strange patterns found with the filtered ones -high number of synonymous mutations in pardinus- was already present prior to the differential per species filtering.

cd $V_PATH/annotation/
screen -S testing_allele_counts.ann.log
script testing_allele_counts.ann.log
...
#Split the annotated VCF into per population VCFs, and then keep only those positions that are variable or fixed for the derived allele within each population (with -env flag).
    ...
    $BCF query -l $V_PATH/annotation/"${i}"_perspecies.trimmed.ann.vcf | grep "${j}" > pop_list_to_remove.txt
    cat pop_list_to_remove.txt
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perspecies.trimmed.ann.vcf \
    -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts.ann.vcf \
    -env \
...
#Subsample all populations that need subsampling (original code where the files with the ids are created is in section 8):
    ...
    if [ -e "${j}"_perpop_minimum_subsample.ann.vcf ]; then
      ...
      -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_minimum_subsample_test_allele_counts.ann.vcf \
      -env \
      --sample_file $FILE
     elif [ -e "${j}"_perpop_random_subsample.ann.vcf ]; then
      ...
      -R $REF \
      -V $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_test_allele_counts.ann.vcf \
      -o $V_PATH/annotation/"${j}"_perpop/"${j}"_perpop_random_subsample_test_allele_counts.ann.vcf \
      -env \
      --sample_file $FILE
      else
...
cd $V_PATH/annotation/
rm snpeff_population_between_test_allele_counts.txt
echo -e "species\tpopulation\tsize\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_population_between_test_allele_counts.txt
...
for pop in "${POPLIST[@]}"
  ...
  if [ -e ${pop}_minimum_subsample_test_allele_counts.ann.vcf ]; then
    echo "Minimum "$POPULATION" subsample available"
    FILE=${pop}_minimum_subsample_test_allele_counts.ann.vcf
  elif [ -e ${pop}_random_subsample_test_allele_counts.ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample_test_allele_counts.ann.vcf
  else
    echo "Original "$POPULATION" available"
    FILE=${pop}_test_allele_counts.ann.vcf
  fi
  ...
  echo -e "$SPECIES\t$POPULATION\t$SIZE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_between_test_allele_counts.txt
  done
...
####Option A: obtain PLINK kinship coefficient estimates between individuals. In the end we used option B.

```{r Obtain per population VCFs, eval=FALSE, engine='bash'}

#Option B was finally chosen over this one.
...
####Option B: obtain NGSrelate kinship coefficient estimates between individuals. This is the chosen option.
...
for (i in pops) {
  print(i)
  kinship_file <- read_tsv(paste0("/Users/Dani/ownCloud/backup/annotation/g-w_relatedness/NGSrelate/",i,".perpop.kinship.gl.res"),col_names=F) %>% select(3,4,12) %>% mutate(pop=i,pair=paste(X3,X4,sep="_")) %>% select(4,5,3)
...
kinship_plot
ggsave("/Users/Dani/ownCloud/backup/annotation/g-w_relatedness/NGSrelate/all_kinships.pdf", width=50, height=10, units="cm", device="pdf")
...
cd $V_PATH/annotation/c_lp_sm_perpop/
screen -S c_lp_sm_perpop_mixed_subsample.ann.log
script c_lp_sm_perpop_mixed_subsample.ann.log

$BCF query -l $V_PATH/annotation/c_lp_sm_perpop/c_lp_sm_perpop.ann.vcf > c_lp_sm_perpop_all_ids.txt #Obtain all sm ids
...
-R $REF \
-V $V_PATH/annotation/c_lp_sm_perpop/c_lp_sm_perpop.ann.vcf \
-o $V_PATH/annotation/c_lp_sm_perpop/c_lp_sm_perpop_mixed_subsample.ann.vcf \
-env \
--sample_file c_lp_sm_perpop_mixed_ids.txt

grep '#' -v c_lp_sm_perpop_mixed_subsample.ann.vcf | wc -l #1640979


#100% random subset for KIR:
...
#Minimum size subset for SM (partially random: 8 individuals are randomly selected after discarding 3 with high kinship):

cd $V_PATH/annotation/c_lp_sm_perpop/
screen -S c_lp_sm_perpop_minimum_subsample.ann.log
script c_lp_sm_perpop_minimum_subsample.ann.log
...
-R $REF \
-V $V_PATH/annotation/c_lp_sm_perpop/c_lp_sm_perpop.ann.vcf \
-o $V_PATH/annotation/c_lp_sm_perpop/c_lp_sm_perpop_minimum_subsample.ann.vcf \
-env \
--sample_file c_lp_sm_perpop_minimum_ids.txt

grep '#' -v c_lp_sm_perpop_minimum_subsample.ann.vcf | wc -l #1577071


#Minimum size subset for DO (all 8 individuals are randomly selected):

cd $V_PATH/annotation/c_lp_do_perpop/
screen -S c_lp_do_perpop_minimum_subsample.ann.log
script c_lp_do_perpop_minimum_subsample.ann.log

$BCF query -l $V_PATH/annotation/c_lp_do_perpop/c_lp_do_perpop.ann.vcf | shuf -n 8 > c_lp_do_perpop_minimum_ids.txt #randomly select 8 individuals out of the 12.

cat c_lp_do_perpop_minimum_ids.txt
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
-T SelectVariants \
-R $REF \
-V $V_PATH/annotation/c_lp_do_perpop/c_lp_do_perpop.ann.vcf \
-o $V_PATH/annotation/c_lp_do_perpop/c_lp_do_perpop_minimum_subsample.ann.vcf \
-env \
--sample_file c_lp_do_perpop_minimum_ids.txt

grep '#' -v c_lp_do_perpop_minimum_subsample.ann.vcf | wc -l #1296581
...
#10: Mask positions. Subset the VCFs in order to remove certain variants.
##Mask unpolarized positions from all VCFs. Remove all variants where AA=N.

```{r Mask positions, eval=FALSE, engine='bash'}

#During the polarization step, we didn't code any solution for AA=N, so those were left unresolved (AA=REF). We shouldn't take into account those positions since they could introduce a bias against AF=1 in SM (as we noticed when counting alleles), so we'll remove all of them from all perpop and per individual VCFs before counting alleles again.

screen -S remove_AA_N_positions.log
script remove_AA_N_positions.log

cd $V_PATH/annotation/
POP_VCFS=$(find $V_PATH/annotation/ -name "*perpop*.ann.vcf" | grep -v 'woutN')
for pop in ${POP_VCFS[@]}
  do
  echo "${pop}"
  grep -v "AA=N" ${pop} > ${pop/.ann.vcf/_woutN.ann.vcf}
  done
IND_VCFS=$(find $V_PATH/annotation/ -name "*individual.ann.vcf" | grep -v 'woutN')
for ind in ${IND_VCFS[@]}
  do
  echo "${ind}"
  grep -v "AA=N" ${ind} > ${ind/.ann.vcf/_woutN.ann.vcf}
  done

```

##Mask fixed positions from all VCFs. Remove all variants where AF=1.
...
cd $V_PATH/annotation/
POP_VCFS=$(find $V_PATH/annotation/ -name "*perpop*woutN.ann.vcf" | grep -v 'wout1')
for pop in ${POP_VCFS[@]}
  do
  echo "${pop}"
  grep -v "AF=1.00" ${pop} > ${pop/.ann.vcf/_wout1.ann.vcf}
  done
IND_VCFS=$(find $V_PATH/annotation/ -name "*individual_woutN.ann.vcf" | grep -v 'wout1')
for ind in ${IND_VCFS[@]}
  do
  echo "${ind}"
  grep -v "AF=1.00" ${ind} > ${ind/.ann.vcf/_wout1.ann.vcf}
  done

```

#11: Get annotation statistics.
##Without N. Obtain statistics for all VCFs where AA=N positions have been removed.
###At the population level (within species comparison).

```{r Get annotation statistics, eval=FALSE, engine='bash'}

cd $V_PATH/annotation/
screen -S snpeff_population_within_summary.log
script snpeff_population_within_summary.log

rm snpeff_population_within_summary.txt
echo -e "species\tpopulation\tsize\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_population_within_summary.txt
POPLIST=($(ls -d c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}perpop))
for pop in "${POPLIST[@]}"
  do
  echo "${pop}"
  ...
  SPECIES=$(echo "${pop}" | cut -c3-4)
  POPULATION=$(echo "${pop}" | cut -c6-7)
  if [ -e ${pop}_mixed_subsample_woutN.ann.vcf ]; then
    echo "Mixed "$POPULATION" subsample available"
    FILE=${pop}_mixed_subsample_woutN.ann.vcf
  elif [ -e ${pop}_random_subsample_woutN.ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample_woutN.ann.vcf
  else
    echo "Original "$POPULATION" available"
    FILE=${pop}_woutN.ann.vcf
  fi
  SIZE=$($BCF query -l $FILE | wc -l)
  TOTAL_V=$(grep -v '#' $FILE | wc -l)
  INTERGENIC_V=$(grep 'intergenic' $FILE | wc -l)
  ...
  MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
  MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
  echo -e "$SPECIES\t$POPULATION\t$SIZE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_within_summary.txt
  done
...
cd $V_PATH/annotation/
screen -S snpeff_population_between_summary.log
script snpeff_population_between_summary.log

rm snpeff_population_between_summary.txt
echo -e "species\tpopulation\tsize\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_population_between_summary.txt
POPLIST=($(ls -d c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}perpop))
for pop in "${POPLIST[@]}"
  do
  echo "${pop}"
  ...
  SPECIES=$(echo "${pop}" | cut -c3-4)
  POPULATION=$(echo "${pop}" | cut -c6-7)
  if [ -e ${pop}_minimum_subsample_woutN.ann.vcf ]; then
    echo "Minimum "$POPULATION" subsample available"
    FILE=${pop}_minimum_subsample_woutN.ann.vcf
  elif [ -e ${pop}_random_subsample_woutN.ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample_woutN.ann.vcf
  else
  ...
  MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
  MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
  echo -e "$SPECIES\t$POPULATION\t$SIZE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_between_summary.txt
  done
...
cd $V_PATH/annotation/
screen -S snpeff_individual_summary.log
script snpeff_individual_summary.log

rm snpeff_individual_summary.txt
echo -e "species\tpopulation\tsample\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_individual_summary.txt
INDLIST=($(ls `find . -name '*individual_woutN.ann.vcf' -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}')
  ...
  MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
  MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
  echo -e "$SPECIES\t$POPULATION\t$SAMPLE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_individual_summary.txt
  done

```

##Without 1. Obtain statistics for all VCFs where AA=N and AF=1 positions have been removed.
###At the population level (within species comparison).

```{r Get annotation statistics, eval=FALSE, engine='bash'}

cd $V_PATH/annotation/
screen -S snpeff_population_within_summary_wout1.log
script snpeff_population_within_summary_wout1.log

rm snpeff_population_within_summary_wout1.txt
echo -e "species\tpopulation\tsize\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_population_within_summary_wout1.txt
POPLIST=($(ls -d c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}perpop))
for pop in "${POPLIST[@]}"
  do
  echo "${pop}"
  ...
  SPECIES=$(echo "${pop}" | cut -c3-4)
  POPULATION=$(echo "${pop}" | cut -c6-7)
  if [ -e ${pop}_mixed_subsample_woutN_wout1.ann.vcf ]; then
    echo "Mixed "$POPULATION" subsample available"
    FILE=${pop}_mixed_subsample_woutN_wout1.ann.vcf
  elif [ -e ${pop}_random_subsample_woutN_wout1.ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample_woutN_wout1.ann.vcf
  else
    echo "Original "$POPULATION" available"
    FILE=${pop}_woutN_wout1.ann.vcf
  fi
  SIZE=$($BCF query -l $FILE | wc -l)
  TOTAL_V=$(grep -v '#' $FILE | wc -l)
  INTERGENIC_V=$(grep 'intergenic' $FILE | wc -l)
  ...
  MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
  MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
  echo -e "$SPECIES\t$POPULATION\t$SIZE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_within_summary_wout1.txt
  done
...
cd $V_PATH/annotation/
screen -S snpeff_population_between_summary_wout1.log
script snpeff_population_between_summary_wout1.log

rm snpeff_population_between_summary_wout1.txt
echo -e "species\tpopulation\tsize\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_population_between_summary_wout1.txt
POPLIST=($(ls -d c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}perpop))
for pop in "${POPLIST[@]}"
  do
  echo "${pop}"
  ...
  SPECIES=$(echo "${pop}" | cut -c3-4)
  POPULATION=$(echo "${pop}" | cut -c6-7)
  if [ -e ${pop}_minimum_subsample_woutN_wout1.ann.vcf ]; then
    echo "Minimum "$POPULATION" subsample available"
    FILE=${pop}_minimum_subsample_woutN_wout1.ann.vcf
  elif [ -e ${pop}_random_subsample_woutN_wout1.ann.vcf ]; then
    echo "Random "$POPULATION" subsample available"
    FILE=${pop}_random_subsample_woutN_wout1.ann.vcf
  else
  ...
  MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
  MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
  echo -e "$SPECIES\t$POPULATION\t$SIZE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_population_between_summary_wout1.txt
  done
...
cd $V_PATH/annotation/
screen -S snpeff_individual_summary_wout1.log
script snpeff_individual_summary_wout1.log

rm snpeff_individual_summary_wout1.txt
echo -e "species\tpopulation\tsample\ttotal_V\tintergenic_V\tintronic_V\tcoding\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tmissense/synonymous_V\tmissense/synonymous_A" > snpeff_individual_summary_wout1.txt
INDLIST=($(ls `find . -name '*individual_woutN_wout1.ann.vcf' -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ind=$(echo "${i}" | awk -F'[/]' '{print $3}')
  ...
  MISSENSE_SYNONYMOUS_V=$(echo "scale=3; $MISSENSE_V/$SYNONYMOUS_V" | bc)
  MISSENSE_SYNONYMOUS_A=$(echo "scale=3; $MISSENSE_A/$SYNONYMOUS_A" | bc)
  echo -e "$SPECIES\t$POPULATION\t$SAMPLE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A" >> $V_PATH/annotation/snpeff_individual_summary_wout1.txt
  done
...
wd <- file.path("/Users/Dani/ownCloud/backup/annotation/genetic_load")
setwd(wd)
files <- grep(grep(list.files(file.path("/Users/Dani/ownCloud/backup/annotation/genetic_load"), pattern = "snpeff", full.names=T), pattern='test_allele_counts', inv=T, value=T),pattern='between', value=T)
files
...
for (f in c(1,3,2)) {
  ...
  current_file <- current_file %>% mutate("I_ratio"=intronic_V/intergenic_V,"S_ratio*10"=10*synonymous_V/intergenic_V,"NS_ratio*10"=10*missense_V/intergenic_V,"LOF_ratio*1000"=1000*nonsense_V/intergenic_V,"NS/S_v"=missense_V/synonymous_V,"NS/S_a"=missense_A/synonymous_A,"LOF/S_v*100"=100*nonsense_V/synonymous_V,"LOF/S_a*100"=100*nonsense_A/synonymous_A,dataset=unlist(strsplit(files[f],"/|_|\\."))[length(unlist(strsplit(files[f],"/|_|\\.")))-1]) %>% select(2,c(16:24))
  current_file
...
perpop_between_statistics$dataset = factor(perpop_between_statistics$dataset,levels(perpop_between_statistics$dataset)[c(1,3,2)])
...
ggplot_perpop_between <- ggplot(data=perpop_between_statistics, aes(x=category,y=value)) +
  ...
  facet_grid(. ~ dataset) +
  ggtitle("Ratios per population (N=8)") +
  theme_bw() +
...
ggsave("perpop_counts_ratios.pdf", width=30, height=15, units="cm", device="pdf", path="/Users/Dani/ownCloud/backup/annotation/genetic_load/")
...
#woutN: for each population (minimum subsample, woutN version) extract allele frequency from all SNPs in each class of interest, and save them in a file:

cd $V_PATH/annotation/
screen -S features_frequency_distribution_woutN.log
script features_frequency_distribution_woutN.log

rm $V_PATH/annotation/features_frequency_distribution_woutN.txt
POPLIST=($(ls -d c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}perpop))
for pop in "${POPLIST[@]}"
  do
  echo "${pop}"
  ...
  for feat in "${FEATURES[@]}"
    do
    echo "${feat}"
    grep "${feat}" $FILE | cut -f8 | cut -d';' -f3 | cut -d'=' -f2 | awk -v pop="$POPULATION" -v feat="${feat}" '{print $1" "pop" "feat}' >> $V_PATH/annotation/features_frequency_distribution_woutN.txt
    done
    cd $V_PATH/annotation/
  done

scp dkleinman@genomics-b.ebd.csic.es:/home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/features_frequency_distribution_woutN.txt /Users/Dani/ownCloud/backup/annotation/genetic_load/features_frequency_distribution/
...
cd $V_PATH/annotation/
screen -S features_frequency_distribution_wout1.log
script features_frequency_distribution_wout1.log

rm $V_PATH/annotation/features_frequency_distribution_wout1.txt
POPLIST=($(ls -d c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}perpop))
for pop in "${POPLIST[@]}"
  do
  echo "${pop}"
  ...
  for feat in "${FEATURES[@]}"
    do
    echo "${feat}"
    grep "${feat}" $FILE | cut -f8 | cut -d';' -f3 | cut -d'=' -f2 | awk -v pop="$POPULATION" -v feat="${feat}" '{print $1" "pop" "feat}' >> $V_PATH/annotation/features_frequency_distribution_wout1.txt
    done
    cd $V_PATH/annotation/
  done

scp dkleinman@genomics-b.ebd.csic.es:/home/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/annotation/features_frequency_distribution_wout1.txt /Users/Dani/ownCloud/backup/annotation/genetic_load/features_frequency_distribution/
...
features_frequency_distribution <- read_delim("/Users/Dani/ownCloud/backup/annotation/genetic_load/features_frequency_distribution/features_frequency_distribution_woutN.txt",col_names=c("AF","pop","feat"),delim=" ")
...
for (pop in pops) {
  ...
  ggtitle(paste0("AF distribution for ",pop)) +
  #ylab("density") +
  #xlab("heritability") +
  theme_bw() +
  theme(text=element_text(size=12,face="bold"),
  ...
  pop_feat_ggplot
  ggsave(paste0(pop,"_AF_distribution_woutN"), width=30, height=25, units="cm", device="pdf", path="/Users/Dani/ownCloud/backup/annotation/genetic_load/features_frequency_distribution/woutN/")
}

#wout1:

features_frequency_distribution <- read_delim("/Users/Dani/ownCloud/backup/annotation/genetic_load/features_frequency_distribution/features_frequency_distribution_wout1.txt",col_names=c("AF","pop","feat"),delim=" ")
features_frequency_distribution
features_frequency_distribution$feat <- as.factor(features_frequency_distribution$feat)
features_frequency_distribution$feat = factor(features_frequency_distribution$feat,levels(features_frequency_distribution$feat)[c(1,2,4,3)])
pops <- unique(features_frequency_distribution$pop)
...
for (pop in pops) {
  ...
  pop_feat_ggplot
  ggsave(paste0(pop,"_AF_distribution_wout1"), width=30, height=25, units="cm", device="pdf", path="/Users/Dani/ownCloud/backup/annotation/genetic_load/features_frequency_distribution/wout1/")
}
...
cd $V_PATH/annotation/
rm $V_PATH/annotation/snpeff_lp_total_synonymous_woutN.txt
rm $V_PATH/annotation/snpeff_lp_total_nonsynonymous_woutN.txt
...
for pop in "${POPLIST[@]}"
  ...
  grep 'synonymous_variant' $FILE | cut -f1-2 >> $V_PATH/annotation/snpeff_lp_total_synonymous_woutN.txt
  grep 'missense_variant' $FILE | cut -f1-2 >> $V_PATH/annotation/snpeff_lp_total_nonsynonymous_woutN.txt
  done
  cd $V_PATH/annotation/
  S_V=$(cat $V_PATH/annotation/snpeff_lp_total_synonymous_woutN.txt | wc -l)
  NS_V=$(cat $V_PATH/annotation/snpeff_lp_total_nonsynonymous_woutN.txt | wc -l)
  echo "scale=3 ; $NS_V / $S_V" | bc #equals 0.272, which is about the same than for each population separately
...
for pop in "${POPLIST[@]}"
  ...
  cd $V_PATH/annotation/
  S_V=$(cat $V_PATH/annotation/snpeff_lp_total_synonymous_wout1.txt | wc -l)
  NS_V=$(cat $V_PATH/annotation/snpeff_lp_total_nonsynonymous_wout1.txt | wc -l)
  echo "scale=3 ; $NS_V / $S_V" | bc #equals 0.350, which is about the same than for each population separately
