...
/opt/ngsF-HMM/ngsF-HMM.sh --geno /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/a_ll_pv_analysis/beagle_gl/${POP}.tv.genolike.beagle.gz --lkl --pos ${POP}_variable_coordinates.tv.list --n_ind $N_IND --n_sites $N_SITES --out ${POP}.tv.sh.ngsF-HMM --log 1


#freq_est runs:
screen -S ${POP}.fe2.ngsF-HMM
POP=$(echo ${STY#*.} | cut -d'.' -f1)
script ${POP}.fe2.ngsF-HMM.log
POP=$(echo ${STY#*.} | cut -d'.' -f1)

cd /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/a_ll_pv_analysis/ngsF-HMM/
N_IND=$(wc -l < /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/a_ll_pv_analysis/beagle_gl/${POP}.bamlist)
N_SITES=$(cat ${POP}_variable_coordinates.tv.list | wc -l)
/opt/ngsF-HMM/ngsF-HMM --geno /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/a_ll_pv_analysis/beagle_gl/${POP}.tv.genolike.beagle.gz --lkl --pos ${POP}_variable_coordinates.tv.list --freq_est 2 --n_ind $N_IND --n_sites $N_SITES --out ${POP}.tv.fe2.ngsF-HMM
...
screen -S ${POP}.tv.fe1
POP=$(echo ${STY#*.} | cut -d'.' -f1)
script ${POP}.tv.fe1.per_ind.ibdtract.bed.log
POP=$(echo ${STY#*.} | cut -d'.' -f1)
...
for i in ${INDIVIDUALS[@]}
  ...
  POP=$(echo ${i} | cut -c1-7)
  mkdir -p ./$POP"_ibdtracts"
  grep ${i} $INPUT_FILE > $POP"_ibdtracts"/${i}".tv.fe1.ibdtract.bed"
  done
...
screen -S ${POP}.tv.fe1
POP=$(echo ${STY#*.} | cut -d'.' -f1)
script ${POP}.tv.fe1.ibd_counts.log
POP=$(echo ${STY#*.} | cut -d'.' -f1)

rm ${POP}.tv.fe1.ibdtract_interval_counts.txt
echo -e "tract_length\tn\tind" > ${POP}.tv.fe1.ibdtract_interval_counts.txt
INDIVIDUALS=$(ls `find . -name '*.tv.fe1.ibdtract.bed' -path '*_ibdtracts/*' -print`)
for i in ${INDIVIDUALS[@]}
  do
  ind=$(echo ${i} | cut -d'/' -f3 | cut -d'.' -f1)
  echo "processing individual" ${ind}
  ...
  awk -v ind=$ind '{ 
   ...
   printf "%s\t%d\t%s\n%s\t%d\t%s\n%s\t%d\t%s\n%s\t%d\t%s\n%s\t%d\t%s\n", "<1000",first,ind,"<10000",second,ind,"<100000",third,ind,"<1000000",fourth,ind,">1000000",fifth,ind
   }' ${i/.ibdtract.bed/.ibdtract_counts.bed} >> ${POP}.tv.fe1.ibdtract_interval_counts.txt
   done

#From outside the server:
scp dkleinman@genomics-b.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/a_ll_pv_analysis/ngsF-HMM/*.tv.fe*.ibdtract_interval_counts.txt /Users/dani/ownCloud/backup/g-w_analysis/IBD_analysis/a_ll_pv_analysis/ngsF-HMM/
...
# )
long_ROHs_n_ggplot
ggsave(">1000kb_ibd_tract_n.tv.pdf", width=15, height=12, units="cm", device="pdf", path="/Users/dani/ownCloud/backup/g-w_analysis/IBD_analysis/a_ll_pv_analysis/ngsF-HMM")
