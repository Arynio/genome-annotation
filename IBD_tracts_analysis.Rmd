...
POP=$(ls -rth *.bamlist | cut -d'.' -f1)
cd /GRUPOS/grupolince/CatRef_bams_aln
ls -rth *_ll_*.bam | cut -d'_' -f-4 > /GRUPOS/grupolince/lynx_genomes_5x/IBD_analysis/a_ll_pv_analysis/ngsF-HMM/${POP}.ind
...
echo "tract_length\tn\tind" > ${POP}.ibdtract_interval_counts.txt
INDIVIDUALS=$(ls `find . -name '*.ibdtract.bed' -path '*_ibdtracts/*' -print`)
for i in ${INDIVIDUALS[@]}
  do
  ind=$(echo ${i} | cut -d'/' -f3 | cut -d'.' -f1)
