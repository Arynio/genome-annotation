...
POP=(c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060)
read POP mean sd mean_truncated sd_truncated maxDepth minDepth maxDepth_truncated minDepth_truncated < $I_PATH/beagle_gl/"${POP}"_mean_sd_depthGlobal_lynx_per_pop_mean_folds_0.95.csv
...
cd $I_PATH/beagle_gl
zcat c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060.genolike.beagle.gz | cut -f1 | cut -d$'_' -f1,2 --output-delimiter=' ' | tail -n +2 > $I_PATH/ngsF-HMM/variable_coordinates.list
...
cd $I_PATH/ngsF-HMM
N_SITES=$(cat variable_coordinates.list | wc -l)
$NGSF --geno $I_PATH/beagle_gl/c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060.genolike.beagle.gz --lkl --pos variable_coordinates.list --n_ind 60 --n_sites $N_SITES --out c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060_ngsF-HMM

```

#3: Modify ngsF-HMM output.
Author: Enrico, Dani, Maria

Vamos a modificar el archivo de Enrico que genero para plotear los valores de admixutee. 

Nosotros tenemos un archivo donde están todas las posiciones de cada scaffold, se llama genome_coordinates.list. A este archivo le vamos a pegar una modificación del archivo ibd que en vez de ser una fila por cada individio sea una columna. Así por cada individuo vamos a pegar la columna con sus valores 0 y 1 y la columna con información de scaffold y de base. Vamos a tener tantos archivos como individuos. 

Creamos el archivo con la informaicón de los individuos.
##Count number of large IBD tracts.

```{bash}
# Archivo información scaffold-base: genome_coordinates.list
# El archivo con la informacion ibd por cada individuos en una fila: c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060_ngsF-HMM_modif.ibd (es modif pq no tiene la primera fila que no nos interesa)
# Primero para este analisis nos vamos a quedar solo con los scaffold mayores de 1.6 MB (que es el n50, así que hacemos una lista con ellos)
awk '$2 > 1600000 {print $1}' /home/mlucena/grupolince/reference_genomes/lynx_pardinus_genome/Length_scaffolds_lp23 > scaffold_bigger_than_1.6.borrar
cat <(awk -v OFS='\t' '$2 > 1600000 {print $1,".*",$2}' /home/mlucena/grupolince/reference_genomes/lynx_pardinus_genome/Length_scaffolds_lp23) <(awk  -v OFS='\t' '$2 > 1600000 {print $1, 0}' /home/mlucena/grupolince/reference_genomes/lynx_pardinus_genome/Length_scaffolds_lp23 ) | sort -k1,1 -k2,2n > scaffold_bigger_than_1.6_filtered.borrar
COUNTER=1
rm c_ll_ki-h_ll_pv_n014.ibdtract.bed
echo -e "scaffold\tstart_position\tend_position\tlenght\tind" > c_ll_ki-h_ll_pv_n014.ibdtract.bed
while read IBD_TRACK 
do
# Selecciono el nombre del individuo en cuestión del archivo con los nombres de individuos, usando el número de la iteración
IND_NAME=($(sed -n "${COUNTER}p"  c_ll_ki-h_ll_pv_n014.ind))
echo $IND_NAME
# Guardamos la fila con información ibd del individuo que toque en un archivo temporal
...
# Genero un archivo en el que reporto las regiones en las que hay 1 de forma consecutiva (i.e. que son ibd). Estas regiones tienen una longitud igual a la separación entre dos snps con identidad 0 (la identidad 0 significa que no son ibd, todo lo demás sería 1 por defecto). 
# Primero uno los dos archivos temporales con la información de los sitios variables tomados en consideración y la información del estado de estos sitios (ibd yes/no)
paste <(cat variable_coordinates.list ) <(cat ibd_tracks_temp.borrar) | \
# Aquí convierto este archivo en un bed y selecciono solo los valores que no son ibd.
awk -v OFS='\t' '{print $1, $2-1, $2, $3}' | grep '0$' | \
# Aquí sustraigo de los valores wg aquellas posiciones que son ibd=0 (no ibd)
bedtools subtract -a /home/mlucena/grupolince/reference_genomes/lynx_pardinus_genome/bed_file_all_the_genome.bed -b stdin | \
# Ahora calculo la longitud de estos tracts que son 1. 
...
grep -v -f scaffold_bigger_than_1.6_filtered.borrar - >> c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060.ibdtract.bed
COUNTER=$[$COUNTER+1]
done < <(cat c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060_ngsF-HMM_modif.ibd)
# Sanity check
# Hemos contado numero de scaffold antes y despues de filtros para un individuo y hemos visto que los filtros se aplican correctamente. 
```
...
library("dplyr")
library("ggrepel")
ibd_tract <- read_tsv ("/Users/marialucenaperez/Documents/IBD_tract/c_ll_ki-c_ll_no-c_ll_po-c_lp_do-c_lp_sm_n060.ibdtract.bed", col_names = T) %>%  
  mutate (lenght_category = ifelse (lenght>10000 & lenght<100000,"10-100kb",
                            ifelse (lenght>100000 & lenght<1000000,"100-1000kb",
                            ifelse (lenght>1000000, ">1000kb", NA)))) %>% 
  rename (lenght_ibd=lenght)
ibd_tract$lenght_category <- factor(ibd_tract$lenght_category, levels=c("10-100kb", "100-1000kb", ">1000kb"))
ibd_tract2 <- ibd_tract %>% filter (!is.na(ibd_tract$lenght_category)) %>% 
  mutate (pop = substring(ind,1,7)) %>% 
  mutate (sps = substring(ind,1,4))
  
# ibd_tract3 <- ibd_tract2 %>% filter (., ind=="c_ll_ki_0090" | ind=="c_lp_sm_0450") 
ibd_tract_cumsumlenght <- ibd_tract2 %>% dplyr::group_by(ind,lenght_category, pop,sps) %>% 
                                         dplyr::summarise(
                                                sum_lenght = sum(lenght_ibd),
                                                n = n())
                     
 ggplot (ibd_tract_cumsumlenght %>% filter (lenght_category==">1000kb"), aes(x=ind, y=sum_lenght, colour=pop, fill=pop)) +
  geom_col() +
  facet_grid(~sps, scales="free_x") +
   theme (axis.text.x = element_text(angle = 90, vjust=0)) +
   ggsave ()
 ggplot (ibd_tract_cumsumlenght %>% filter (lenght_category==">1000kb"), aes(x=sum_lenght, y=n)) +
   geom_point(aes(colour=pop, fill=pop))+
   geom_text_repel(aes(label=ind, colour=pop)) +
