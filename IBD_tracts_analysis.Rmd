...
ibd_tract_cumsumlength_final <- full_join(ibd_tract_cumsumlength,missing_ind)
ibd_tract_cumsumlength_final$Populations <- factor(ibd_tract_cumsumlength_final$Populations,levels=sort(unique(ibd_tract_cumsumlength_final$Populations)))

ibd_tract_cumsumlength_final <- arrange(ibd_tract_cumsumlength_final,Populations,ind)

ibd_tract_cumsumlength_final$ind <- factor(ibd_tract_cumsumlength_final$ind,levels=arrange(ibd_tract_cumsumlength_final,Populations)[,1] %>% unlist(use.names=F) %>% unique())
...
ggplot(ibd_tract_cumsumlength_final %>% filter(length_category==">5000kb"), aes(x=ind, y=sum_length, colour=Populations, fill=Populations)) +
