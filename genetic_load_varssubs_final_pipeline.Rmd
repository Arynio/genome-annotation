...
cd /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered
screen -S c_lp_do_nm3_BAMs.log
script c_lp_do_nm3_BAMs.log
...
for i in ${SAMPLES[@]}
  do
  echo "${i}"
  bamtools filter -tag "NM:<=3" -in c_lp_do_"${i}"_recal_round-1.bam -out BAM_nm_filtered/c_lp_do_"${i}"_recal_round-1.nm3.bam
  samtools index BAM_nm_filtered/c_lp_do_"${i}"_recal_round-1.nm3.bam
  done
...
cd /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered
screen -S c_lp_do_nm3_BAMs_25x.log
script c_lp_do_nm3_BAMs_25x.log
...
for i in ${SAMPLES[@]}
  do
  echo "${i}"
  bamtools filter -tag "NM:<=3" -in c_lp_do_"${i}"_recal_round-1_25x.bam -out ./../BAM_nm_filtered/c_lp_do_"${i}"_recal_round-1_25x.nm3.bam
  samtools index ./../BAM_nm_filtered/c_lp_do_"${i}"_recal_round-1_25x.nm3.bam
  done
...
cd /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered
screen -S c_lp_sm_nm3_BAMs.log
script c_lp_sm_nm3_BAMs.log
...
for i in ${SAMPLES[@]}
  do
  echo "${i}"
  bamtools filter -tag "NM:<=3" -in c_lp_sm_"${i}"_recal_round-1.bam -out BAM_nm_filtered/c_lp_sm_"${i}"_recal_round-1.nm3.bam
  samtools index BAM_nm_filtered/c_lp_sm_"${i}"_recal_round-1.nm3.bam
  done
...
cd /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/BAM_nm_filtered
screen -S c_lp_sm_nm3_BAMs_25x.log
script c_lp_sm_nm3_BAMs_25x.log
...
for i in ${SAMPLES[@]}
  do
  echo "${i}"
  bamtools filter -tag "NM:<=3" -in c_lp_sm_"${i}"_recal_round-1_25x.bam -out ./../BAM_nm_filtered/c_lp_sm_"${i}"_recal_round-1_25x.nm3.bam
  samtools index ./../BAM_nm_filtered/c_lp_sm_"${i}"_recal_round-1_25x.nm3.bam
  done
...
cd /GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final
declare SAMPLES=$(ls c_ll_ki_*_recal_round-1.bam | cut -c9-12 | sort | uniq | head -n1)
for i in ${SAMPLES[@]}
  do
  echo "${i}"
...
bgzip -c /GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa > /GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz

#Next they should be fai indexed:
/opt/samtools-1.6/samtools faidx /GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz
...
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nmNnmN_origcov)
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING
screen -S "${CALLING}-polarized.log"
CALLING=$(echo ${STY#*.} | cut -d'-' -f1)
script "${CALLING}-polarized.log"
CALLING=$(echo ${STY#*.} | cut -d'-' -f1)
...
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nmNnmN_origcov)
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING
screen -S "${CALLING}-polarized.lr_ann"
CALLING=$(echo ${STY#*.} | cut -d'-' -f1)
script "${CALLING}-polarized.lr_ann.log"
CALLING=$(echo ${STY#*.} | cut -d'-' -f1)
...
O_PATH=/home/dkleinman/datos/snpEff #output path
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path
mkdir -p $V_PATH/$CALLING/annotation/
...
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov) #write down name of the calling
...
if [ $TYPE != "SNP" ] && [ $TYPE != "INDEL" ] #Don't apply filters if $TYPE is invalid
  ...
  if [ -f ${CALLING}"_polarized_filtered1.lr_ann.vcf" ] #Since filter 1 is common for both types of variants, skip it if it's been performed before
    ...
    bedtools subtract -a ${CALLING}"_polarized.lr_ann.vcf" -b /home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/repeats_and_lowcomp_no_redundant_mappability.k75_lessthan90.bed -header | uniq > ${CALLING}"_polarized_filtered1.lr_ann.vcf"
...
#This should be run after reaching section 11 with the other definition of variants and substitutions.
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm3nm3_origcov) #write down name of the calling
TYPE=(SNP) #write down SNP or INDEL
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation
screen -S "${CALLING}-${TYPE}"
CALLING=${STY#*.}
...
if [[ $CALLING == "c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm3nm3"* ]] #the double brackets turns the test into a regex
  then
  for s in ${SPECIES[@]}
    do
...
##For varssubs, variants and substitutions. All of these should be filtered next (steps 10 and 11).
...
##For segregating and fixed (true variants and substitutions). These come from the already filtered varssubs, so steps 10 and 11 should be skipped. Go straight to step 12 after this.
```{r Obtain per dataset VCFs, eval=FALSE, engine='bash'}
...
#10: Depth range calculus. Obtain depth range for each species in order to filter high depth positions as part of the next section's many filterings.
...
#11: Perform high-depth filtering. Obtain list of sites with very high depth within each dataset, join them, and remove those sites.
...
#This code will only work if the whole-genome individual VCFs have been generated before.
CALLING=(c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov)
VAR=(fixed) #varssubs #variants #substitutions #fixed #segregating
TYPE=(SNP) #write down SNP or INDEL
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/$CALLING/annotation
screen -S "${CALLING}-${VAR}-${TYPE}"
CALLING=$(echo ${STY#*.} | cut -d'-' -f1)
...
#Obsolete16 (go to statistics script): Get annotation statistics.
...
#Obsolete17 (go to statistics script): Plot variant count results.
