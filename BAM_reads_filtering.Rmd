...
cd $V_PATH
screen -S c_lp_sm_c_lp_do_nm4.log
script c_lp_sm_c_lp_do_nm4.log

cd $B_PATH
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -jar $GATK \
-T HaplotypeCaller \
-R $REF \
$(for var in c_lp*recal_round-1.nm4.bam; do echo -I ${var}" ";done) \
-o $V_PATH/c_lp_sm_c_lp_do_nm4.vcf

grep -v '#' $V_PATH/c_lp_sm_c_lp_do_nm4.vcf | wc -l #4189966

#Rename the samples with wrong names.
cd $V_PATH
bcftools query -l c_lp_sm_c_lp_do_nm4.vcf #check sample names
#!/bin/bash
...
EOF
cat lp_rename.txt
bcftools reheader -s lp_rename.txt -o c_lp_sm_c_lp_do_nm4_renamed.vcf c_lp_sm_c_lp_do_nm4.vcf #copy the VCF and rename the wrong named samples in the new VCF
rm lp_rename.txt
mv c_lp_sm_c_lp_do_nm4_renamed.vcf c_lp_sm_c_lp_do_nm4.vcf

grep -v '#' c_lp_sm_c_lp_do_nm4.vcf | wc -l #4189966
...
cd $V_PATH
screen -S c_ll_ki_c_ll_no_c_ll_po_nm4.log
script c_ll_ki_c_ll_no_c_ll_po_nm4.log

cd $B_PATH
java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -jar $GATK \
-T HaplotypeCaller \
-R $REF \
$(for var in c_{ll_ki*,ll_no*,ll_po*}recal_round-1.nm4.bam; do echo -I ${var}" ";done) \
-o $V_PATH/c_ll_ki_c_ll_no_c_ll_po_nm4.vcf

grep -v '#' $V_PATH/c_ll_ki_c_ll_no_c_ll_po_nm4.vcf | wc -l #10866667

#Rename the samples with wrong names.
cd $V_PATH
bcftools query -l c_ll_ki_c_ll_no_c_ll_po_nm4.vcf #check sample names
#!/bin/bash
...
EOF
cat ll_rename.txt
bcftools reheader -s ll_rename.txt -o c_ll_ki_c_ll_no_c_ll_po_nm4_renamed.vcf c_ll_ki_c_ll_no_c_ll_po_nm4.vcf #copy the VCF and rename the wrong named samples in the new VCF
rm ll_rename.txt
mv c_ll_ki_c_ll_no_c_ll_po_nm4_renamed.vcf c_ll_ki_c_ll_no_c_ll_po_nm4.vcf

grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm4.vcf | wc -l #10866667
...
cd $V_PATH
screen -S c_lp_sm_c_lp_do_nm4_aafilled.log
script c_lp_sm_c_lp_do_nm4_aafilled.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path

cat $V_PATH/c_lp_sm_c_lp_do_nm4.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz | bgzip -c > c_lp_sm_c_lp_do_nm4_aafilled.vcf.gz #I tried to run this first without bgzipping it, and it was increasingly slow. Each day it did around half the previous day. When the server crashed, I relaunched it adding the bgzip command and it finished in a little over 24h.

gzip -d -c c_lp_sm_c_lp_do_nm4_aafilled.vcf.gz > c_lp_sm_c_lp_do_nm4_aafilled.vcf #Get decompressed version. I ran md5sum for both the gzipped and the unzipped versions and there were no differences.

grep -v '#' c_lp_sm_c_lp_do_nm4_aafilled.vcf | wc -l #
...
#Code to annotate the AA (ancestral alleles) subfield from the INFO field in the VCF. I keep getting the following error: "Can't locate Vcf.pm in @INC". I look it up and apparently it's easy to solve: an environment variable PERL5LIB should be defined as the path to perl.
cd $V_PATH
screen -S c_ll_ki_c_ll_no_c_ll_po_nm4_aafilled.log
script c_ll_ki_c_ll_no_c_ll_po_nm4_aafilled.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path

cat $V_PATH/c_ll_ki_c_ll_no_c_ll_po_nm4.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz | bgzip -c > c_ll_ki_c_ll_no_c_ll_po_nm4_aafilled.vcf.gz #I tried to run this first without bgzipping it, and it was increasingly slow. Each day it did around half the previous day. When the server crashed, I relaunched it adding the bgzip command and it finished in a little over 24h.

gzip -d -c c_ll_ki_c_ll_no_c_ll_po_nm4_aafilled.vcf.gz > c_ll_ki_c_ll_no_c_ll_po_nm4_aafilled.vcf #Get decompressed version. I ran md5sum for both the gzipped and the unzipped versions and there were no differences.

grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm4_aafilled.vcf | wc -l #
...
cd $V_PATH
screen -S c_lp_sm_c_lp_do_nm2_genes_polarized.log
script c_lp_sm_c_lp_do_nm2_genes_polarized.log
...
return vb.make();' -o c_lp_sm_c_lp_do_nm2_genes_polarized.vcf c_lp_sm_c_lp_do_nm2_genes_aafilled.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_genes_polarized.vcf | wc -l #1341848
...
screen -S c_lp_sm_c_lp_do_nm2_genes_polarized.lr_ann.log #open a dettachable screen in case the test takes too long
script c_lp_sm_c_lp_do_nm2_genes_polarized.lr_ann.log #initiate the log file
...
java -Xmx16g -jar $S_PATH/snpEff.jar LYRU.23 -v -s $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_genes_polarized.lr_ann.html -csvStats $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_genes_polarized.lr_ann.csv -interval $C_PATH/data/LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCNE.intergenic.nr.bed $V_PATH/c_lp_sm_c_lp_do_nm2_genes_polarized.vcf > $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_genes_polarized.lr_ann.vcf #run this code from the directory where the config is located.

cd $V_PATH/annotation/
grep -v '#' c_lp_sm_c_lp_do_nm2_genes_polarized.lr_ann.vcf | wc -l #1341848
...
cd $V_PATH/annotation
screen -S c_lp_sm_c_lp_do_nm2_genes_polarized_filtered.lr_ann.log
script c_lp_sm_c_lp_do_nm2_genes_polarized_filtered.lr_ann.log
...
bedtools subtract -a c_lp_sm_c_lp_do_nm2_genes_polarized.lr_ann.vcf -b /home/GRUPOS/grupolince/reference_genomes/lynx_pardinus_genome/repeats_and_lowcomp_no_redundant_mappability.k75_lessthan90.bed -header > c_lp_sm_c_lp_do_nm2_genes_polarized_filtered1.lr_ann.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_genes_polarized_filtered1.lr_ann.vcf | wc -l #645623
...
-R $REF \
-V c_lp_sm_c_lp_do_nm2_genes_polarized_filtered1.lr_ann.vcf \
-o c_lp_sm_c_lp_do_nm2_genes_polarized_filtered2.lr_ann.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_genes_polarized_filtered2.lr_ann.vcf | wc -l #497025


#Remove polarized ANC fixed variants (AF=0) or those that weren't polarizable (AA different from either REF or ALT):
$BCF view -e '(INFO/AA!=REF & INFO/AA!=ALT) | INFO/AF=0.00' c_lp_sm_c_lp_do_nm2_genes_polarized_filtered2.lr_ann.vcf > c_lp_sm_c_lp_do_nm2_genes_polarized_filtered3.lr_ann.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_genes_polarized_filtered3.lr_ann.vcf | wc -l #475783
...
-R $REF \
-V c_lp_sm_c_lp_do_nm2_genes_polarized_filtered3.lr_ann.vcf \
-o c_lp_sm_c_lp_do_nm2_genes_polarized_filtered4.lr_ann.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_genes_polarized_filtered4.lr_ann.vcf | wc -l #446094
...
echo $MIN_DP
echo $MAX_DP
$BCF filter -e "DP < ${MIN_DP} || DP > ${MAX_DP} || F_MISSING > 0.2" -Ov -o c_lp_sm_c_lp_do_nm2_genes_polarized_filtered5.lr_ann.vcf c_lp_sm_c_lp_do_nm2_genes_polarized_filtered4.lr_ann.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_genes_polarized_filtered5.lr_ann.vcf | wc -l #402692
...
cd $V_PATH/annotation/
screen -S perpop_nm2_genes.lr_ann.log
script perpop_nm2_genes.lr_ann.log
