...
grep -v '#' $V_PATH/c_lp_sm_c_lp_do_nm2_samecov.vcf | wc -l #
...
grep -v '#' c_lp_sm_c_lp_do_nm2_samecov.vcf | wc -l #
...
grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm3_samecov.vcf | wc -l #

```

###For Lynx lynxs, all samples at their original cov (~6-25x).
...
###For both together, all samples at the same cov (~6x).
...
-o $V_PATH/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov.vcf

grep -v '#' $V_PATH/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov.vcf | wc -l #
...
cd $V_PATH
screen -S c_lp_sm_c_lp_do_nm2_samecov_aafilled.log
script c_lp_sm_c_lp_do_nm2_samecov_aafilled.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path

cat $V_PATH/c_lp_sm_c_lp_do_nm2_samecov.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz | bgzip -c > c_lp_sm_c_lp_do_nm2_samecov_aafilled.vcf.gz #I tried to run this first without bgzipping it, and it was increasingly slow. Each day it did around half the previous day. When the server crashed, I relaunched it adding the bgzip command and it finished in a little over 24h.

gzip -d -c c_lp_sm_c_lp_do_nm2_samecov_aafilled.vcf.gz > c_lp_sm_c_lp_do_nm2_samecov_aafilled.vcf #Get decompressed version. I ran md5sum for both the gzipped and the unzipped versions and there were no differences.

grep -v '#' c_lp_sm_c_lp_do_nm2_samecov_aafilled.vcf | wc -l #
...
#Code to annotate the AA (ancestral alleles) subfield from the INFO field in the VCF. I keep getting the following error: "Can't locate Vcf.pm in @INC". I look it up and apparently it's easy to solve: an environment variable PERL5LIB should be defined as the path to perl.
cd $V_PATH
screen -S c_lp_sm_c_lp_do_nm2_origcov_aafilled.log
script c_lp_sm_c_lp_do_nm2_origcov_aafilled.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path

cat $V_PATH/c_lp_sm_c_lp_do_nm2_origcov.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz | bgzip -c > c_lp_sm_c_lp_do_nm2_origcov_aafilled.vcf.gz #I tried to run this first without bgzipping it, and it was increasingly slow. Each day it did around half the previous day. When the server crashed, I relaunched it adding the bgzip command and it finished in a little over 24h.

gzip -d -c c_lp_sm_c_lp_do_nm2_origcov_aafilled.vcf.gz > c_lp_sm_c_lp_do_nm2_origcov_aafilled.vcf #Get decompressed version. I ran md5sum for both the gzipped and the unzipped versions and there were no differences.

grep -v '#' c_lp_sm_c_lp_do_nm2_origcov_aafilled.vcf | wc -l #
...
#Code to annotate the AA (ancestral alleles) subfield from the INFO field in the VCF. I keep getting the following error: "Can't locate Vcf.pm in @INC". I look it up and apparently it's easy to solve: an environment variable PERL5LIB should be defined as the path to perl.
cd $V_PATH
screen -S c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_aafilled.log
script c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_aafilled.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path

cat $V_PATH/c_ll_ki_c_ll_no_c_ll_po_nm3_samecov.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz | bgzip -c > c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_aafilled.vcf.gz #I tried to run this first without bgzipping it, and it was increasingly slow. Each day it did around half the previous day. When the server crashed, I relaunched it adding the bgzip command and it finished in a little over 24h.

gzip -d -c c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_aafilled.vcf.gz > c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_aafilled.vcf #Get decompressed version. I ran md5sum for both the gzipped and the unzipped versions and there were no differences.

grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_aafilled.vcf | wc -l #
...
#Code to annotate the AA (ancestral alleles) subfield from the INFO field in the VCF. I keep getting the following error: "Can't locate Vcf.pm in @INC". I look it up and apparently it's easy to solve: an environment variable PERL5LIB should be defined as the path to perl.
cd $V_PATH
screen -S c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_aafilled.log
script c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_aafilled.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path

cat $V_PATH/c_ll_ki_c_ll_no_c_ll_po_nm3_origcov.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz | bgzip -c > c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_aafilled.vcf.gz #I tried to run this first without bgzipping it, and it was increasingly slow. Each day it did around half the previous day. When the server crashed, I relaunched it adding the bgzip command and it finished in a little over 24h.

gzip -d -c c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_aafilled.vcf.gz > c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_aafilled.vcf #Get decompressed version. I ran md5sum for both the gzipped and the unzipped versions and there were no differences.

grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_aafilled.vcf | wc -l #
...
#Code to annotate the AA (ancestral alleles) subfield from the INFO field in the VCF. I keep getting the following error: "Can't locate Vcf.pm in @INC". I look it up and apparently it's easy to solve: an environment variable PERL5LIB should be defined as the path to perl.
cd $V_PATH
screen -S c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_aafilled.log
script c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_aafilled.log

export PERL5LIB=/opt/vcftools_0.1.13/perl/ #set required environmental variable
V_PATH=/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs #VCFs path

cat $V_PATH/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov.vcf | /opt/vcftools_0.1.13/perl/fill-aa -a /home/GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa.gz | bgzip -c > c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_aafilled.vcf.gz #I tried to run this first without bgzipping it, and it was increasingly slow. Each day it did around half the previous day. When the server crashed, I relaunched it adding the bgzip command and it finished in a little over 24h.

gzip -d -c c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_aafilled.vcf.gz > c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_aafilled.vcf #Get decompressed version. I ran md5sum for both the gzipped and the unzipped versions and there were no differences.

grep -v '#' c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_aafilled.vcf | wc -l #
...
cd $V_PATH
screen -S c_lp_sm_c_lp_do_nm2_samecov_polarized.log
script c_lp_sm_c_lp_do_nm2_samecov_polarized.log
...
return vb.make();' -o c_lp_sm_c_lp_do_nm2_samecov_polarized.vcf c_lp_sm_c_lp_do_nm2_samecov_aafilled.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_samecov_polarized.vcf | wc -l #
...
cd $V_PATH
screen -S c_lp_sm_c_lp_do_nm2_origcov_polarized.log
script c_lp_sm_c_lp_do_nm2_origcov_polarized.log
...
vb.attribute("AF",1.0d - Double.parseDouble(variant.getAttributeAsString("AF",""))); vb.attribute("AC",variant.getGenotypes().stream().flatMap(G->G.getAlleles().stream()).filter(A->A.equals(oldref)).count()); 
vb.genotypes(genotypes); 
return vb.make();' -o c_lp_sm_c_lp_do_nm2_origcov_polarized.vcf c_lp_sm_c_lp_do_nm2_origcov_aafilled.vcf

grep -v '#' c_lp_sm_c_lp_do_nm2_origcov_polarized.vcf | wc -l #
...
cd $V_PATH
screen -S c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.log
script c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.log
...
vb.attribute("AF",1.0d - Double.parseDouble(variant.getAttributeAsString("AF",""))); vb.attribute("AC",variant.getGenotypes().stream().flatMap(G->G.getAlleles().stream()).filter(A->A.equals(oldref)).count()); 
vb.genotypes(genotypes); 
return vb.make();' -o c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.vcf c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_aafilled.vcf

grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.vcf | wc -l #
...
cd $V_PATH
screen -S c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.log
script c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.log
...
vb.attribute("AF",1.0d - Double.parseDouble(variant.getAttributeAsString("AF",""))); vb.attribute("AC",variant.getGenotypes().stream().flatMap(G->G.getAlleles().stream()).filter(A->A.equals(oldref)).count()); 
vb.genotypes(genotypes); 
return vb.make();' -o c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.vcf c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_aafilled.vcf

grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.vcf | wc -l #
...
cd $V_PATH
screen -S c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.log
script c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.log
...
vb.attribute("AF",1.0d - Double.parseDouble(variant.getAttributeAsString("AF",""))); vb.attribute("AC",variant.getGenotypes().stream().flatMap(G->G.getAlleles().stream()).filter(A->A.equals(oldref)).count()); 
vb.genotypes(genotypes); 
return vb.make();' -o c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.vcf c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_aafilled.vcf

grep -v '#' c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.vcf | wc -l #
...
###For Lynx pardinus, all samples at the same cov (~6x). Monomorphic positions (incl. substitutions) are NOT polarized. Grab substitutions from the joint dataset.

```{r Annotate using SnpEff, eval=FALSE, engine='bash'}

cd $V_PATH/annotation/
screen -S c_lp_sm_c_lp_do_nm2_samecov_polarized.lr_ann.log #open a dettachable screen in case the test takes too long
script c_lp_sm_c_lp_do_nm2_samecov_polarized.lr_ann.log #initiate the log file
...
java -Xmx16g -jar $S_PATH/snpEff.jar LYRU.23 -v -s $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_samecov_polarized.lr_ann.html -csvStats $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_samecov_polarized.lr_ann.csv -interval $C_PATH/data/LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCNE.intergenic.nr.bed $V_PATH/c_lp_sm_c_lp_do_nm2_samecov_polarized.vcf > $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_samecov_polarized.lr_ann.vcf #run this code from the directory where the config is located.

cd $V_PATH/annotation/
grep -v '#' c_lp_sm_c_lp_do_nm2_samecov_polarized.lr_ann.vcf | wc -l #

```

###For Lynx pardinus, all samples at their original cov (~6-25x). Monomorphic positions (incl. substitutions) are NOT polarized. Grab substitutions from the joint dataset.

```{r Annotate using SnpEff, eval=FALSE, engine='bash'}

cd $V_PATH/annotation/
screen -S c_lp_sm_c_lp_do_nm2_origcov_polarized.lr_ann.log #open a dettachable screen in case the test takes too long
script c_lp_sm_c_lp_do_nm2_origcov_polarized.lr_ann.log #initiate the log file
...
cd $O_PATH #run this code from the directory where the config is located.
java -Xmx16g -jar $S_PATH/snpEff.jar LYRU.23 -v -s $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_origcov_polarized.lr_ann.html -csvStats $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_origcov_polarized.lr_ann.csv -interval $C_PATH/data/LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCNE.intergenic.nr.bed $V_PATH/c_lp_sm_c_lp_do_nm2_origcov_polarized.vcf > $V_PATH/annotation/c_lp_sm_c_lp_do_nm2_origcov_polarized.lr_ann.vcf #run this code from the directory where the config is located.

cd $V_PATH/annotation/
grep -v '#' c_lp_sm_c_lp_do_nm2_origcov_polarized.lr_ann.vcf | wc -l #

```

###For Lynx lynx, all samples at the same cov (~6x). Monomorphic positions (incl. substitutions) are NOT polarized. Grab substitutions from the joint dataset.

```{r Annotate using SnpEff, eval=FALSE, engine='bash'}

cd $V_PATH/annotation/
screen -S c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.lr_ann.log #open a dettachable screen in case the test takes too long
script c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.lr_ann.log #initiate the log file
...
cd $O_PATH #run this code from the directory where the config is located.
java -Xmx16g -jar $S_PATH/snpEff.jar LYRU.23 -v -s $V_PATH/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.lr_ann.html -csvStats $V_PATH/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.lr_ann.csv -interval $C_PATH/data/LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCNE.intergenic.nr.bed $V_PATH/c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.vcf > $V_PATH/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.lr_ann.vcf #run this code from the directory where the config is located.

cd $V_PATH/annotation/
grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm3_samecov_polarized.lr_ann.vcf | wc -l #

```

###For Lynx lynx, all samples at their original cov (~6-25x). Monomorphic positions (incl. substitutions) are NOT polarized. Grab substitutions from the joint dataset.

```{r Annotate using SnpEff, eval=FALSE, engine='bash'}

cd $V_PATH/annotation/
screen -S c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.lr_ann.log #open a dettachable screen in case the test takes too long
script c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.lr_ann.log #initiate the log file
...
cd $O_PATH #run this code from the directory where the config is located.
java -Xmx16g -jar $S_PATH/snpEff.jar LYRU.23 -v -s $V_PATH/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.lr_ann.html -csvStats $V_PATH/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.lr_ann.csv -interval $C_PATH/data/LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCNE.intergenic.nr.bed $V_PATH/c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.vcf > $V_PATH/annotation/c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.lr_ann.vcf #run this code from the directory where the config is located.

cd $V_PATH/annotation/
grep -v '#' c_ll_ki_c_ll_no_c_ll_po_nm3_origcov_polarized.lr_ann.vcf | wc -l #

```

###For both together, all samples at the same cov (~6x). Substitutions should be retrieved from this joint dataset.

```{r Annotate using SnpEff, eval=FALSE, engine='bash'}

cd $V_PATH/annotation/
screen -S c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.lr_ann.log #open a dettachable screen in case the test takes too long
script c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.lr_ann.log #initiate the log file
...
cd $O_PATH #run this code from the directory where the config is located.
java -Xmx16g -jar $S_PATH/snpEff.jar LYRU.23 -v -s $V_PATH/annotation/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.lr_ann.html -csvStats $V_PATH/annotation/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.lr_ann.csv -interval $C_PATH/data/LYPA23C.CDS.GENE_promoters.GENE_introns.UTRs.ncRNA.lncRNA.lncRNA_introns.lncRNA_promoters.UCNE.intergenic.nr.bed $V_PATH/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.vcf > $V_PATH/annotation/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.lr_ann.vcf #run this code from the directory where the config is located.

cd $V_PATH/annotation/
grep -v '#' c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_samecov_polarized.lr_ann.vcf | wc -l #
...
POP=(c_lp_do-c_lp_sm_nm2_samecov_n031)
...
cd /home/dkleinman/datos/nm_depth_calculus
POP=(c_lp_do-c_lp_sm_nm2_origcov_n031)
screen -S "${POP}_depth_calculus.log"
POP=(c_lp_do-c_lp_sm_nm2_origcov_n031)
script "${POP}_depth_calculus.log"
...
cd /home/dkleinman/datos/nm_depth_calculus
POP=(c_ll_no-c_ll_po-c_ll_ki_nm3_samecov_n029)
screen -S "${POP}_depth_calculus.log"
POP=(c_ll_no-c_ll_po-c_ll_ki_nm3_samecov_n029)
script "${POP}_depth_calculus.log"
...
cd /home/dkleinman/datos/nm_depth_calculus
POP=(c_ll_no-c_ll_po-c_ll_ki_nm3_origcov_n029)
screen -S "${POP}_depth_calculus.log"
POP=(c_ll_no-c_ll_po-c_ll_ki_nm3_origcov_n029)
script "${POP}_depth_calculus.log"
...
CALLING=(c_lp_sm_c_lp_do_nm2_samecov)
cd $B_PATH/BAM_genes_5x
declare SPECIES=$(ls {*_lp_*,*_ll_*}_recal_round-1.genes-nm2.bam | cut -c3-4 | sort | uniq)
cd $V_PATH/annotation/
for i in ${SPECIES[@]}
  do
  echo "${i}"
  cd $B_PATH/BAM_genes_5x
  declare POP=$(ls c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}_recal_round-1.genes-nm2.bam | cut -c1-7 | uniq | grep "${i}")
  cd $V_PATH/annotation/
  for j in ${POP[@]}
    do
    echo "${j}"
    rm "${j}"_pop_list_to_remove.txt
    $BCF query -l $V_PATH/annotation/c_"${i}"*_nm2_genes_polarized_filtered5.lr_ann.vcf | grep "${j}" > "${j}"_pop_list_to_remove.txt
    cat "${j}"_pop_list_to_remove.txt
    mkdir "${j}"_perpop_nm2_genes
    java -XX:MaxMetaspaceSize=1g -XX:+UseG1GC -XX:+UseStringDeduplication -Xms16g -Xmx32g -jar $GATK \
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/c_"${i}"*_nm2_genes_polarized_filtered5.lr_ann.vcf \
    -o $V_PATH/annotation/"${j}"_perpop_nm2_genes/"${j}"_perpop_nm2_genes.lr_ann.vcf \
    -env \
    --sample_file "${j}"_pop_list_to_remove.txt
    rm "${j}"_pop_list_to_remove.txt
    grep -v '#' $V_PATH/annotation/"${j}"_perpop_nm2_genes/"${j}"_perpop_nm2_genes.lr_ann.vcf | wc -l #
    done
  done
...
cd $V_PATH/annotation
screen -S individuals_genes.lr_ann.log
script individuals_genes.lr_ann.log
...
cd $B_PATH/BAM_genes_5x
declare POP=$(ls c_{lp_sm*,lp_do*,ll_ki*,ll_po*,ll_no*}_recal_round-1.genes.bam | cut -c1-7 | uniq | grep "${i}")
for i in ${POP[@]}
  do
  echo "${i}"
  mkdir $V_PATH/annotation/"${i}"_individuals_genes
  declare INDIVIDUALS=$(ls "${i}"*_recal_round-1.genes.bam | cut -c1-12 | uniq)
  for j in ${INDIVIDUALS[@]}
    ...
    -T SelectVariants \
    -R $REF \
    -V $V_PATH/annotation/"${i}"_perpop_genes/"${i}"_perpop_genes.lr_ann.vcf \
    -o $V_PATH/annotation/"${i}"_individuals_genes/"${j}"_individual_genes.lr_ann.vcf \
    -env \
    -sn $ID
    done
  done
...
cd $V_PATH/annotation/
screen -S snpeff_individual_summary_nm4_genes.lr_ann.log
script snpeff_individual_summary_nm4_genes.lr_ann.log

rm snpeff_individual_summary_nm4_genes.lr_ann.txt
echo -e "species\tpopulation\tdataset\tsample\ttotal_V\tintergenic_V\tintronic_V\tcoding_V\tsynonymous_V\tsynonymous_A\tmissense_V\tmissense_A\tnonsense_V\tnonsense_A\tUCNE_V\tmissense/synonymous_V\tmissense/synonymous_A\tsynonymous/intronic_V\tmissense/intronic_V" > snpeff_individual_summary_nm4_genes.lr_ann.txt
INDLIST=($(ls `find . -name '*individual_nm4_genes.lr_ann.vcf' -print`))
for i in "${INDLIST[@]}"
  do
  echo "${i}"
  ...
  DATASET=$(if grep -Fxq $ind /home/GRUPOS/grupolince/lynx_genomes_5x/BAM_files_final/c_lp_GP_samples; then echo "GP"; else echo "5x"; fi)
  ...
  echo -e "$SPECIES\t$POPULATION\t$DATASET\t$SAMPLE\t$TOTAL_V\t$INTERGENIC_V\t$INTRONIC_V\t$CODING_V\t$SYNONYMOUS_V\t$SYNONYMOUS_A\t$MISSENSE_V\t$MISSENSE_A\t$NONSENSE_V\t$NONSENSE_A\t$UCNE_V\t$MISSENSE_SYNONYMOUS_V\t$MISSENSE_SYNONYMOUS_A\t$SYNONYMOUS_INTRONIC_V\t$MISSENSE_INTRONIC_V" >> snpeff_individual_summary_nm4_genes.lr_ann.txt
  done

scp dkleinman@genomics-b.ebd.csic.es:/GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/annotation/snpeff_individual_summary_*genes.lr_ann.txt /Users/Dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/sep_calling

```

#13: Compare NM filtering results.
...
#First draw the NM distribution for each individual:
sample_files <- list.files("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/sep_calling/", pattern="*.genes.lr_ann.txt")
snpeff_filters_summary <- data_frame()
for (file in sample_files) {
  snpeff_individual_summary <- read_tsv(paste0("/Users/dani/ownCloud/backup/g-w_analysis/genetic_load/snpeff_summary_ratios/sep_calling/",file))
