...
bedtools getfasta -fi /GRUPOS/grupolince/reference_genomes/lynx_rufus_genome/c_lr_zz_0001_recal1.fa -bed synonymous_variants_cds_list.gff3 -fo synonymous_variants_cds_sequence.fa

#Paste each CDS' sequence with the rest of the information in the gff.
paste synonymous_variants_cds_list.gff3 <(grep -v '>' synonymous_variants_cds_sequence.fa) > synonymous_variants_cds_list_and_sequence.gff3
...
awk '/partial_gene/ {printf ("%s\t%s\n", $0,NR)}' synonymous_variants_cds_list_and_sequence.gff3 > synonymous_variants_cds_list_and_sequence_partialgenes.gff3
while read -r row; do
  ...
  sed -i "${ROW_N}s/$OLD_SEQUENCE/$CODING_SEQUENCE/" synonymous_variants_cds_list_and_sequence.gff3
done < synonymous_variants_cds_list_and_sequence_partialgenes.gff3  

#Fuse all exons from each gene and store them in a file together with the gene name and the strand information.
GENES=$(cat synonymous_variants_cds_list_and_sequence.gff3 | cut -f 10 | uniq)
COUNTER=0
rm synonymous_variants_cds_list_and_sequence_combined.txt
for gen in ${GENES[@]}
  do
  #echo "${gen}"
  STRAND=$(awk -F"\t" -v gen=$gen '$10 == gen' synonymous_variants_cds_list_and_sequence.gff3 | shuf -n1 | cut -f 7)
  CODING_SEQUENCE=$(awk -F"\t" -v gen=$gen '$10 == gen {print $11}' synonymous_variants_cds_list_and_sequence.gff3 | tr -d '\n')
  echo -e "$gen\t$STRAND\t$CODING_SEQUENCE" >> synonymous_variants_cds_list_and_sequence_combined.txt
  ((COUNTER++))
  if [ $(( $COUNTER % 10 )) == 0 ]
    then
    echo "processed $COUNTER genes out of $(echo "$GENES" | wc -l)"
...
sort synonymous_variants_cds_list_and_sequence_combined.txt -k1,1 | uniq > synonymous_variants_cds_list_and_sequence_combined_sorted.txt
mv synonymous_variants_cds_list_and_sequence_combined_sorted.txt synonymous_variants_cds_list_and_sequence_combined.txt
...
cd /GRUPOS/grupolince/lynx_genomes_5x/VCFs_Dani/nmVCFs/c_lp_sm_c_lp_do_c_ll_ki_c_ll_no_c_ll_po_nm2nm3_origcov/annotation/syn_pref
cat synonymous_variants_cds_list_and_sequence_combined.txt > all_genes_sequence_combined.txt
cat ./../provean/missense_variants_cds_list_and_sequence_combined.txt >> all_genes_sequence_combined.txt
sort all_genes_sequence_combined.txt | uniq > all_genes_sequence_combined_bis.txt
mv all_genes_sequence_combined_bis.txt all_genes_sequence_combined.txt

#Retrieve all codons from each gene:
rm all_genes_sequence_combined_codons.txt
TOTAL=$(cat all_genes_sequence_combined.txt | wc -l)
COUNTER=0
while read -r entry; do
  ...
  if [ $STRAND == "+" ]
    then
    echo $CODING_SEQUENCE | sed 's/.../&\n/g' >> all_genes_sequence_combined_codons.txt
  elif [ $STRAND == "-" ]
    then
  ...
  done < all_genes_sequence_combined.txt

#Count the occurrences of each codon:
sort all_genes_sequence_combined_codons.txt | uniq -c > all_genes_sequence_combined_codon_counts.txt

#Store the genetic code in a file:
echo -e "TTT\tF\nTTC\tF\nTTA\tL\nTTG\tL\nCTT\tL\nCTC\tL\nCTA\tL\nCTG\tL\nATT\tI\nATC\tI\nATA\tI\nATG\tM\nGTT\tV\nGTC\tV\nGTA\tV\nGTG\tV\nTCT\tS\nTCC\tS\nTCA\tS\nTCG\tS\nCCT\tP\nCCC\tP\nCCA\tP\nCCG\tP\nACT\tT\nACC\tT\nACA\tT\nACG\tT\nGCT\tA\nGCC\tA\nGCA\tA\nGCG\tA\nTAT\tY\nTAC\tY\nTAA\t-\nTAG\t-\nCAT\tH\nCAC\tH\nCAA\tQ\nCAG\tQ\nAAT\tN\nAAC\tN\nAAA\tK\nAAG\tK\nGAT\tD\nGAC\tD\nGAA\tE\nGAG\tE\nTGT\tC\nTGC\tC\nTGA\t-\nTGG\tW\nCGT\tR\nCGC\tR\nCGA\tR\nCGG\tR\nAGT\tS\nAGC\tS\nAGA\tR\nAGG\tR\nGGT\tG\nGGC\tG\nGGA\tG\nGGG\tG\t" > genetic_code.txt

#Join both files to obtain the final codon usage file:
join -1 2 -2 1 <(sort -k2 all_genes_sequence_combined_codon_counts.txt) <(sort genetic_code.txt) | sort -k3,3 -k2,2nr | awk '{printf ("%s\t%s\t%s\n", $1, $3, $2)}' > codon_usage.txt
...
rm synonymous_variants_complete_info.txt
echo -e "scaffold\tmutated_position\tgene\tstrand\tmutated_codon_N\tmutated_nucleotide_N\twild_codon\tmutated_codon\tmutated_base\twild_sequence" > synonymous_variants_complete_info.txt
...
while read -r entry; do
  ...
  #echo $GENE
  STRAND=$(grep $GENE synonymous_variants_cds_list_and_sequence_combined.txt | cut -f 2)
  #echo $STRAND
  CODING_SEQUENCE=$(grep $GENE synonymous_variants_cds_list_and_sequence_combined.txt | cut -f 3)
  #echo $CODING_SEQUENCE  
  if [ $STRAND == "+" ]
    then
  ...
  #echo $MUTATED_CODON
  echo -e "$SCAFFOLD\t$SNP\t$GENE\t$STRAND\t$AFFECTED_CODON\t$AFFECTED_NUCLEOTIDE\t$WILD_CODON\t$MUTATED_CODON\t$MUTATED_BASE\t$FINAL_SEQUENCE" >> synonymous_variants_complete_info.txt
  ((COUNTER++))
  if [ $(( $COUNTER % 100 )) == 0 ]
    then
